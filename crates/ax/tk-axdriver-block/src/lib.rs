...
/// Immutable geometry of one block-addressable device or logical volume.
///
/// This is deliberately a value rather than a pair of ad-hoc trait calls: a
/// volume snapshots it once while it is assembled, so an upper filesystem has
/// a stable address space for the lifetime of its mount.
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
...
impl BlockGeometry {
    /// Total addressable capacity in bytes, if it fits in `u64`.
    pub const fn capacity_bytes(self) -> Option<u64> {
        self.blocks.checked_mul(self.block_size as u64)
    }

    /// Validates a block range against this geometry.
    pub const fn contains(self, range: BlockRange) -> bool {
        range.start <= self.blocks && range.blocks <= self.blocks - range.start
    }
}

/// A whole-block range.  Byte-granular callers must perform their own
/// read/modify/write; block drivers never silently round a destructive range.
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
...
/// Explicit storage features.  An unsupported operation is never emulated by
/// a different persistence contract without the upper layer opting into it.
#[derive(Debug, Clone, Copy, Default, Eq, PartialEq)]
pub struct BlockCapabilities {
    /// A completed flush makes earlier writes durable according to the device.
    pub flush: bool,
    /// A write can be issued with force-unit-access semantics.
    pub fua: bool,
    /// A non-flushing ordering fence is supported.
    pub fence: bool,
    /// Ranges can be discarded/deallocated.
    pub discard: bool,
    /// Ranges can be made zero without a caller-owned zero buffer.
...
pub trait BlockDriverOps: BaseDriverOps {
    ...
    /// Returns a self-consistent geometry snapshot for this device.
    fn block_geometry(&self) -> DevResult<BlockGeometry> {
        let geometry = BlockGeometry {
            block_size: self.block_size(),
            blocks: self.num_blocks(),
        };
        if geometry.block_size == 0 || geometry.capacity_bytes().is_none() {
            return Err(DevError::BadState);
        }
        Ok(geometry)
    }

    /// Returns typed optional storage capabilities.
    ...
    /// Writes blocks with force-unit-access semantics.
    fn write_block_fua(&mut self, block_id: u64, buf: &[u8]) -> DevResult {
        let _ = (block_id, buf);
        Err(DevError::Unsupported)
    }

    /// Establishes ordering for previously accepted writes without adding a
    /// device-cache persistence boundary.
    fn fence(&mut self) -> DevResult {
        Err(DevError::Unsupported)
    }

    /// Deallocates a whole-block range.  A subsequent read is device-defined;
    /// callers needing zero-filled reads must use `write_zeroes` instead.
    fn discard_blocks(&mut self, range: BlockRange) -> DevResult {
        let _ = range;
        Err(DevError::Unsupported)
    }

    /// Writes logical zeroes to a whole-block range.
