...
fn test_drain_allocation_failure_preserves_present_leaves() -> PagingResult<()> {
    ...
    let result = {
        let mut cursor = table.cursor_no_flush();
        cursor.drain_mapped_leaves(VirtAddr::from_usize(0), 0x20_000)
    };
    clear_heap_allocation_failure();
...
fn test_collect_and_drain_use_one_exact_leaf_allocation() -> PagingResult<()> {
    ...
    let collected = table.collect_mapped_leaves(VirtAddr::from_usize(0), 0x20_000);
...
#[test]
fn test_mapped_bytes_sparse_protnone_and_huge_x86() -> PagingResult<()> {
    ...
    let distant = 1usize << 39;
    for (address, size) in [(0x1000, PageSize::Size4K),
        (0x20_0000, PageSize::Size2M), (distant, PageSize::Size1G)] {
        table.cursor_no_flush().map(VirtAddr::from_usize(address),
            PhysAddr::from_usize(size as usize), size, MappingFlags::READ)?;
    }
    table.cursor_no_flush().protect_region(VirtAddr::from_usize(0x1000),
        0x1000, MappingFlags::empty())?;
    table.cursor_no_flush().protect_region(VirtAddr::from_usize(0x20_0000),
        0x20_0000, MappingFlags::empty())?;
    assert!(table.query(VirtAddr::from_usize(0x1000)).is_err());
    for (start, size) in [(0, 0x50_0000), (0x20_1000, 0x3000),
        (0x1f_f000, 0x2000), (0x3f_f000, 0x2000),
        (distant + 0x1000, 0x5000), (0x8000, 0x1000)] {
        let expected = (start..start + size).step_by(4096)
            .filter(|address| table.query_mapped(VirtAddr::from_usize(*address)).is_ok())
            .count() * 4096;
        assert_eq!(table.mapped_bytes(VirtAddr::from_usize(start), size)?, expected);
    }
    HEAP_ALLOCATION_COUNT.with(|count| count.set(0));
    TRACK_HEAP_ALLOCATIONS.with(|tracking| tracking.set(true));
    let sparse_bytes = table.mapped_bytes(VirtAddr::from_usize(0), distant + (1 << 30));
    TRACK_HEAP_ALLOCATIONS.with(|tracking| tracking.set(false));
    assert_eq!(HEAP_ALLOCATION_COUNT.with(Cell::get), 0);
    assert_eq!(sparse_bytes?, 4096 + (1 << 21) + (1 << 30));
    assert_eq!(table.mapped_bytes(VirtAddr::from_usize(0), 0)?, 0);
    assert_eq!(table.mapped_bytes(VirtAddr::from_usize(1), 4096), Err(PagingError::NotAligned));
    assert_eq!(table.mapped_bytes(VirtAddr::from_usize(0), 1), Err(PagingError::NotAligned));
    assert_eq!(table.mapped_bytes(VirtAddr::from_usize(usize::MAX - 4095), 4096),
        Err(PagingError::NotAligned));
    assert_eq!(table.mapped_bytes(VirtAddr::from_usize(1usize << 47), 4096),
        Err(PagingError::NotAligned));
    let high = 0xffff_8000_0000_1000usize;
    table.cursor_no_flush().map(VirtAddr::from_usize(high), PhysAddr::from_usize(0x1000),
        PageSize::Size4K, MappingFlags::READ)?;
    assert_eq!(table.mapped_bytes(VirtAddr::from_usize(high - 4096), 0x3000)?, 4096);
    drop(table);
...
fn test_collect_mapped_leaves_x86() -> PagingResult<()> {
    ...
    let sparse = table.collect_mapped_leaves(VirtAddr::from_usize(0), 0x20_000)?;
    assert_eq!(
        sparse,
        vec![
            (
    ...
    assert!(matches!(
        table.collect_mapped_leaves(VirtAddr::from_usize(0x20_1000), 0x1000),
        Err(PagingError::NotAligned)
    ));
...
#[test]
fn test_collect_mapped_leaves_nonpresent_tables() -> PagingResult<()> {
    ALLOCATED.with_borrow_mut(|it| it.clear());
    ...
    let sparse = table.collect_mapped_leaves(VirtAddr::from_usize(0x3fff_9000), 0x3000)?;
    assert_eq!(
        sparse,
        vec![
            (
...
#[test]
fn test_drain_mapped_leaves_x86() -> PagingResult<()> {
    type Meta = page_table_multiarch::x86_64::X64PagingMetaData;
    type Pte = page_table_entry::x86_64::X64PTE;
    ...
    assert_eq!(
        table.collect_mapped_leaves(VirtAddr::from_usize(0), 0x20_000)?,
        Vec::new()
    );
    ...
    assert!(matches!(
        {
            let mut cursor = table.cursor_no_flush();
            cursor.drain_mapped_leaves(VirtAddr::from_usize(0x20_1000), 0x1000)
        },
        Err(PagingError::NotAligned)
    ));
    assert_eq!(
        table.collect_mapped_leaves(VirtAddr::from_usize(0x20_0000), 0x20_0000)?,
        vec![(
            VirtAddr::from_usize(0x20_0000),
            PhysAddr::from_usize(0x40_0000),
            MappingFlags::READ,
    ...
    assert!(matches!(
        {
            let mut cursor = table.cursor_no_flush();
            cursor.drain_mapped_leaves(VirtAddr::from_usize(0x10_0000), 0x101000)
        },
        Err(PagingError::NotAligned)
    ));
    assert_eq!(
        table.collect_mapped_leaves(VirtAddr::from_usize(0x10_0000), 0x30_0000)?,
        vec![
            (
                VirtAddr::from_usize(0x10_0000),
                PhysAddr::from_usize(0x50_0000),
...
#[test]
fn test_drain_mapped_leaves_nonpresent_tables() -> PagingResult<()> {
    ALLOCATED.with_borrow_mut(|it| it.clear());
    ...
    let drained = {
        let mut cursor = table.cursor_no_flush();
        cursor.drain_mapped_leaves(VirtAddr::from_usize(0x3fff_9000), 0x3000)?
    };
    assert_eq!(
        drained,
        vec![
    ...
    assert_eq!(
        table.collect_mapped_leaves(VirtAddr::from_usize(0x3fff_9000), 0x3000)?,
        vec![]
    );
...
#[test]
fn test_protect_region_restores_prot_none_owned_leaves_x86() -> PagingResult<()> {
    type Meta = page_table_multiarch::x86_64::X64PagingMetaData;
    type Pte = page_table_entry::x86_64::X64PTE;
    use page_table_entry::x86_64::Pkey;
    ...
    for size in [PageSize::Size4K, PageSize::Size2M, PageSize::Size1G] {
        ...
        {
            let mut cursor = table.cursor_no_flush();
            cursor.map(address, physical, size, flags)?;
            cursor.set_pkey(address, key)?;
            cursor.protect_region(address, size as usize, MappingFlags::empty())?;
        }
        assert_eq!(table.query(address), Err(PagingError::NotMapped));
        let allocated_before = ALLOCATED.with_borrow(|frames| frames.len());
        {
            let mut cursor = table.cursor_no_flush();
            cursor.protect_region(address, size as usize, flags)?;
            assert_eq!(cursor.pkey(address)?, (key, size));
        }
        assert_eq!(table.query(address)?, (physical, flags.with_pkey(3), size));
        assert_eq!(ALLOCATED.with_borrow(|frames| frames.len()), allocated_before);
        let hole = address + size as usize;
        table.cursor_no_flush().protect_region(hole, size as usize, flags)?;
        assert_eq!(table.query(hole), Err(PagingError::NotMapped));
        table.cursor_no_flush().unmap(address)?;
        drop(table);
    }
    assert_eq!(ALLOCATED.with_borrow(|frames| frames.len()), 0);
    Ok(())
}

#[test]
fn test_prot_none_mapping_ownership_clone_rekey_and_drain_x86() -> PagingResult<()> {
    type Meta = page_table_multiarch::x86_64::X64PagingMetaData;
    type Pte = page_table_entry::x86_64::X64PTE;
    use page_table_entry::x86_64::Pkey;
    ALLOCATED.with_borrow_mut(|it| it.clear());
    for size in [PageSize::Size4K, PageSize::Size2M, PageSize::Size1G] {
        let mut source = PageTable64::<Meta, Pte, TrackPagingHandler<Meta>>::try_new()?;
        let mut child = PageTable64::<Meta, Pte, TrackPagingHandler<Meta>>::try_new()?;
        let address = VirtAddr::from_usize(0x4000_0000);
        // Physical zero must remain distinguishable from an unused leaf.
        source.cursor_no_flush().map(address, PhysAddr::from_usize(0), size, MappingFlags::READ)?;
        source.cursor_no_flush().protect_region(address, size as usize, MappingFlags::empty())?;
        assert_eq!(source.query(address), Err(PagingError::NotMapped));
        source.cursor_no_flush().set_pkey(address, Pkey::new(5).unwrap())?;
        let leaves = source.collect_mapped_leaves(address, size as usize)?;
        assert_eq!(leaves, vec![(address, PhysAddr::from_usize(0), MappingFlags::empty().with_pkey(5), size)]);
        for &(va, pa, flags, size) in &leaves {
            child.cursor_no_flush().map(va, pa, size, flags)?;
        }
        assert_eq!(child.query(address), Err(PagingError::NotMapped));
        child.cursor_no_flush().protect_region(address, size as usize, MappingFlags::READ)?;
        assert_eq!(child.query(address)?, (PhysAddr::from_usize(0), MappingFlags::READ.with_pkey(5), size));
        assert_eq!(source.cursor_no_flush().drain_mapped_leaves(address, size as usize)?, leaves);
        assert_eq!(source.query_mapped(address), Err(PagingError::NotMapped));
        source.cursor_no_flush().map(address, PhysAddr::from_usize(0x8000_0000), size, MappingFlags::READ)?;
        source.cursor_no_flush().unmap(address)?;
        child.cursor_no_flush().unmap(address)?;
    }
    assert_eq!(ALLOCATED.with_borrow(|frames| frames.len()), 0);
    Ok(())
}
...
#[test]
fn test_prot_none_huge_demotion_preserves_frame_offsets_and_keys_x86() -> PagingResult<()> {
    type Meta = page_table_multiarch::x86_64::X64PagingMetaData;
    type Pte = page_table_entry::x86_64::X64PTE;
    use page_table_entry::x86_64::Pkey;
    ALLOCATED.with_borrow_mut(|it| it.clear());
    ...
    table.cursor_no_flush().map(start, PhysAddr::from_usize(0), PageSize::Size2M,
                               MappingFlags::empty().with_pkey(5))?;
    assert_eq!(table.collect_mapped_leaves(start + 4096, 4096), Err(PagingError::NotAligned));
    assert_eq!(table.collect_overlapping_mapped_leaves(start + 4096, 4096)?,
               vec![(start, PhysAddr::from_usize(0), MappingFlags::empty().with_pkey(5), PageSize::Size2M)]);
    let mut prepared = PreparedPageTableFrames::<TrackPagingHandler<Meta>>::try_new(1)
        .map_err(|_| PagingError::NoMemory)?;
    table.cursor_no_flush().demote_leaf_to_4k_prepared(start + 4096, &mut prepared)?;
    table.cursor_no_flush().set_pkey(start + 4096, Pkey::new(7).unwrap())?;
    assert_eq!(table.query(start + 4096), Err(PagingError::NotMapped));
    assert_eq!(table.query_mapped(start)?,
               (PhysAddr::from_usize(0), MappingFlags::empty().with_pkey(5), PageSize::Size4K));
    assert_eq!(table.query_mapped(start + 4096)?,
               (PhysAddr::from_usize(4096), MappingFlags::empty().with_pkey(7), PageSize::Size4K));
    let retired = table.cursor_no_flush().drain_mapped_leaves(start, PageSize::Size2M as usize)?;
    assert_eq!(retired.len(), 512);
    assert_eq!(retired[511].1, PhysAddr::from_usize(511 * 4096));
    drop(prepared);
    drop(table);
    assert_eq!(ALLOCATED.with_borrow(|frames| frames.len()), 0);
    Ok(())
...
#[test]
fn test_pkey_1g_range_demotion_preserves_boundary_neighbors_x86() -> PagingResult<()> {
    type Meta = page_table_multiarch::x86_64::X64PagingMetaData;
    type Pte = page_table_entry::x86_64::X64PTE;
    use page_table_entry::x86_64::Pkey;
    ALLOCATED.with_borrow_mut(|it| it.clear());
    let chunk = PageSize::Size2M as usize;
    for (offset, size, frames) in [
        (17 * chunk + 4096, 4096, 2),
        (17 * chunk + 4096, 3 * chunk, 3),
        (17 * chunk, 3 * chunk, 1),
    ] {
        let mut table = PageTable64::<Meta, Pte, TrackPagingHandler<Meta>>::try_new()?;
        let base = VirtAddr::from_usize(0x4000_0000);
        let flags = MappingFlags::empty().with_pkey(5);
        table.cursor_no_flush().map(base, PhysAddr::from_usize(0), PageSize::Size1G, flags)?;
        let mut insufficient = PreparedPageTableFrames::<TrackPagingHandler<Meta>>::try_new(frames - 1)
            .map_err(|_| PagingError::NoMemory)?;
        assert_eq!(table.cursor_no_flush().demote_leaf_for_range_prepared(base + offset, size, &mut insufficient), Err(PagingError::NoMemory));
        assert_eq!(table.query_mapped(base)?.2, PageSize::Size1G);
        drop(insufficient);
        let mut prepared = PreparedPageTableFrames::<TrackPagingHandler<Meta>>::try_new(frames)
            .map_err(|_| PagingError::NoMemory)?;
        table.cursor_no_flush().demote_leaf_for_range_prepared(base + offset, size, &mut prepared)?;
        table.cursor_no_flush().set_pkey_region(base + offset, size, Pkey::new(7).unwrap())?;
        for page in 0..(PageSize::Size1G as usize / 4096) {
            let address = page * 4096;
            let (pa, actual, _) = table.query_mapped(base + address)?;
            assert_eq!(pa, PhysAddr::from_usize(address));
            assert_eq!(actual, MappingFlags::empty().with_pkey(if address >= offset && address < offset + size { 7 } else { 5 }));
        }
        assert_eq!(table.query_mapped(base)?.2, PageSize::Size2M);
        // A later operation in a previously untouched child must retain the
        // keys and physical offsets established by the first operation.
        let later = base + 31 * chunk + 4096;
        let mut next = PreparedPageTableFrames::<TrackPagingHandler<Meta>>::try_new(1)
            .map_err(|_| PagingError::NoMemory)?;
        table.cursor_no_flush().demote_leaf_for_range_prepared(later, 4096, &mut next)?;
        table.cursor_no_flush().set_pkey_region(later, 4096, Pkey::new(9).unwrap())?;
        assert_eq!(table.query_mapped(later - 4096)?.1, flags);
        assert_eq!(table.query_mapped(later + 4096)?.1, flags);
        assert_eq!(table.query_mapped(base + offset)?.1, MappingFlags::empty().with_pkey(7));
        drop(next);
        drop(prepared);
        drop(table);
        assert_eq!(ALLOCATED.with_borrow(|frames| frames.len()), 0);
