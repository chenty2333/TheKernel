...
impl AddrSpace {
    /// Returns the resolved leaf mapping.  Text-patch users need the original
    /// leaf flags so a temporary writable permission can be restored exactly
    /// rather than assuming kernel text is uniformly RX.
    pub fn query_leaf(&self, vaddr: VirtAddr) -> AxResult<(PhysAddr, MappingFlags, PageSize)> {
        self.pt.query(vaddr).map_err(AxError::from)
    }
