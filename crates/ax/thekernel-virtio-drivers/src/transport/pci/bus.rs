...
/// The offset of the primary, secondary and subordinate bus numbers in a
/// PCI-to-PCI bridge header.
const BRIDGE_BUS_NUMBERS_OFFSET: u8 = 0x18;
...
impl PciRoot {
    ...
    /// Returns the bus-number routing fields from a PCI-to-PCI bridge.
    ///
    /// Callers must first establish that `device_function` has a
    /// [`HeaderType::PciPciBridge`] header. This deliberately exposes only the
    /// read-only topology fields needed by PCI discovery, rather than generic
    /// configuration-space access.
    pub fn bridge_bus_numbers(&self, device_function: DeviceFunction) -> BridgeBusNumbers {
        decode_bridge_bus_numbers(self.config_read_word(device_function, BRIDGE_BUS_NUMBERS_OFFSET))
    }
    ...
    pub(crate) fn command_register(&self, device_function: DeviceFunction) -> *mut u16 {
        let address = self.cam_offset(device_function, STATUS_COMMAND_OFFSET);
        // The root's CAM mapping is permanent; this selects only the command
        // halfword so a write cannot acknowledge adjacent PCI status bits.
        unsafe { self.mmio_base.add((address >> 2) as usize).cast() }
    }
...
/// Bus-number routing fields from a PCI-to-PCI bridge header.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub struct BridgeBusNumbers {
    /// The bus containing the bridge function.
    pub primary: u8,
    /// The directly attached downstream bus.
    pub secondary: u8,
    /// The highest bus number reachable through the bridge.
    pub subordinate: u8,
}
...
/// Decodes the PCI bridge bus-number dword at offset `0x18`.
const fn decode_bridge_bus_numbers(config: u32) -> BridgeBusNumbers {
    BridgeBusNumbers {
        primary: config as u8,
        secondary: (config >> 8) as u8,
        subordinate: (config >> 16) as u8,
    }
}
...
mod tests {
    ...
    #[test]
    fn bridge_bus_numbers_decode_config_layout() {
        assert_eq!(
            decode_bridge_bus_numbers(0xa5_34_12),
            BridgeBusNumbers {
                primary: 0x12,
                secondary: 0x34,
                subordinate: 0xa5,
