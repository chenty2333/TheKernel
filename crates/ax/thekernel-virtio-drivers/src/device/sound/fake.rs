...
pub struct FakeSoundDevice {
    /// Owns the transport's borrowed configuration registers at a stable
    /// address. Keep this fake (or a clone) alive until its transport is dropped.
    _config_space: Arc<Box<VirtIOSoundConfig>>,
...
impl FakeSoundDevice {
    pub fn new(
        ...
        let mut config_space = Box::new(VirtIOSoundConfig {
        ...
        let transport = FakeTransport {
            ...
            max_queue_size: 32,
            device_features: 0,
            config_space: NonNull::from(config_space.as_mut()),
            state: state.clone(),
        };
        ...
        (
            Self {
                _config_space: Arc::new(config_space),
                state,
