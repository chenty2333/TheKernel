...
use crate::{
    ...
    ExportHandleDecodeMode, ExportHandleMode, Filesystem, FilesystemIdentity, FilesystemOps,
    Metadata, MetadataUpdate, Mutex, MutexGuard, NamedCreateOptions, NamespaceGeneration,
    NodeFlags, NodePermission, NodeType, NodeUserData, ObjectKey, OpenOptions, TypeMap, VfsError,
    VfsResult, WeakFilesystemIdentity, XattrProvider, XattrSetMode,
    path::{DOT, DOTDOT, FsName, FsPath, FsPathBuf, try_build_absolute_path},
    unsupported_xattr,
};
...
pub struct Mountpoint {
    ...
    children: Mutex<HashMap<ObjectKey, Arc<Self>>>,
    ...
    /// Linux `IS_MNT_LOCKED` equivalent.  Namespace construction sets this
    /// when a copied tree crosses into a less-privileged user namespace;
    /// placement mutations may read it without consulting task state.
    placement_locked: AtomicBool,
...
impl MountExtensions {
    ...
    fn initialize(&self, extensions: TypeMap) -> VfsResult<()> {
        self.initialize_with(|| extensions)
    }

    /// Installs one immutable extension map.  Keeping construction in the
    /// `Once` closure lets callers which need it defer expensive map creation
    /// until they actually win initialization.
    fn initialize_with<F>(&self, extensions: F) -> VfsResult<()>
    where
        F: FnOnce() -> TypeMap,
    {
...
struct MountHandle {
    ...
    /// Even values admit new `Location`s; an odd value is a sealed normal
    /// unmount epoch.  `FlushedUnmount` owns that epoch across the backend
    /// flush, so its later topology commit cannot be invalidated by a fresh
    /// pathname crossing or a transient cloned Location.
...
impl WritebackAnchor {
    ...
    /// Best-effort delayed writeback counterpart of
    /// [`Location::flush_metadata_time_overlay`].  The anchor deliberately
    /// has no mount handle, but retains the stable entry and filesystem needed
    /// to make deferred inode timestamps durable before cache reclaim.
    pub fn flush_metadata_time_overlay(&self) -> VfsResult<()> {
        let Some(overlay) = self
            .entry
            .persistent_user_data()
            .and_then(NodeUserData::metadata_time_overlay)
        else {
            return Ok(());
        };
        let reservation = overlay.reserve_blocking();
        let pending = reservation.pending_snapshot();
        if pending.is_empty() {
            return Ok(());
        }
        let mut update = MetadataUpdate::default();
        update.atime = pending.atime;
        update.mtime = pending.mtime;
        update.ctime = pending.ctime;
        update.retain_supported(self.entry.filesystem().metadata_update_capabilities());
        if update.is_empty() {
            return Ok(());
        }
        self.entry.update_metadata(update)?;
        reservation.clear_flushed(pending);
        Ok(())
    }
}
...
impl Mountpoint {
    ...
    fn new_with_root(
        ...
        let mountpoint = Arc::new(Self {
            ...
            placement_locked: AtomicBool::new(false),
            handle,
    ...
    /// Creates a namespace-root mount whose visible root is an existing
    /// directory in `fs`.  Namespace cloning uses this for bind/subtree
    /// mounts: selecting `fs.root_dir()` here would silently widen a cloned
    /// mount from its recorded subtree to the whole filesystem.
    pub fn new_root_at_with_extensions(
        fs: &Filesystem,
        root: DirEntry,
        extensions: TypeMap,
    ) -> VfsResult<Arc<Self>> {
        Self::try_new_with_root(fs, root, None, Some(extensions), true, false)
    }
    ...
    pub fn new_detached_with_extensions(
        ...
        Self::new_detached_at_with_extensions(fs, fs.root_dir(), extensions)
    }

    /// Creates a detached mount rooted at an existing directory in `fs`.
    /// The root dentry is part of mount identity, independently of the
    /// superblock, so callers cloning a bind/subtree mount must retain it.
    pub fn new_detached_at_with_extensions(
        fs: &Filesystem,
        root: DirEntry,
        extensions: TypeMap,
    ) -> VfsResult<Arc<Self>> {
    ...
    /// Permanently prevents placement-sensitive mount mutations on this
    /// mount.  There is deliberately no clear operation: Linux mount locks
    /// are inherited restrictions, not a user-toggleable mount attribute.
    pub fn lock_placement(&self) {
        self.placement_locked.store(true, Ordering::Release);
    }

    pub fn is_placement_locked(&self) -> bool {
        self.placement_locked.load(Ordering::Acquire)
    }
    ...
    fn effective_mountpoint_locked(self: &Arc<Self>) -> VfsResult<Arc<Mountpoint>> {
        ...
        for _ in 0..MAX_MOUNT_TREE_DEPTH {
            ...
            let next = mountpoint
                .children
                .lock()
                .get(&mountpoint.root.object_key())
                .cloned();
    ...
    fn validate_detach_locked(self: &Arc<Self>, require_unused: bool) -> VfsResult<()> {
        ...
        if !children
            .get(&parent_location.entry.object_key())
    ...
    fn detach_prevalidated_from_parent_locked(self: &Arc<Self>) -> VfsResult<()> {
        ...
        children.remove(&parent_location.entry.object_key());
        *location = None;
        Ok(())
    }
...
impl PreparedUnmount {
    pub fn flush(mut self) -> VfsResult<FlushedUnmount> {
        ...
        location.mountpoint().filesystem.flush_for_unmount()?;
...
impl FlushedUnmount {
    pub fn commit(mut self) -> VfsResult<()> {
        // Keep the receipt installed until commit succeeds.  A racing
        // Location admission makes normal unmount busy; `Drop` then cancels
        // the sealed epoch before returning that error to the caller.
        self.location
            .as_ref()
            .ok_or(VfsError::Io)?
            .commit_prepared_unmount(self.sealed_epoch)?;
        self.location.take();
        Ok(())
    }
}
...
/// Admit one Location without racing a flushed normal-unmount receipt.
///
/// The odd/even epoch makes the sealing edge observable even after the
/// prepare code releases its short writer lock for backend I/O.  A Location
/// which reaches the sealed epoch is admitted and marks the receipt stale;
/// the later commit then returns `ResourceBusy` and the receipt's drop
/// unseals it.  In particular, this must not spin while the caller holds a
/// mount-tree read lock: commit and cancellation require the tree writer.
fn admit_location(mount: &Arc<MountHandle>, mountpoint: &Arc<Mountpoint>) -> usize {
    // The admission lock closes the race with prepare's even-to-odd store.
    // Holding it as a reader is bounded: unlike the old epoch spin it never
    // waits for normal-unmount commit, which also needs MOUNT_TREE_LOCK's
    // writer side.
    let _admission = mount.admission.read();
    let epoch = mount.admission_epoch.load(Ordering::Acquire);
    let previous = mount.users.fetch_add(1, Ordering::AcqRel);
    debug_assert!(previous != usize::MAX, "mount Location count overflow");
    if epoch & 1 != 0 {
        debug_assert!(mountpoint.unmounting.load(Ordering::Acquire));
...
impl Clone for Location {
    fn clone(&self) -> Self {
        admit_location(&self.mount, &self.mountpoint);
        Self {
...
impl Location {
    ...
    /// Tests ancestry across either one mount view or two bind views of the
    /// same live filesystem.  Cross-view ancestry is delegated to the
    /// backend's stable export identity; a backend that cannot establish it
    /// returns an error so security-sensitive mount admission can fail closed.
    pub fn is_same_or_ancestor_in_filesystem(&self, descendant: &Location) -> VfsResult<bool> {
        if self.same_mount(descendant) {
            return Ok(entry_is_same_or_ancestor_by_inode(
                &self.entry,
                &descendant.entry,
            ));
        }
        let filesystem = self.mountpoint().filesystem_handle();
        // Bind mounts create a distinct Filesystem wrapper around the source
        // root, but retain this immutable identity.  Wrapper pointer equality
        // would therefore let a staging directory hide behind a bind view.
        if filesystem.identity().device()
            != descendant
                .mountpoint()
                .filesystem_handle()
                .identity()
        ...
        filesystem.export_handle_is_descendant(&self.entry, &descendant.entry)
    }
    pub fn inode(&self) -> u64;

    pub fn object_key(&self) -> crate::ObjectKey;
    ...
    pub fn file_attr_provider(&self) -> Option<&dyn crate::FileAttrProvider>;

    pub fn get_file_attr(&self) -> VfsResult<crate::FileAttr>;

    pub fn try_get_file_attr(&self) -> VfsResult<crate::FileAttr>;
...
impl Location {
    ...
    fn new_locked(mountpoint: Arc<Mountpoint>, entry: DirEntry) -> Self {
        ...
        let previous = admit_location(&mount, &mountpoint);
        if previous == 0 {
            mountpoint.refresh_handle_ancestors_locked();
        }
        Self {
    ...
    pub fn metadata_capabilities(&self, metadata: &Metadata) -> crate::MetadataCapabilities {
        self.entry.metadata_capabilities(metadata)
    }
    ...
    pub fn metadata(&self) -> VfsResult<Metadata> {
        ...
        if let Some(overlay) = self
            ...
            .and_then(NodeUserData::metadata_time_overlay)
        {
            overlay.snapshot().apply_to(&mut metadata);
        }
        Ok(metadata)
    }

    /// Strict metadata update with the inode's deferred NOWAIT timestamps
    /// folded into the provider transaction.  Holding the overlay writer
    /// permit prevents a successful provider flush from losing a newer
    /// timestamp publication; ordinary metadata readers only take a bounded
    /// lock-free snapshot and never wait for this operation.
    pub fn update_metadata(&self, mut update: MetadataUpdate) -> VfsResult<()> {
        let Some(overlay) = self
            .entry
            .persistent_user_data()
            .and_then(NodeUserData::metadata_time_overlay)
        else {
            return self.entry.update_metadata(update);
        };
        let reservation = overlay.reserve_blocking();
        let pending = reservation.pending_snapshot();
        if update.atime.is_none() {
            update.atime = pending.atime;
        }
        if update.mtime.is_none() {
            update.mtime = pending.mtime;
        }
        if update.ctime.is_none() {
            update.ctime = pending.ctime;
        }
        self.entry.update_metadata(update)?;
        reservation.clear_flushed(pending);
    ...
    fn begin_file_attr_update(&self) -> VfsResult<Option<crate::FileAttrUpdateGuard>> {
        if self.entry.file_attr_provider().is_none() {
            return Ok(None);
        }
        let data = self
            .entry
        ...
        Ok(Some(data.begin_file_attr_update()))
    }

    /// Serialized with every VFS-side cached write for this stable inode.
    /// Composed NodeOps cannot bypass this because the public Location facade
    /// owns all fileattr setter routing.
    pub fn set_file_attr(&self, attr: crate::FileAttr) -> VfsResult<()> {
        let _update = self.begin_file_attr_update()?;
        self.entry.set_file_attr(attr)
    }

    pub fn set_legacy_file_flags(&self, flags: u32) -> VfsResult<()> {
        let _update = self.begin_file_attr_update()?;
        self.entry.set_legacy_file_flags(flags)
    }
    ...
    /// Persists the currently projected NOWAIT timestamps.  The exact
    /// snapshot is cleared only after the provider accepts it while holding
    /// the overlay writer permit, so a later publication cannot be erased by
    /// an older flush.
    pub fn flush_metadata_time_overlay(&self) -> VfsResult<()> {
        let Some(overlay) = self
            .entry
    ...
    pub fn path_in_mount(&self) -> VfsResult<FsPathBuf> {
        let mut components = Vec::new();
    ...
    pub fn is_mountpoint(&self) -> bool {
        ...
        self.mountpoint()
            .children
            .lock()
            .contains_key(&self.entry.object_key())
    }

    /// Returns the exact topmost child mounted on this covered dentry.
    /// Namespace mutation preflight uses this to distinguish a retained lower
    /// mount FD from the mount currently visible at its old mountpoint.
    pub fn mounted_child(&self) -> Option<Arc<Mountpoint>> {
        let _tree = MOUNT_TREE_LOCK.read();
        self.mountpoint()
            .children
            .lock()
            .get(&self.entry.object_key())
            .cloned()
    }
    ...
    pub fn lookup_no_follow_in_mount(&self, name: &FsName) -> VfsResult<Self> {
        if name == DOT {
            Ok(self.clone())
        } else if name == DOTDOT {
    ...
    pub fn lookup_no_follow(&self, name: &FsName) -> VfsResult<Self> {
        ...
        if name == DOT || name == DOTDOT {
            self.lookup_no_follow_in_mount(name)
        } else {
            self.lookup_no_follow_in_mount(name)?.resolve_mountpoint()
        }
    }
    ...
    pub fn create_symlink_prepared(
        ...
        self.entry
            .as_dir()?
            .create_symlink_prepared(name, target, options)
            .map(|entry| self.wrap(entry))
    }
    ...
    /// Returns whether this directory supplies a native atomic exchange.
    ...
    pub fn link(&self, name: &FsName, node: &Self) -> VfsResult<Self> {
    ...
    pub fn rename(&self, src_name: &FsName, dst_dir: &Self, dst_name: &FsName) -> VfsResult<()> {
    ...
    /// Exchanges the two exact names prepared by a higher-level transaction.
    /// Unlike ordinary rename, both entries are mandatory and neither is
    /// treated as a replacement victim.
    pub fn rename_exchange_checked(
        &self,
        src_name: &FsName,
        src: &Self,
        dst_dir: &Self,
        ...
        self.validate_rename_ancestry_checked(src, dst_dir, Some(dst))?;
        if src.same_node(dst) {
            return Ok(());
        }
        if !self.supports_rename_exchange() {
            return Err(VfsError::OperationNotSupported);
        }
        {
            let _tree = MOUNT_TREE_LOCK.read();
            if self.mountpoint().has_mount_at_or_below_locked(&src.entry)?
                || self.mountpoint().has_mount_at_or_below_locked(&dst.entry)?
            {
                return Err(VfsError::ResourceBusy);
            }
        }
        self.entry.as_dir()?.rename_exchange(
            src_name,
            &src.entry,
            dst_dir.entry.as_dir()?,
            dst_name,
    ...
    /// Performs a checked backend-native whiteout rename.  The public
    /// namespace transaction owns policy; this method owns exact identities,
    /// mount topology, and the one backend commit.
    pub fn rename_whiteout_checked(
        &self,
        src_name: &FsName,
        src: &Self,
        dst_dir: &Self,
        ...
        self.validate_rename_ancestry_checked(src, dst_dir, dst)?;
        if !self.entry.as_dir()?.supports_rename_whiteout() {
            return Err(VfsError::OperationNotSupported);
        }
        {
            let _tree = MOUNT_TREE_LOCK.read();
            let destination_is_mountpoint = match dst {
                Some(entry) => self
                    .mountpoint()
                    .has_mount_at_or_below_locked(&entry.entry)?,
                None => false,
            };
            if self.mountpoint().has_mount_at_or_below_locked(&src.entry)?
                || destination_is_mountpoint
            {
                return Err(VfsError::ResourceBusy);
            }
        }
        self.entry.as_dir()?.rename_whiteout(
            src_name,
            &src.entry,
            dst_dir.entry.as_dir()?,
            dst_name,
            dst.map(|entry| &entry.entry),
        )
    }
    ...
    pub fn unlink_checked(&self, name: &FsName, is_dir: bool, expected: &Self) -> VfsResult<()> {
    ...
    fn unlink_entry_checked(
    ...
    ) -> VfsResult<()> {
        {
            ...
            if self
                ...
                .children
                .lock()
                .contains_key(&expected.object_key())
            {
                return Err(VfsError::ResourceBusy);
            }
        }
    ...
    pub fn open_file(&self, name: &FsName, options: &OpenOptions) -> VfsResult<Location> {
        self.entry
            .as_dir()?
    ...
    pub fn move_mount_to(&self, target: &Self) -> VfsResult<()> {
        ...
        let target_key = target.entry.object_key();
        {
        ...
        if let Some(old_location) = location.as_ref() {
            ...
            if !old_children
                .get(&old_location.entry.object_key())
                .is_some_and(|mounted| Arc::ptr_eq(mounted, self.mountpoint()))
            {
                return Err(VfsError::InvalidInput);
            }
            old_children.remove(&old_location.entry.object_key());
        }
    ...
    pub fn pivot_root_to(&self, put_old: &Self) -> VfsResult<()> {
        ...
        let old_parent = new_location
            ...
            .ok_or(VfsError::InvalidInput)?;
        let new_key = new_location.as_ref().unwrap().entry.object_key();
        let put_old_key = put_old.entry.object_key();
        if !old_parent
            .children
            .lock()
            .get(&new_key)
    ...
    pub fn prepare_unmount(self) -> VfsResult<PreparedUnmount> {
        ...
        let sealed_epoch = {
            ...
            let open_epoch = self.mount.admission_epoch.load(Ordering::Acquire);
            debug_assert_eq!(open_epoch & 1, 0);
            let sealed_epoch = open_epoch.checked_add(1).ok_or(VfsError::Io)?;
            self.mount
                .admission_epoch
                .store(sealed_epoch, Ordering::Release);
    ...
    fn cancel_prepared_unmount(&self) {
        ...
        let sealed_epoch = self.mount.admission_epoch.load(Ordering::Acquire);
        debug_assert_ne!(sealed_epoch & 1, 0);
        self.mount.admission_epoch.store(
            sealed_epoch.checked_add(1).expect("mount admission epoch"),
            Ordering::Release,
        );
    }

    fn commit_prepared_unmount(&self, sealed_epoch: u64) -> VfsResult<()> {
        let _tree = mount_tree_write();
        let _admission = self.mount.admission.upgradeable_read().upgrade();
        // `PreparedUnmount` sealed admission before flushing.  All mutable
        // parent-edge checks were completed under this same tree lock in
        // prepare.  A post-seal Location admission leaves a sticky witness so
        // a pathwalk can never wait for this tree writer while holding its
        // read guard; such a receipt must fail rather than detach a live view.
        if self.mount.admission_epoch.load(Ordering::Acquire) != sealed_epoch
            || sealed_epoch & 1 == 0
        {
            return Err(VfsError::Io);
        }
        if self.mount.admitted_during_unmount.load(Ordering::Acquire)
            || self.mountpoint().active_location_count() != 1
        {
            return Err(VfsError::ResourceBusy);
        }
        self.mountpoint()
            .detach_prevalidated_from_parent_locked()
            .expect("sealed normal-unmount parent edge");
        self.mountpoint().unmounting.store(false, Ordering::Release);
        self.mount.admission_epoch.store(
            sealed_epoch.checked_add(1).expect("mount admission epoch"),
            Ordering::Release,
...
mod tests {
    ...
    impl LookupTestDir {
        fn child(&self, name: &FsName, inode: u64) -> DirEntry {
            DirEntry::new_dir(
                {
    ...
    impl DirNodeOps for LookupTestDir {
        ...
        fn lookup(&self, name: &FsName) -> VfsResult<DirEntry> {
            match name.as_bytes() {
                b"child" => Ok(self.child(name, self.filesystem.root_inode + 1)),
                b"other" => Ok(self.child(name, self.filesystem.root_inode + 2)),
    ...
    fn in_mount_lookup_exposes_a_covered_dentry_without_crossing() {
        ...
        let covered_before_mount = parent
        ...
        let ordinary = parent.lookup_no_follow(FsName::new(b"child")).unwrap();
        let covered = parent
            .lookup_no_follow_in_mount(FsName::new(b"child"))
            .unwrap();
        ...
        let ordinary_other = parent.lookup_no_follow(FsName::new(b"other")).unwrap();
        let in_mount_other = parent
            .lookup_no_follow_in_mount(FsName::new(b"other"))
            .unwrap();
    ...
    #[test]
    fn sealed_unmount_admission_is_busy_not_a_tree_lock_deadlock() {
        let parent_filesystem = Filesystem::new(LookupTestFs::new(100));
        let parent_mount = Mountpoint::new_root(&parent_filesystem);
        let parent = parent_mount.root_location();
        let covered = parent
        ...
        // `prepare_unmount` is holding the sole Location lease across a
        // lock-free flush.  A pathname admission here used to spin while its
        // caller held MOUNT_TREE_LOCK.read(), preventing commit from taking
        // the writer lock forever.
        let receipt = child_mount.root_location().prepare_unmount().unwrap();
        let racing_location = child_mount.root_location();
        // The transient reference must still invalidate this receipt after
        // it is dropped; otherwise the active-count check alone would hide a
        // completed admission in the flush window.
        drop(racing_location);
        assert_eq!(
            receipt.flush().unwrap().commit(),
            Err(VfsError::ResourceBusy)
        );
        assert!(child_mount.is_attached());

        // The failed receipt's Drop unseals admission.  A later unmount has
        // no racing witness and may detach normally.
        child_mount
            .root_location()
            .prepare_unmount()
            .unwrap()
    ...
    #[test]
    fn concurrent_start_extension_loser_observes_completed_publication() {
        let extensions = StdArc::new(MountExtensions::new(None));
        let (winner_inside_tx, winner_inside_rx) = mpsc::channel();
        let (loser_admitted_tx, loser_admitted_rx) = mpsc::channel();
        let (release_winner_tx, release_winner_rx) = mpsc::channel();
        let winner = {
            let extensions = extensions.clone();
            thread::spawn(move || {
                extensions.initialize_with(|| {
                    winner_inside_tx.send(()).unwrap();
                    release_winner_rx.recv().unwrap();
                    marker_map(7)
                })
            })
        };
        winner_inside_rx.recv().unwrap();
        let loser = {
            let extensions = extensions.clone();
            thread::spawn(move || {
                // Start the competing initializer while the winner is held in
                // its `Once` closure. The observable contract is that this
                // call cannot return before the winner's map is published.
                loser_admitted_tx.send(()).unwrap();
                let result = extensions.initialize(marker_map(9));
                let observed = extensions.get_ref::<Marker>().map(|marker| marker.0);
                (result, observed)
            })
        };
        loser_admitted_rx.recv().unwrap();
        release_winner_tx.send(()).unwrap();

        assert_eq!(winner.join().unwrap(), Ok(()));
        let (loser_result, loser_observed) = loser.join().unwrap();
        assert_eq!(loser_result, Err(VfsError::AlreadyExists));
        let published = extensions
            .get_ref::<Marker>()
            .expect("winner must publish before either initializer returns")
            .0;
        assert_eq!(published, 7);
        assert_eq!(loser_observed, Some(published));
    }
