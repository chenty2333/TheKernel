...
/// Metadata which survives the raw socket queue and is attached to the
/// eventual device transmit token.  The OS uses this for a route-plan handle;
/// keeping it in the queue (rather than deriving it from packet bytes) makes
/// identical datagrams unambiguous.
pub type PacketMetadata = crate::storage::PacketMetadata<u32>;

/// A UDP packet ring buffer.
pub type PacketBuffer<'a> = crate::storage::PacketBuffer<'a, u32>;
...
pub struct Socket<'a> {
    ...
    tx_discard: Option<Arc<dyn Fn(u32) + Send + Sync>>,
    #[cfg(feature = "async")]
...
impl<'a> Socket<'a> {
    ...
    /// Install the OS-owned terminal callback for metadata discarded by raw
    /// parsing. The callback is deliberately outside packet-buffer storage:
    /// it cannot make queue teardown retain the socket itself.
    pub fn set_transmit_discard_handler(
        &mut self,
        handler: Option<Arc<dyn Fn(u32) + Send + Sync>>,
    ) {
        self.tx_discard = handler;
    }
        ...
        F: FnOnce(&mut [u8]) -> usize,
    {
        let size = self
            .tx_buffer
            .enqueue_with_infallible(max_size, 0, f)
            .map_err(|_| SendError::BufferFull)?;
    ...
    /// Like [`send_slice`], but attach an opaque OS-owned transmit handle to
    /// this *exact* queue entry.  Zero remains ordinary smoltcp traffic.
    pub fn send_slice_with_metadata(
        &mut self,
        data: &[u8],
        metadata: u32,
    ) -> Result<(), SendError> {
        self.tx_buffer
            .enqueue(data.len(), metadata)
            .map_err(|_| SendError::BufferFull)?
            .copy_from_slice(data);
        Ok(())
    }

    /// Drop every queued transmit entry while reporting its opaque metadata.
    /// This is used by the owning OS on final socket close so external leases
    /// attached to raw entries cannot outlive queue teardown.
    pub fn discard_queued_transmit_metadata(&mut self, mut discard: impl FnMut(u32)) {
        while let Ok((metadata, _)) = self.tx_buffer.dequeue() {
            if metadata != 0 {
                discard(metadata);
            }
        }
    ...
    pub fn recv(&mut self) -> Result<&[u8], RecvError> {
        let (_, packet_buf) = self.rx_buffer.dequeue().map_err(|_| RecvError::Exhausted)?;
    ...
    pub fn peek(&mut self) -> Result<&[u8], RecvError> {
        let (_, packet_buf) = self.rx_buffer.peek().map_err(|_| RecvError::Exhausted)?;
    ...
    pub(crate) fn process(&mut self, cx: &mut Context, ip_repr: &IpRepr, payload: &[u8]) {
        ...
        match self.rx_buffer.enqueue(total_len, 0) {
            Ok(buf) => {
    ...
    where
        F: FnOnce(&mut Context, (u32, IpRepr, &[u8])) -> Result<(), E>,
        D: FnOnce(u32),
    {
        let ip_protocol = self.ip_protocol;
        let ip_version = self.ip_version;
        let _checksum_caps = &cx.checksum_caps();
        let mut discard = Some(discard);
        let res = self.tx_buffer.dequeue_with(|&mut metadata, buffer| {
                ...
                Ok(IpVersion::Ipv4) => {
                    let mut packet = match Ipv4Packet::new_checked(buffer) {
                        ...
                        Err(_) => {
                            ...
                            if let Some(handler) = self.tx_discard.as_ref() {
                                handler(metadata);
                            }
                            if let Some(discard) = discard.take() {
                                discard(metadata);
                            }
                            return Ok(());
                        }
                    ...
                    emit(cx, (metadata, IpRepr::Ipv4(ipv4_repr), packet.payload()))
                }
                #[cfg(feature = "proto-ipv6")]
                Ok(IpVersion::Ipv6) => {
                    ...
                    net_trace!("raw:{:?}:{:?}: sending", ip_version, ip_protocol);
                    emit(cx, (metadata, IpRepr::Ipv6(ipv6_repr), packet.payload()))
                }
                Err(_) => {
...
mod test {
        ...
        ($module:ident, $socket:path, $hdr:path, $packet:path, $payload:path) => {
            mod $module {
                ...
                fn test_send_dispatch(#[case] medium: Medium) {
                    ...
                    assert_eq!(
                        socket.dispatch(&mut cx, |_, _| unreachable!(), |_| {}),
                        Ok::<_, ()>(())
                    );
    ...
    fn test_send_illegal(#[case] medium: Medium) {
        #[cfg(feature = "proto-ipv4")]
        {
            ...
            assert_eq!(
                socket.dispatch(cx, |_, _| unreachable!(), |_| {}),
                Ok::<_, ()>(())
            );
