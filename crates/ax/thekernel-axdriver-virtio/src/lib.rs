...
/// Inspect a PCI function's supported VirtIO class without changing hardware.
pub fn pci_device_type(dev_info: &DeviceFunctionInfo) -> Option<DeviceType> {
    virtio_drivers::transport::pci::virtio_device_type(dev_info).and_then(as_dev_type)
}

/// Construct the transport for a recognized VirtIO PCI function.
pub fn probe_pci_device<H: VirtIoHal>(
    ...
    // hold a shared INTx line asserted without an acknowledgment owner.
    #[cfg(feature = "irq")]
    if matches!(
        dev_type,
