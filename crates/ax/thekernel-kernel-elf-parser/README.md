...
let args: Vec<Vec<u8>> = vec![b"arg1".to_vec(), b"arg2".to_vec(), b"arg3".to_vec()];
let envs: Vec<Vec<u8>> = vec![b"LOG=file".to_vec()];
let auxv = [
...
let stack_data =
    kernel_elf_parser::app_stack_region(&args, &envs, &auxv, b"/bin/example", ustack_end);
