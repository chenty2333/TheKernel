...
use crate::{
    ...
    iomap::{Extent, ExtentMap, ExtentState, MappedRun, MappedRunKind},
    util::get_block_size,
};
...
fn retain_extent(
    ...
    retained: &mut Option<Vec<Extent>>,
    max_extents: usize,
) -> Ext4Result<()> {
...
impl<Hal: SystemHal> InodeRef<Hal> {
    ...
    /// Looks up one extent-tree run without touching the hot
    ...
    fn map_allocated_extent_run(
        &mut self,
        block: u32,
        max_blocks: u32,
    ) -> Ext4Result<Option<AllocatedExtentRun>> {
    ...
    /// Scans the existing ext4 block map for typed allocated extents.
    ///
    ...
    /// can distinguish a complete result.
    pub fn map_extents(
        &mut self,
        start: u64,
        length: u64,
        max_extents: usize,
    ) -> Ext4Result<ExtentMap> {
        ...
        if length == 0 || start >= file_size || end <= start {
            return Ok(ExtentMap::new(Vec::new(), 0, true));
        }
        ...
        let mut retained: Option<Vec<Extent>> = (max_extents != 0).then(Vec::new);
        ...
        loop {
            ...
            let (pblock, blocks, unwritten) = if self.uses_extents() {
                match self.map_allocated_extent_run(block, remaining)? {
            ...
            if pblock != 0 {
                ...
                let run_end = run_logical
                    .checked_add(
                        u64::from(blocks).checked_mul(block_size).ok_or_else(|| {
                ...
                if logical >= logical_end {
                    ...
                    block = block.checked_add(blocks).ok_or_else(|| {
                ...
                let physical = physical.checked_add(logical - run_logical).ok_or_else(|| {
                    Ext4Error::new(EINVAL as _, "fiemap physical offset overflow")
                })?;
                let bytes = logical_end - logical;

                let state = if unwritten {
                    ExtentState::Unwritten
                } else {
                    ExtentState::Written
                };
                let current = Extent::new(logical, physical, bytes, state);
                let can_extend = pending.as_ref().is_some_and(|previous: &Extent| {
                    previous.logical.checked_add(previous.length) == Some(current.logical)
                        && previous.physical.checked_add(previous.length) == Some(current.physical)
                        && previous.state == current.state
                });
                if can_extend {
                    if let Some(previous) = pending.as_mut() {
                        previous.length =
                            previous.length.checked_add(current.length).ok_or_else(|| {
                    ...
                    if let Some(previous) = pending.take() {
                        ...
                        retain_extent(previous, &mut total_extents, &mut retained, max_extents)?;
                    }
    ...
    // Retained for the planned single-extent physical-I/O fast path.
    #[allow(dead_code)]
    ...
    /// Reserves physical blocks without modifying any initialized bytes.
    ///
    /// `ext4_fs_get_inode_dblk_idx(..., true)` is the wrapper's native extent
    /// allocator: it inserts/splits the extent tree and journals bitmap and
    /// inode updates.  Do not express fallocate allocation as a zero write,
    /// since that would corrupt an already initialized range.
    pub fn allocate_range(&mut self, offset: u64, len: u64, keep_size: bool) -> Ext4Result<()> {
        let end = offset
            .checked_add(len)
            .ok_or_else(|| Ext4Error::new(EINVAL as _, "range overflow"))?;
        let block_size = u64::from(get_block_size(self.superblock()));
        ...
        if last > u64::from(u32::MAX) {
            return Err(Ext4Error::new(
                EINVAL as _,
                "extent range exceeds ext4 logical blocks",
            ));
        }
        self.invalidate_mapping_seq();
        for block in first..last {
            let _ = self.get_inode_fblock(block as u32)?;
        }
    ...
    /// Removes fully-covered extent blocks and returns them to ext4's block
    /// bitmap.  Boundary bytes remain allocated and are zeroed separately,
    /// matching PUNCH_HOLE rather than replacing it with an all-zero write.
    pub fn punch_hole(&mut self, offset: u64, len: u64) -> Ext4Result<()> {
        ...
        let after_last_full = end / block_size;

        // Zero partial edge blocks before releasing the fully-covered middle.
        let head_end = (first_full * block_size).min(end);
        if head_end > offset {
            self.write_at(
                &ZERO[..usize::try_from(head_end - offset)
                    .map_err(|_| Ext4Error::new(EINVAL as _, "range too large"))?],
                offset,
            )?;
        }
        let tail_start = (after_last_full * block_size).max(offset);
        if end > tail_start {
            self.write_at(
                &ZERO[..usize::try_from(end - tail_start)
                    .map_err(|_| Ext4Error::new(EINVAL as _, "range too large"))?],
                tail_start,
            )?;
        }
        if first_full < after_last_full {
            if after_last_full > u64::from(u32::MAX) {
                return Err(Ext4Error::new(
                    EINVAL as _,
                    "extent range exceeds ext4 logical blocks",
                ));
            ...
            unsafe {
                ext4_extent_remove_space(
                    self.inner.as_mut(),
                    first_full as u32,
                    (after_last_full - 1) as u32,
                )
                .context("ext4_extent_remove_space")?;
            }
            if ENABLE_EXTENT_STATUS_CACHE.load(core::sync::atomic::Ordering::Relaxed) {
                self.extent_status.invalidate_from(first_full as u32);
            }
            self.invalidate_mapping_seq();
        }
        Ok(())
    ...
    pub fn zero_range(&mut self, offset: u64, len: u64, keep_size: bool) -> Ext4Result<()> {
        static ZERO: [u8; 65_536] = [0; 65_536];
        let end = offset
            .checked_add(len)
            .ok_or_else(|| Ext4Error::new(EINVAL as _, "range overflow"))?;
        let size = self.size();
        self.allocate_range(offset, len, keep_size)?;
        let zero_end = if keep_size { end.min(size) } else { end };
        let mut cursor = offset;
        while cursor < zero_end {
            let bytes = usize::try_from((zero_end - cursor).min(ZERO.len() as u64))
                .map_err(|_| Ext4Error::new(EINVAL as _, "range too large"))?;
            self.write_at(&ZERO[..bytes], cursor)?;
            cursor = cursor
    ...
    pub fn collapse_range(&mut self, offset: u64, len: u64) -> Ext4Result<()> {
        let mut copy = Box::try_new([0u8; 65_536])
            .map_err(|_| Ext4Error::new(ENOMEM as _, "collapse bounce allocation failed"))?;
        let size = self.size();
        let end = offset
            .checked_add(len)
            .ok_or_else(|| Ext4Error::new(EINVAL as _, "range overflow"))?;
        if end > size {
            return Err(Ext4Error::new(EINVAL as _, "collapse beyond EOF"));
        }
        let mut source = end;
        let mut destination = offset;
        while source < size {
            let bytes = usize::try_from((size - source).min(65_536))
                .map_err(|_| Ext4Error::new(EINVAL as _, "range too large"))?;
            let buffer = &mut copy[..bytes];
            let read = self.read_at(buffer, source)?;
            if read == 0 {
                break;
            }
            self.write_at(&buffer[..read], destination)?;
            source += read as u64;
            destination += read as u64;
        }
        self.truncate(size - len)
    }

    pub fn insert_range(&mut self, offset: u64, len: u64) -> Ext4Result<()> {
        static ZERO: [u8; 65_536] = [0; 65_536];
        let mut copy = Box::try_new([0u8; 65_536])
            .map_err(|_| Ext4Error::new(ENOMEM as _, "insert bounce allocation failed"))?;
        let size = self.size();
        if offset >= size {
            return Err(Ext4Error::new(EINVAL as _, "insert at or beyond EOF"));
        }
        let new_size = size
            .checked_add(len)
            .ok_or_else(|| Ext4Error::new(EINVAL as _, "range overflow"))?;
        self.set_len(new_size)?;
        let mut source_end = size;
        while source_end > offset {
            let bytes = usize::try_from((source_end - offset).min(65_536))
                .map_err(|_| Ext4Error::new(EINVAL as _, "range too large"))?;
            let source = source_end - bytes as u64;
            let buffer = &mut copy[..bytes];
            let read = self.read_at(buffer, source)?;
            if read != bytes {
                return Err(Ext4Error::new(EIO as _, "short ext4 range read"));
            }
            self.write_at(&buffer[..read], source + len)?;
            source_end = source;
        }
        let mut cursor = offset;
        let end = offset + len;
        while cursor < end {
            let bytes = usize::try_from((end - cursor).min(ZERO.len() as u64))
                .map_err(|_| Ext4Error::new(EINVAL as _, "range too large"))?;
            self.write_at(&ZERO[..bytes], cursor)?;
            cursor += bytes as u64;
        }
