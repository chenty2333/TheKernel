...
fn add_include(filename: &str) {
    // Headers generated by this build script (e.g. the freestanding headers under
    // OUT_DIR/llvm-{arch}-toolchain) are rewritten on every run; watching them
    // would make cargo consider the build script dirty forever.
    let generated = env::var("OUT_DIR")
        .map(|out_dir| Path::new(filename).starts_with(out_dir))
        .unwrap_or(false);
    if !generated && !Path::new(filename).ends_with("ext4_config.h") {
