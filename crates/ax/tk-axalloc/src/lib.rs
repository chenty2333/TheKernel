...
pub enum UsageKind {
    ...
    Global,
    /// Physically fixed pages retained by a loaded kexec image.
    Kexec,
}
...
/// Atomically reserves exactly the requested page range from the global page
/// allocator.  The range remains unavailable until it is released with
/// [`GlobalAllocator::dealloc_pages`].
#[cfg(not(feature = "hv"))]
pub fn replace_pages_at(
    start: usize,
    num_pages: usize,
    align_pow2: usize,
) -> axallocator::AllocResult<usize> {
    global_allocator().replace_pages_at(start, num_pages, align_pow2)
}
