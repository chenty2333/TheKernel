...
#[cfg(feature = "shared-block")]
pub use self::block_volume::{
    BlockFaultLifetime, BlockFaultOperation, BlockFaultRule, BlockVolume,
    BlockVolumeCompletionOwner, BlockVolumeDevice, MemberMap,
};
...
impl AllDevices {
    ...
    /// Inserts the bootloader rootfs before ordinary device discovery so it
    /// remains block device zero even when PCI exposes a VirtIO disk.
    #[cfg(all(not(feature = "dyn"), feature = "rootfs-module", feature = "block"))]
    fn probe_rootfs_module(&mut self) {
        let Some((start, end)) = axhal::boot::rootfs_module() else {
            return;
        };
        let Some(length) = end.checked_sub(start) else {
            warn!("ignored malformed rootfs module range [{start:#x}, {end:#x})");
            return;
        };
        if length == 0 {
            warn!("ignored empty rootfs module");
            return;
        }
        let virtual_start = axhal::mem::phys_to_virt(start.into()).as_usize();
        // The platform reserves validated module memory before the allocator
        // starts.  `boot_module` only exposes it immutably and retains no
        // reference to the Multiboot information structure.
        let bytes = unsafe { core::slice::from_raw_parts(virtual_start as *const u8, length) };
        match StaticBlockDevice::boot_module(bytes) {
            Ok(device) => {
                info!("registered immutable Multiboot rootfs module");
                self.add_device(AxDeviceEnum::Block(device));
            }
            Err(error) => warn!("ignored invalid rootfs module: {error:?}"),
        }
    }
...
pub fn init_drivers() -> AllDevices {
    debug!("Initialize device drivers...");
