...
impl<const PAGE_SIZE: usize> BaseAllocator for BitmapPageAllocator<PAGE_SIZE> {
    fn init(&mut self, start: usize, size: usize) {
        ...
        self.managed.insert(start_idx..end_idx);
    }

    fn add_memory(&mut self, start: usize, size: usize) -> AllocResult {
        let end = start.checked_add(size).ok_or(AllocError::InvalidParam)?;
        let start = crate::align_up(start, PAGE_SIZE);
        let end = crate::align_down(end, PAGE_SIZE);
        if end <= start || start < self.base {
            return Err(AllocError::InvalidParam);
        }
        let start_idx = (start - self.base) / PAGE_SIZE;
        let end_idx = (end - self.base) / PAGE_SIZE;
        if end_idx > BitAllocUsed::CAP {
            return Err(AllocError::NoMemory);
        }
        if (start_idx..end_idx).any(|index| self.managed.test(index)) {
...
impl<const PAGE_SIZE: usize> PageAllocator for BitmapPageAllocator<PAGE_SIZE> {
    ...
    fn alloc_pages_at(
        ...
        if idx
            ...
            .is_none_or(|end| end > BitAllocUsed::CAP)
        {
            return Err(AllocError::NoMemory);
        }
...
mod tests {
    ...
    #[test]
    fn add_memory_keeps_disjoint_regions_and_fixed_reservations_atomic() {
        let mut allocator = BitmapPageAllocator::<PAGE_SIZE>::new();
        allocator.init(0x20_0000, 2 * PAGE_SIZE);
        allocator.add_memory(0x80_0000, 3 * PAGE_SIZE).unwrap();

        assert_eq!(allocator.total_pages(), 5);
        assert_eq!(
            allocator.alloc_pages_at(0x80_0000, 2, PAGE_SIZE).unwrap(),
            0x80_0000
        );
        assert!(matches!(
            allocator.alloc_pages_at(0x80_0000, 2, PAGE_SIZE),
            Err(AllocError::NoMemory)
        ));
        assert_eq!(allocator.available_pages(), 3);
        assert!(matches!(
            allocator.add_memory(0x80_0000, PAGE_SIZE),
