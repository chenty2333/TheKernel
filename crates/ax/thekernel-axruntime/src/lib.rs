...
/// Allocation-free late-panic handoff installed by the monolithic kernel.
///
/// The runtime cannot depend on the kernel crate, so crash-kexec registers a
/// plain function pointer after its image state has been initialized.  A
/// zero pointer means that normal platform shutdown remains the panic path.
static PANIC_CRASH_HOOK: AtomicUsize = AtomicUsize::new(0);

/// Installs the terminal crash hook used by the panic handler.
///
/// Registration is replaceable so a later kernel layer can atomically retire
/// an earlier handler without retaining compatibility shims in the runtime.
pub fn register_panic_crash_hook(hook: fn()) {
    PANIC_CRASH_HOOK.store(hook as usize, Ordering::Release);
}

pub(crate) fn invoke_panic_crash_hook() {
    let hook = PANIC_CRASH_HOOK.load(Ordering::Acquire);
    if hook != 0 {
        // SAFETY: the only writer accepts a Rust `fn()` and publishes it with
        // release ordering.  Kernel text remains resident for the runtime's
        // entire lifetime.
...
/// The input hotplug service is deliberately a single, low-frequency worker.
...
/// The timer context observer records the origin CPL before the generic IRQ
/// path constructs its no-preempt guard. `0` means ring 0 and `1` ring 3;
/// the ordinary timer callback consumes this CPU-local value for that same
/// non-nested timer delivery.
#[cfg(feature = "irq")]
#[percpu::def_percpu]
static TIMER_INTERRUPTED_USER: u8 = 0;
...
fn call_function_ipi_handler() {
    ...
    assert_eq!(
        unknown, 0,
        "unknown bounded call-function work: {unknown:#x}"
    );
}
...
pub fn rust_main(cpu_id: usize, arg: usize) -> ! {
    ...
    // Spawning allocates task state and may trigger cross-CPU TLB work.  All
...
fn init_interrupt() {
    ...
    assert!(
        axhal::irq::register_with_context(
            axhal::time::irq_num(),
            timer_irq_context,
            timer_irq_handler,
        ),
...
/// CPU-local byte: trap observers run before the generic IRQ handler has
/// established its no-preempt guard and must never call scheduler code.
#[cfg(feature = "irq")]
fn timer_irq_context(_vector: usize, frame: &axhal::context::TrapFrame) {
    // SAFETY: this timer's trap-entry observer and ordinary handler execute
    // serially on the same CPU with interrupts masked by the architecture.
    unsafe { TIMER_INTERRUPTED_USER.write_current_raw((frame.cs & 3 == 3) as u8) };
}

/// Ordinary platform timer callback. It retains the established IRQ guard,
/// dispatch and EOI ordering while consuming the precise origin CPL recorded
/// by `timer_irq_context`.
#[cfg(feature = "irq")]
fn timer_irq_handler() {
    ...
    #[cfg(feature = "multitask")]
    if periodic_due {
        // x86 RPL was captured from this IRQ's trap frame before dispatch.
        let interrupted_user = unsafe { TIMER_INTERRUPTED_USER.read_current_raw() } != 0;
        axtask::on_timer_tick(interrupted_user);
    } else if early_due {
        axtask::on_timer_event();
    }
