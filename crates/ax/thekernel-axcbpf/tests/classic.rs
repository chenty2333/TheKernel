...
#[test]
fn high_offsets_are_owned_by_the_input_adapter() {
    struct InputAdapter;

    impl Input for InputAdapter {
        fn len(&self) -> u32 {
            0
        }

        fn load(&self, offset: u32, width: LoadWidth) -> Option<u32> {
            (offset == 0xffff_f002 && width == LoadWidth::Word).then_some(0x1234_5678)
        }
    }
    ...
    assert_eq!(program.evaluate(&InputAdapter), 0x1234_5678);
}
