...
`tk-axcbpf` is a `no_std`, unsafe-free classic-BPF verifier and
...
Profiles own all domain-specific offset interpretation, including any Linux
socket-filter extensions; the mechanism forwards each `u32` offset to its
...
`CodeImage::bytes` into a W^X executable mapping; `entry_offset()` is zero and
`page_aligned_size()` gives the exact page-rounded mapping size.

Use `Program::translate_with_profile(InputProfile::NativeAlignedWords)` for a
native-endian aligned-word input. That profile rejects byte and
...
`CodeImage::validate()` independently decodes the restricted native instruction
