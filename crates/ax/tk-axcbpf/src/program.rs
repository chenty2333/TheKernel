...
/// Slice adapters and native images share a u32 input-length ABI. A larger
/// backing slice must not expose bytes beyond its advertised saturated length.
pub(crate) fn input_range(length: usize, offset: u32, width: LoadWidth) -> Option<Range<usize>> {
    let end = offset.checked_add(width.bytes() as u32)?;
    if end > u32::try_from(length).unwrap_or(u32::MAX) {
        return None;
    }
...
impl Input for [u8] {
    ...
    fn load(&self, offset: u32, width: LoadWidth) -> Option<u32> {
        let bytes = self.get(input_range(<[u8]>::len(self), offset, width)?)?;
        match width {
...
impl Program {
    ...
    pub fn translate(&self) -> Result<crate::CodeImage, crate::TranslationError> {
        self.translate_with_profile(crate::InputProfile::BigEndianBytes)
    }
...
mod tests {
    ...
    #[test]
    fn slice_load_ranges_never_exceed_the_advertised_u32_length() {
        let limit = u32::MAX as usize;
        for length in [limit, usize::MAX] {
            assert_eq!(
                input_range(length, u32::MAX - 1, LoadWidth::Byte),
                Some(limit - 1..limit)
            );
            assert_eq!(
                input_range(length, u32::MAX - 4, LoadWidth::Word),
                Some(limit - 4..limit)
            );
            assert_eq!(input_range(length, u32::MAX, LoadWidth::Byte), None);
            assert_eq!(input_range(length, u32::MAX - 1, LoadWidth::Half), None);
            assert_eq!(input_range(length, u32::MAX - 3, LoadWidth::Word), None);
        }
        assert_eq!(input_range(3, 0, LoadWidth::Word), None);
        assert_eq!(input_range(4, 0, LoadWidth::Word), Some(0..4));
    }
