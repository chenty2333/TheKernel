...
use crate::{Input, Instruction, LoadWidth, SCRATCH_WORDS, VerifyError, opcode};
...
pub enum InputProfile {
    /// Byte, halfword, and word loads in big-endian order.
    BigEndianBytes,
    /// Aligned native-endian word loads.
    ///
    ...
    /// An adapter may additionally admit only a four-byte-aligned snapshot
...
impl<'a> NativeWordInput<'a> {
    ...
    /// offset check; adapters whose snapshot contract requires four-byte base
    /// alignment can enforce that separately with [`Self::new_aligned`].
...
impl Input for NativeWordInput<'_> {
    ...
    fn load(&self, offset: u32, width: LoadWidth) -> Option<u32> {
        ...
        let bytes = self.bytes.get(crate::program::input_range(
            self.bytes.len(),
            offset,
            width,
        )?)?;
...
impl Assembler {
    ...
    fn emit_prologue(&mut self, profile: InputProfile) {
        ...
        match profile {
            InputProfile::BigEndianBytes | InputProfile::NativeAlignedWords => {
                self.bytes.extend_from_slice(&[
    ...
    fn emit_load_r14(&mut self, width: LoadWidth, profile: InputProfile) {
        match width {
            ...
            LoadWidth::Half => {
                ...
                if matches!(profile, InputProfile::BigEndianBytes) {
