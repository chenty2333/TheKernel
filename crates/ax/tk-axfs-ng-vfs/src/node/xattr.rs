...
pub enum XattrSetMode {
    ...
    Replace,
    /// Preserve both `XATTR_CREATE` and `XATTR_REPLACE` for the provider's
    /// atomic existence decision. An existing attribute fails the create
    /// predicate; a missing attribute fails the replace predicate.
    CreateAndReplace,
}
