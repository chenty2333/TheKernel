...
pub trait FilesystemOps: Send + Sync {
    ...
    fn export_handle_is_descendant(
        ...
        // Callers that need this relation for mount admission must reject an
        // unknown answer.  Returning `false` here would mistake an
        // unsupported backend proof for evidence that staging is disjoint.
        Err(crate::VfsError::OperationNotSupported)
    }
    ...
    /// Flushes immediately before a normal mount detach.  Backends whose
    /// clean-shutdown protocol differs from ordinary sync (for example, a
    /// journalled filesystem writing a terminal unmount record) override
    /// this fallible hook.  Keeping it separate prevents `fsync`/`syncfs`
    /// from falsely advertising that an actively writable mount was closed.
...
impl Filesystem {
    /// Tests whether two handles retain the exact same live filesystem
    /// instance.  A device number is not sufficient here: loopback, remote,
    /// and synthetic filesystems may legitimately reuse it.
    ...
    pub(crate) fn flush_for_unmount(&self) -> VfsResult<()> {
        self.inner.ops.flush_for_unmount()
    }
