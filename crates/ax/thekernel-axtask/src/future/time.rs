...
impl TimerRuntime {
    ...
    fn wake_expired(&self) -> usize {
        ...
        // Wake in place: IntoIter owns another capacity-sized array which
        // remains on the IRQ stack while each scheduler wake executes.
        for waker in pending.iter_mut().filter_map(Option::take) {
            waker.wake();
        }
        count
    }
