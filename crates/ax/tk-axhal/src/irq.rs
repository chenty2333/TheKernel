...
pub fn register_shared_dispatcher(dispatcher: fn(usize) -> bool) -> bool {
    ...
    {
        axplat_x86_pc::register_shared_dispatcher(dispatcher)
    }
...
/// Set an existing PCI INTx route to level-triggered, active-low delivery.
/// Returns false when the platform or IOAPIC route is unavailable.
#[must_use]
pub fn configure_pci_intx(vector: usize) -> bool {
    #[cfg(all(target_os = "none", feature = "defplat", not(feature = "myplat")))]
    {
        axplat_x86_pc::configure_pci_intx(vector)
    }
    #[cfg(not(all(target_os = "none", feature = "defplat", not(feature = "myplat"))))]
    {
        let _ = vector;
...
pub enum IpiReason {
    ...
    DeferredWork    = 5,
    /// Refresh the HWP request from the destination CPU's published
    /// run-queue utilization clamp.
    HwpClampRefresh = 6,
    /// Stop and settle a perf placement before the last descriptor reference
    /// is released.  The payload is owned by the perf subsystem's per-CPU
    /// mailbox; this broker lane only supplies the allocation-free kick.
    PerfReconcile   = 7,
}
...
pub fn register_context(vector: usize, handler: fn(usize, &TrapFrame)) -> bool {
    register_with_context(vector, handler, context_marker)
}

/// Registers one trap-frame observer together with the ordinary platform IRQ
/// callback. The observer executes at raw trap entry and therefore must only
/// perform bounded, non-blocking state capture; scheduling and device work
/// remain in `handler`, after the platform IRQ path has established its
/// normal guard/EOI discipline.
#[must_use]
pub fn register_with_context(
    vector: usize,
    observer: fn(usize, &TrapFrame),
    ...
    if slot
        ...
        .is_err()
    {
        return slot.load(Ordering::Acquire) == observer as usize;
    }
    if !axplat::irq::register(vector, handler) {
        slot.store(0, Ordering::Release);
        return false;
    }
    slot.store(observer as usize, Ordering::Release);
    true
}
...
mod tests {
    ...
    #[test]
    fn pci_irq_operations_are_unavailable_on_host() {
        assert!(!super::configure_pci_intx(0x2b));
        assert!(!super::register_shared_dispatcher(|_| true));
    }
