...
#[cfg(test)]
use crate::cfs::RR_TIMESLICE_TICKS;

const NICE_RANGE_POS: i8 = 19;
const NICE_MIN: i8 = -20;
const NICE_MAX: i8 = 19;
/// A fair-class service interval requested by a task.
///
/// `None` leaves the choice to the active scheduler profile. A concrete value
/// is passed through unchanged; granularity policy belongs to the scheduler,
/// not to an operating-system ABI.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct RequestedSlice(Option<u64>);
...
impl RequestedSlice {
    ...
    pub const fn from_nanos(nanoseconds: u64) -> Option<Self> {
        if nanoseconds == 0 {
            None
        } else {
            Some(Self(Some(nanoseconds)))
        }
    }
...
pub enum EevdfTaskClass {
    ...
    /// layer.
    Fifo = 4,
    /// CBS reservation scheduled by earliest absolute deadline.
    Deadline = 5,
}

/// One deadline-server reservation, expressed in nanoseconds.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct DeadlineParameters {
    pub runtime_ns: u64,
    pub deadline_ns: u64,
    pub period_ns: u64,
}
...
///
/// Reservations use an optimistic CAS so admission never needs to take one
/// CPU's runqueue lock while holding another.  Callers first update this
/// domain ledger, then their local runqueue; failed local publication refunds
/// the exact delta before releasing the runqueue lock.
#[derive(Debug)]
pub struct DeadlineAdmissionDomain {
    utilization: AtomicU64,
    capacity: u64,
}

impl DeadlineAdmissionDomain {
    pub const fn new() -> Self {
        Self {
            utilization: AtomicU64::new(0),
            capacity: DEADLINE_UTIL_SCALE as u64,
        }
    }

    /// Construct a root domain spanning `cpus` participating CPUs.
    pub fn with_cpu_capacity(cpus: usize) -> Self {
        Self {
            utilization: AtomicU64::new(0),
            capacity: (DEADLINE_UTIL_SCALE as u64).saturating_mul(cpus.max(1) as u64),
        }
    }

    fn replace(&self, old: u128, new: u128) -> Result<(), SchedulerError> {
        let old = u64::try_from(old).map_err(|_| SchedulerError::ArithmeticExhausted)?;
        let new = u64::try_from(new).map_err(|_| SchedulerError::ArithmeticExhausted)?;
        let mut current = self.utilization.load(Ordering::Acquire);
        loop {
            let total = current
                .checked_sub(old)
            ...
            match self.utilization.compare_exchange_weak(
                current,
                total,
                Ordering::AcqRel,
                Ordering::Acquire,
...
impl Default for DeadlineAdmissionDomain {
    fn default() -> Self {
        Self::new()
    }
}

impl DeadlineParameters {
    pub const fn is_valid(self) -> bool {
        self.runtime_ns != 0
            && self.runtime_ns <= self.deadline_ns
...
impl EevdfTaskParams {
    fn validated(mut self) -> Option<Self> {
        match self.class {
            ...
            EevdfTaskClass::RoundRobin | EevdfTaskClass::Fifo | EevdfTaskClass::Deadline => {
                self.nice = 0;
            }
        }
        let valid = match self.class {
            ...
            EevdfTaskClass::Deadline => self.rt_priority == 0,
...
struct MigrationState {
    ...
    /// prevents a parked migration from observing an allocator-reused domain
    /// address after its source queue has been dropped.
    deadline_domain: Option<Arc<DeadlineAdmissionDomain>>,
...
pub(crate) struct EevdfOwnedState {
    ...
    /// CBS state, owned by the run queue.  `absolute_deadline_ns` and
    /// `replenish_at_ns` are in the monotonic domain supplied by axtask.
...
impl<T> EevdfTaskPayload<T> {
    pub(crate) const fn new(inner: T) -> Self {
        Self {
            ...
            fair_runtime_ns: AtomicU64::new(0),
    ...
    fn fair_runtime_ns(&self) -> u64 {
        self.fair_runtime_ns.load(Ordering::Acquire)
    }

    fn set_fair_runtime_ns(&self, runtime_ns: u64) {
        self.fair_runtime_ns.store(runtime_ns, Ordering::Release);
    }

    fn deadline_config(&self) -> DeadlineParameters {
        DeadlineParameters {
    ...
    fn publish_deadline_config(&self, value: DeadlineParameters) {
        self.deadline_runtime_ns
            .store(value.runtime_ns, Ordering::Release);
...
impl<T> EevdfNode<EevdfReadyKey, EevdfTaskPayload<T>> {
    ...
    /// Configured requested slice in nanoseconds, or zero for the profile
    /// default. Published tasks are changed through the scheduler transaction.
    pub fn fair_runtime_ns(&self) -> u64 {
        self.value().fair_runtime_ns()
    }

    pub fn deadline_config(&self) -> DeadlineParameters {
        self.value().deadline_config()
    }

    pub fn configure_deadline(&self, value: DeadlineParameters) -> Result<(), SchedulerError> {
        if !value.is_valid() {
            return Err(SchedulerError::InvalidParameters);
        }
        let claim = self.value().claim_configuration()?;
        self.value().publish_deadline_config(value);
        claim.finish()
    }
    ...
    /// Configure a virgin task's requested fair slice.
    pub fn configure_fair_runtime_ns(&self, runtime_ns: u64) -> Result<(), SchedulerError> {
        let claim = self.value().claim_configuration()?;
        if runtime_ns != 0
            && !matches!(
                self.sched_params().class,
                EevdfTaskClass::Normal | EevdfTaskClass::Batch
            )
        {
            drop(claim);
        ...
        self.value().set_fair_runtime_ns(runtime_ns);
        claim.finish()
    }
...
const DEADLINE_CLASS_RANK: u8 = 0;
const RT_CLASS_RANK: u8 = 1;
const FAIR_CLASS_RANK: u8 = 2;
const DEADLINE_UTIL_SCALE: u128 = 1_000_000_000;
...
fn is_deadline_class(class: EevdfTaskClass) -> bool {
    matches!(class, EevdfTaskClass::Deadline)
}
...
fn fair_class(class: EevdfTaskClass) -> Option<crate::eevdf_model::RequestClass> {
    match class {
        ...
        EevdfTaskClass::RoundRobin | EevdfTaskClass::Fifo | EevdfTaskClass::Deadline => None,
    }
}
...
pub struct EEVDFScheduler<T> {
    ...
    deadline_throttled: EevdfTree<EevdfReadyKey, EevdfTaskPayload<T>>,
...
impl<T> EEVDFScheduler<T> {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            deadline_throttled: EevdfTree::new(),
            running: None,
            clock: Clock::new(0),
            id: UNOWNED,
            fair_sequence: 0,
    ...
    /// Bind an empty runqueue to the shared ledger for its CPU-affinity
    /// domain. Live reservations move between domains only through migration.
    pub fn set_deadline_admission_domain(
        &mut self,
        domain: Arc<DeadlineAdmissionDomain>,
    ) -> Result<(), SchedulerError> {
        // A domain is part of the identity retained by a parked migration.
        // Rebinding an otherwise empty source/destination queue could make a
        // later commit either leak that retained reservation or observe it in
        // the wrong ledger. Run queues are bound once at construction.
        if self.deadline_utilization != 0 || self.deadline_domain.is_some() {
            return Err(SchedulerError::TaskBusy);
        }
        self.deadline_domain = Some(domain);
        Ok(())
    }

    /// Rebase CBS onto the task layer's global monotonic nanosecond clock.
    /// This is intentionally an absolute timestamp, never a per-runqueue
    /// elapsed delta: a migrated server must retain the same replenishment
    /// instant on every CPU.
    pub fn advance_deadline_clock(&mut self, now_ns: u64) {
        self.rebase_deadline_clock(now_ns);
    }

    fn deadline_utilization(config: DeadlineParameters) -> Result<u128, SchedulerError> {
        if !config.is_valid() {
            return Err(SchedulerError::InvalidParameters);
        }
        u128::from(config.runtime_ns)
            .checked_mul(DEADLINE_UTIL_SCALE)
            .and_then(|value| value.checked_add(u128::from(config.period_ns - 1)))
            .map(|value| value / u128::from(config.period_ns))
            .ok_or(SchedulerError::ArithmeticExhausted)
    }

    fn admit_deadline(&mut self, config: DeadlineParameters) -> Result<(), SchedulerError> {
        self.admit_deadline_local(config)?;
        if let Some(domain) = self.deadline_domain.as_ref() {
            if let Err(error) = domain.replace(0, Self::deadline_utilization(config)?) {
                self.release_deadline_local(config);
                return Err(error);
            }
        }
        Ok(())
    ...
    fn admit_deadline_local(&mut self, config: DeadlineParameters) -> Result<(), SchedulerError> {
        let util = Self::deadline_utilization(config)?;
        let total = self
            .deadline_utilization
            .checked_add(util)
            .ok_or(SchedulerError::ArithmeticExhausted)?;
        if total > DEADLINE_UTIL_SCALE {
            return Err(SchedulerError::TaskBusy);
        }
        self.deadline_utilization = total;
        Ok(())
    }

    fn release_deadline(&mut self, config: DeadlineParameters) {
        let util = Self::deadline_utilization(config).unwrap_or(0);
        if let Some(domain) = self.deadline_domain.as_ref() {
            domain
                .replace(util, 0)
                .expect("deadline domain ledger lost an admitted reservation");
        }
        self.release_deadline_local(config);
    }

    fn release_deadline_local(&mut self, config: DeadlineParameters) {
        let util = Self::deadline_utilization(config).unwrap_or(0);
        self.deadline_utilization = self.deadline_utilization.saturating_sub(util);
    }

    fn replace_deadline_domain(
        &self,
        old: Option<DeadlineParameters>,
        new: Option<DeadlineParameters>,
    ) -> Result<(), SchedulerError> {
        if let Some(domain) = self.deadline_domain.as_ref() {
            let old = old
                .map(Self::deadline_utilization)
                .transpose()?
                .unwrap_or(0);
            let new = new
                .map(Self::deadline_utilization)
                .transpose()?
                .unwrap_or(0);
            domain.replace(old, new)?;
        }
        Ok(())
    }

    /// Calculate a reservation replacement without publishing it.  The
    /// caller performs all potentially fallible tree work first, then commits
    /// this value together with the new task representation.
    fn replaced_deadline_utilization(
        &self,
        old: Option<DeadlineParameters>,
        new: Option<DeadlineParameters>,
    ...
    /// Rebase CBS time from the globally monotonic task-layer clock before a
    /// migration destination admits a reservation.
    pub fn rebase_deadline_clock(&mut self, now_ns: u64) {
        if now_ns > self.deadline_now_ns {
            self.deadline_now_ns = now_ns;
            self.replenish_deadlines();
        }
    }
    ...
    /// Returns a task's scheduling class and current scheduler-owned interval.
    ///
    /// The caller must hold this scheduler's run-queue lock. A sleeping fair
    /// task retains its request in `dormant_fair`, so its interval remains
    /// observable even though it no longer contributes to `total_weight`.
    pub fn task_timeslice(
        &self,
        task: &Arc<EEVDFTask<T>>,
        allow_unowned_sleeping_fair: bool,
    ) -> Result<(EevdfTaskClass, usize), SchedulerError> {
        if self.id == UNOWNED {
            return Err(SchedulerError::TaskBusy);
        }
        let class = task.sched_params().class;
        let owner = task.owner();
        let sleeping_fair = allow_unowned_sleeping_fair
            && owner == UNOWNED
            && matches!(class, EevdfTaskClass::Normal | EevdfTaskClass::Batch)
            // SAFETY: the caller holds this run queue's lock. A blocked fair
            // task remains associated with this queue until wakeup, while a
            // migration retains its explicit marker below.
            && unsafe {
                let state = task.owned_state();
                state.migration.is_none()
                    && state.entity.is_some_and(|entity| entity.request.q != 0)
            };
        if owner != self.id && !sleeping_fair {
            return Err(SchedulerError::TaskBusy);
        }
        let interval_ticks = match class {
            EevdfTaskClass::RoundRobin => rr_timeslice_ticks(),
            EevdfTaskClass::Fifo | EevdfTaskClass::Idle | EevdfTaskClass::Deadline => 0,
            EevdfTaskClass::Normal | EevdfTaskClass::Batch => {
                if self.clock.total_weight == 0 {
                    return Ok((class, 0));
                }
                // SAFETY: the task layer calls this while holding this scheduler's
                // run-queue lock, which serializes all entity-state access.
                unsafe {
                    task.owned_state()
                        .entity
                        .or(task.owned_state().dormant_fair)
                        .map_or(0, |entity| {
                            entity.request.q.min(usize::MAX as u128) as usize
                        })
                }
            }
        };
        Ok((class, interval_ticks))
    }

    /// Whether this queue has a current task whose wall-clock service must be
    /// accounted before advancing the shared CBS clock.
    pub fn has_running_task(&self) -> bool {
        self.running.is_some()
    }

    /// Clones the task currently owning this scheduler's execution slot.
    ///
    /// The caller must hold the run-queue lock.  This is used by task-layer
    /// lifecycle boundaries which need to charge one sampled wall-clock delta
    /// before mutating the CBS clock for an otherwise unrelated operation.
    pub fn running_task(&self) -> Option<Arc<EEVDFTask<T>>> {
        self.running.as_ref().cloned()
    }
    ...
    /// Converts a requested slice to scheduler periods, preserving all
    /// nonzero values without ABI-specific clamping.
    fn fair_runtime_quantum(runtime_ns: u64, period_ns: u64) -> Result<u128, SchedulerError> {
        if period_ns == 0 {
            return Err(SchedulerError::InvalidParameters);
        }
        Ok(u128::from(runtime_ns.saturating_add(period_ns - 1) / period_ns).max(1))
    }

    fn rebind_fair_runtime(
        entity: &mut Entity,
        runtime_ns: u64,
        total_weight: u128,
        period_ns: u64,
        ...
        if runtime_ns == 0 {
            entity
                .rebind_default_request_quantum(total_weight)
                .map_err(model_error)?;
        } else {
            entity
                .rebind_request_quantum(Self::fair_runtime_quantum(runtime_ns, period_ns)?)
                .map_err(model_error)?;
        }
        Ok(())
    }
    ...
    fn deadline_key(absolute_ns: u64, sequence: i128) -> EevdfReadyKey {
        EevdfReadyKey::new(DEADLINE_CLASS_RANK, u128::from(absolute_ns), sequence)
    }

    fn throttled_deadline_key(replenish_ns: u64, sequence: i128) -> EevdfReadyKey {
        EevdfReadyKey::new(DEADLINE_CLASS_RANK, u128::from(replenish_ns), sequence)
    }

    fn initialize_deadline_state(&self, task: &Arc<EEVDFTask<T>>) -> Result<(), SchedulerError> {
        let config = task.deadline_config();
        if !config.is_valid() {
            return Err(SchedulerError::InvalidParameters);
        }
        let absolute = self
        ...
        unsafe {
            ...
            state.deadline_replenish_at_ns = self
                .deadline_now_ns
    ...
    fn replenish_deadlines(&mut self) {
        while let Some(front) = self.deadline_throttled.front() {
            if unsafe { front.eligible_at() } > u128::from(self.deadline_now_ns) {
                break;
            }
            let task = self
                .deadline_throttled
                .remove(unsafe { &*(front as *const _) })
                .expect("deadline throttle queue lost its front server");
            let config = task.deadline_config();
            assert!(
                config.is_valid(),
            ...
            self.ready_tree
                .insert(task)
                .expect("deadline replenish key collision");
        }
    }
    ...
    fn enqueue_new(&mut self, task: Arc<EEVDFTask<T>>) -> Result<(), SchedulerError> {
        ...
        let (next_clock, entity, key, eligible_at) = if is_rt_class(params.class) {
            if is_deadline_class(params.class) {
                if let Err(error) = self.admit_deadline(task.deadline_config()) {
                    self.release_claim(&task);
                    return Err(error);
                }
                if let Err(error) = self.initialize_deadline_state(&task) {
                    self.release_deadline(task.deadline_config());
                    self.release_claim(&task);
                    return Err(error);
                }
            }
            let sequence = match params.class {
                EevdfTaskClass::Fifo | EevdfTaskClass::RoundRobin | EevdfTaskClass::Deadline => {
            ...
            let key = if is_deadline_class(params.class) {
                Self::deadline_key(unsafe { task.owned_state().deadline_absolute_ns }, sequence)
            } else {
                Self::rt_key(params, sequence)
            };
            (self.clock, None, key, 0)
        } else {
            ...
            let mut entity = match Entity::with_lag(
                class,
                weight,
            ...
            if let Err(error) = Self::rebind_fair_runtime(
                &mut entity,
                task.fair_runtime_ns(),
                next_clock.total_weight,
                10_000_000,
            ) {
                self.release_claim(&task);
    ...
    pub fn commit_reserved_task(
        ...
        let (next_clock, entity, key, eligible_at) = if is_rt_class(params.class) {
            let key = if is_deadline_class(params.class) {
                ...
                if !config.is_valid() {
                    return Err(EevdfReservationCommitError {
                        kind: SchedulerError::InvalidParameters,
                        reservation,
                    });
                }
                Self::deadline_key(
                    match self.deadline_now_ns.checked_add(config.deadline_ns) {
                        Some(value) => value,
                        None => {
                            return Err(EevdfReservationCommitError {
                                kind: SchedulerError::ArithmeticExhausted,
                                reservation,
                            });
                        }
                    },
            ...
            if let Err(kind) = Self::rebind_fair_runtime(
                &mut entity,
                task.fair_runtime_ns(),
                next_clock.total_weight,
                10_000_000,
        ...
        let old_state = unsafe { task.owned_state().clone() };
        let deadline_admitted = is_deadline_class(params.class);
        if deadline_admitted {
            if let Err(kind) = self.admit_deadline(task.deadline_config()) {
                return Err(EevdfReservationCommitError { kind, reservation });
            }
            if let Err(kind) = self.initialize_deadline_state(&task) {
                self.release_deadline(task.deadline_config());
                return Err(EevdfReservationCommitError { kind, reservation });
            }
        }
        ...
        if let Err(kind) = self.insert_staged(&task, key, eligible_at, old_key, 0) {
            if deadline_admitted {
                self.release_deadline(task.deadline_config());
                // SAFETY: the reservation still exclusively owns this virgin task.
                unsafe { *task.owned_state_mut() = old_state };
            }
            return Err(EevdfReservationCommitError { kind, reservation });
        }
        if matches!(params.class, EevdfTaskClass::RoundRobin) {
    ...
    fn migration_token_from_task(
        ...
        unsafe {
            let metadata = task.owned_state().migration.clone()?;
            Some(EevdfMigration {
                task: Some(task),
    ...
    pub fn begin_ready_migration(
        ...
        let throttled = unsafe { task.owned_state().deadline_throttled };
        let removed = if throttled {
            self.deadline_throttled.remove(task).map_err(tree_error)?
        } else {
            self.ready_tree.remove(task).map_err(tree_error)?
        };
        removed
        ...
        if is_deadline_class(params.class) {
            // Retain a shared-domain reservation through the handoff.  The
            // destination will transfer only its local ledger when it shares
            // this exact domain, leaving no admission window in between.
            if self.deadline_domain.is_some() {
                self.release_deadline_local(removed.deadline_config());
            } else {
                self.release_deadline(removed.deadline_config());
            }
        }
    ...
    pub fn begin_running_migration(
        ...
        if is_deadline_class(params.class) {
            if self.deadline_domain.is_some() {
                self.release_deadline_local(task.deadline_config());
            } else {
                self.release_deadline(task.deadline_config());
            }
        }
    ...
    pub fn commit_migration(
        ...
        let metadata = unsafe { task.owned_state().migration.clone() };
        ...
        let (key, eligible_at) = if let Some(entity) = entity.as_ref() {
            ...
            (
                if is_deadline_class(metadata.params.class) {
                    Self::deadline_key(metadata.deadline_absolute_ns, sequence)
                } else {
                    Self::rt_key(metadata.params, sequence)
                },
                0,
            )
        };
        let admitted_deadline = is_deadline_class(metadata.params.class);
        let retained_domain = metadata.deadline_domain.is_some();
        if let Some(source_domain) = metadata.deadline_domain.as_ref() {
            if self
                .deadline_domain
                .as_ref()
                .map_or(true, |domain| !Arc::ptr_eq(domain, source_domain))
            {
                // A retained reservation can only move within the domain which
                // owns it.  Leave the token intact so its source can roll back;
                // admitting it here would either leak the old ledger entry or
                // create a window for a competing reservation.
                return Err(Self::migration_error(
        ...
        if admitted_deadline {
            let admission = if retained_domain {
                self.admit_deadline_local(task.deadline_config())
            } else {
                self.admit_deadline(task.deadline_config())
            };
            if let Err(kind) = admission {
                return Err(Self::migration_error(kind, migration));
            }
        }
        let inserted = if metadata.deadline_throttled {
            let sequence = match self.peek_rt_sequence(false) {
                Ok(sequence) => sequence,
                Err(kind) => return Err(Self::migration_error(kind, migration)),
            };
            let key = Self::throttled_deadline_key(metadata.deadline_replenish_at_ns, sequence);
            Self::stage_key(&task, key, u128::from(metadata.deadline_replenish_at_ns));
            self.deadline_throttled
                .insert(Arc::clone(&task))
                .map_err(|error| {
                    let kind = tree_error(error.kind());
                    drop(error.into_node());
                    kind
                })
        } else {
    ...
    pub fn rollback_migration(
        ...
        // `begin_*_migration` released the source reservation before
        // parking. Reacquire it before publishing the task back on this run
        // queue; failure leaves the token intact for the caller to resolve.
        let admitted_deadline = is_deadline_class(metadata.params.class);
        let retained_domain = metadata.deadline_domain.is_some();
        if let Some(source_domain) = metadata.deadline_domain.as_ref() {
            if self
        ...
        let inserted = if metadata.deadline_throttled {
            let key = Self::throttled_deadline_key(
                metadata.deadline_replenish_at_ns,
                metadata.source_key.sequence(),
            );
            Self::stage_key(&task, key, u128::from(metadata.deadline_replenish_at_ns));
            self.deadline_throttled
                .insert(Arc::clone(&task))
        ...
        match (old_fair, new_fair) {
            (true, true) => {
                ...
                entity
                    ...
                    .map_err(model_error)?;
                Self::rebind_fair_runtime(&mut entity, fair_runtime_ns, total_weight, 10_000_000)?;
    ...
    fn set_task_params_impl(
        ...
        match task.owner() {
            ...
            UNOWNED => {
                ...
                let (entity, dormant, rt_sleeping) = self.sleeping_reconfigured_state(
                    &state,
                    old_params,
                    params,
                    task.fair_runtime_ns(),
                ...
                unsafe {
                    ...
                    if is_deadline_class(params.class) {
                        ...
                        next.deadline_remaining_ns = config.runtime_ns;
                        next.deadline_absolute_ns = self
                            .deadline_now_ns
                            .checked_add(config.deadline_ns)
                            .ok_or(SchedulerError::ArithmeticExhausted)?;
                        next.deadline_replenish_at_ns = self
                            .deadline_now_ns
                            .checked_add(config.period_ns)
                            .ok_or(SchedulerError::ArithmeticExhausted)?;
            ...
            _ => {
                ...
                let next_deadline_utilization = self.replaced_deadline_utilization(
                    is_deadline_class(old_params.class).then(|| task.deadline_config()),
                    is_deadline_class(params.class).then(|| task.deadline_config()),
                )?;
                let old_deadline =
                    is_deadline_class(old_params.class).then(|| task.deadline_config());
                let new_deadline = is_deadline_class(params.class).then(|| task.deadline_config());
                ...
                let replacement = if !running {
                    ...
                    } else if is_deadline_class(params.class) {
                        let config = task.deadline_config();
                        if !config.is_valid() {
                            return Err(SchedulerError::InvalidParameters);
                        }
                ...
                self.replace_deadline_domain(old_deadline, new_deadline)?;
                let (removed, old_key, old_eligible_at, sequence_kind) = if running {
                    ...
                    let removed = match (if old_state.deadline_throttled {
                        self.deadline_throttled.remove(task)
                    } else {
                        self.ready_tree.remove(task)
                    })
                    .map_err(tree_error)
                    {
                        Ok(removed) => removed,
                        Err(error) => {
                            self.replace_deadline_domain(new_deadline, old_deadline)
                                .expect("deadline domain replacement rollback failed");
                            return Err(error);
                        }
                    };
                ...
                if sequence_kind.is_some() {
                        ...
                        self.insert_staged(task, key, eligible_at, old_key, old_eligible_at)
                    {
                        ...
                        if old_state.deadline_throttled {
                            self.deadline_throttled
                                .insert(removed)
                                .expect("EEVDF throttled rollback encountered a duplicate old key");
                        } else {
                            self.ready_tree
                                .insert(removed)
                ...
                self.clock = next_clock;
                self.deadline_utilization = next_deadline_utilization;
                drop(removed);
    ...
    pub fn set_task_params_with_runtime(
        ...
        if running {
            ...
            let before_state = unsafe { task.owned_state().clone() };
    ...
    /// Change a CBS reservation while holding its owning scheduler lock.
    /// Unlike `configure_deadline`, this is valid for runnable and current
    /// tasks and publishes the budget/deadline state with the new immutable
    /// task configuration before the task can be selected again.
    pub fn set_task_deadline_config_with_runtime(
        &mut self,
        task: &Arc<EEVDFTask<T>>,
        config: DeadlineParameters,
    ) -> Result<EevdfParamUpdate<T>, SchedulerError> {
        if !config.is_valid() || !is_deadline_class(task.sched_params().class) {
            return Err(SchedulerError::InvalidParameters);
        }
        match task.owner() {
            UNOWNED => {
                let claim = task.value().claim_reconfiguration()?;
                // UNOWNED also represents a parked migration. Its source
                // snapshot retains the exact admitted tuple in the shared
                // domain, so mutating this task would invalidate rollback or
                // same-domain commit accounting.
                if unsafe { task.owned_state().migration.is_some() } {
                    drop(claim);
                    return Err(SchedulerError::TaskBusy);
                }
                // A sleeping deadline server retains its policy-lifetime
                // admission. Replace its retained CBS state before publishing
                // the tuple, so wakeup observes one coherent new server rather
                // than the old residual budget/deadline. Its
                // node is unlinked, but stage its EDF key as well to avoid
                // retaining stale ordering state across the sleep boundary.
                let sleeping_deadline = is_deadline_class(task.sched_params().class)
                    && unsafe { task.owned_state().rt_sleeping };
                if sleeping_deadline {
                    let old_config = task.deadline_config();
                    let next_utilization =
                        self.replaced_deadline_utilization(Some(old_config), Some(config))?;
                    let absolute = self
                        .deadline_now_ns
                        .checked_add(config.deadline_ns)
                        .ok_or(SchedulerError::ArithmeticExhausted)?;
                    let replenish = self
                        ...
                        .checked_add(config.period_ns)
                        .ok_or(SchedulerError::ArithmeticExhausted)?;
                    let sequence = unsafe { task.key().sequence };
                    // Sleeping is not an admission boundary. Replace the
                    // policy-lifetime reservation in both ledgers before
                    // publishing the coherent new CBS state.
                    self.replace_deadline_domain(Some(old_config), Some(config))?;
                    task.value().publish_deadline_config(config);
                    unsafe {
                        let mut state = task.owned_state_mut();
                        state.deadline_remaining_ns = config.runtime_ns;
                        state.deadline_absolute_ns = absolute;
                    ...
                    Self::stage_key(task, Self::deadline_key(absolute, sequence), 0);
                    self.deadline_utilization = next_utilization;
                    claim.finish()?;
                    return Ok(EevdfParamUpdate::NoPreemption);
                }
                task.value().publish_deadline_config(config);
                claim.finish()?;
            ...
            _ => {
                ...
                if !running {
                    let old_key = unsafe { task.key() };
                    let old_eligible = unsafe { task.eligible_at() };
                    let sequence = match self.peek_rt_sequence(false) {
                        Ok(sequence) => sequence,
                        Err(error) => {
                            self.replace_deadline_domain(Some(config), Some(old_config))
                                .expect("deadline domain replacement rollback failed");
                            return Err(error);
                        }
                    };
                    let removed = if old_state.deadline_throttled {
                        self.deadline_throttled.remove(task).map_err(tree_error)
                    } else {
                        self.ready_tree.remove(task).map_err(tree_error)
                    };
                    ...
                    if let Err(error) = self.insert_staged(
                        task,
                        Self::deadline_key(absolute, sequence),
                        0,
                        old_key,
                        old_eligible,
                    ) {
                        self.ready_tree
                            .insert(removed)
                            .expect("deadline config rollback failed");
                        return Err(error);
                    }
                    self.commit_rt_sequence(false);
                }
    ...
    /// Stage validated CBS parameters for a same-transaction class change.
    /// The caller already owns this run queue and immediately follows with
    /// `set_task_params_with_runtime`; no external observer can interleave.
    pub fn stage_task_deadline_config(
        &mut self,
        task: &Arc<EEVDFTask<T>>,
        config: DeadlineParameters,
    ) -> Result<(), SchedulerError> {
        ...
        if owner == UNOWNED {
            ...
            task.value().publish_deadline_config(config);
            claim.finish()
        } else if owner == self.id && self.id != UNOWNED {
            task.value().publish_deadline_config(config);
            Ok(())
        } else {
    ...
    /// Atomically change a fair task's requested slice. The
    /// request is stored on the task and, for an admitted fair entity, is
    /// immediately rebound into the EEVDF deadline calculation under this
    /// scheduler's ownership.  Thus it is a scheduling input rather than
    /// externally visible metadata only.
    pub fn set_task_fair_runtime_with_runtime(
        &mut self,
        task: &Arc<EEVDFTask<T>>,
        runtime_ns: u64,
        period_ns: u64,
        ...
        let params = task.sched_params();
        if runtime_ns != 0
            && !matches!(params.class, EevdfTaskClass::Normal | EevdfTaskClass::Batch)
        {
            return Err(SchedulerError::InvalidParameters);
        }
        if is_rt_class(params.class) {
            ...
            task.value().set_fair_runtime_ns(0);
            return Ok(EevdfParamUpdate::NoPreemption);
        }
        match task.owner() {
            CONFIGURING => Err(SchedulerError::TaskBusy),
            UNOWNED => {
                ...
                if let Some(mut entity) = state.entity {
                    Self::rebind_fair_runtime(
                        &mut entity,
                        runtime_ns,
                        self.clock.total_weight,
                        period_ns,
                    )?;
                    unsafe { task.owned_state_mut().entity = Some(entity) };
                }
                task.value().set_fair_runtime_ns(runtime_ns);
                claim.finish().map(|_| EevdfParamUpdate::NoPreemption)
            }
            owner if owner != self.id || self.id == UNOWNED => Err(SchedulerError::ForeignQueue),
            _ => {
                ...
                if running {
                    let before_clock = self.clock;
                    let before_state = unsafe { task.owned_state().clone() };
                    let before_runtime = task.fair_runtime_ns();
                    self.settle_runtime_remainder(task, period_ns)?;
                    let result = (|| {
                        let mut entity = unsafe { task.owned_state().entity }
                            .ok_or(SchedulerError::InconsistentState)?;
                        Self::rebind_fair_runtime(
                            &mut entity,
                            runtime_ns,
                    ...
                    if result.is_err() {
                        self.clock = before_clock;
                        unsafe { *task.owned_state_mut() = before_state };
                        task.value().set_fair_runtime_ns(before_runtime);
                    }
                    return result;
                }
                if !task.is_linked() {
                ...
                let sequence = self.peek_fair_sequence()?;
                let (key, eligible_at) = Self::fair_key(&entity, sequence)?;
                let old_key = unsafe { task.key() };
                ...
                if let Err(error) =
                    self.insert_staged(task, key, eligible_at, old_key, old_eligible_at)
                {
                    if self.ready_tree.insert(removed).is_err() {
                        return Err(SchedulerError::InconsistentState);
                    }
                    return Err(error);
                }
    ...
    fn enqueue_wakeup(&mut self, task: Arc<EEVDFTask<T>>) -> Result<(), SchedulerError> {
        ...
        let mut throttle_deadline = false;
        let (next_clock, entity, dormant_fair, key, eligible_at, reset_rr, renewed_deadline) =
            if let Some(entity) = old_entity {
                ...
                let dormant_fair = match old_dormant_fair {
                    Some(mut dormant) => {
                        if let Err(error) =
                            dormant.end_sleep_frozen(self.clock.v).map_err(model_error)
                        {
                            self.release_claim(&task);
                            return Err(error);
                        }
                ...
                let (key, renewed_deadline) = if is_deadline_class(params.class) {
                    // Sleeping CBS servers retain their residual budget and
                    // absolute deadline. Linux wakeup does not create a fresh
                    // reservation: admission is retained for the complete
                    // lifetime of the deadline policy.
                    let config = task.deadline_config();
                    let state = unsafe { task.owned_state() };
                    // A server can block precisely as it consumes its final
                    // budget unit.  It is no longer linked in the throttle
                    // tree while asleep, but its CBS replenishment remains
                    // due at the original instant.  Recreate that structural
                    // state on wake instead of treating zero residual budget
                    // as an impossible sleeper.
                    let exhausted = state.deadline_remaining_ns == 0;
                    let renewed = if (exhausted
                    ...
                    throttle_deadline = exhausted && renewed.is_none();
                    let key = if throttle_deadline {
                        Self::throttled_deadline_key(state.deadline_replenish_at_ns, sequence)
                    } else {
                        let absolute =
                            renewed.map_or(state.deadline_absolute_ns, |(_, absolute, _)| absolute);
                        Self::deadline_key(absolute, sequence)
                    };
                    (key, renewed)
                } else {
                    (Self::rt_key(params, sequence), None)
                };
                (
                    self.clock,
                    None,
        ...
        if throttle_deadline {
            Self::stage_key(&task, key, eligible_at);
            if let Err(error) = self.deadline_throttled.insert(Arc::clone(&task)) {
                Self::stage_key(&task, old_key, old_eligible_at);
                let kind = error.kind();
                drop(error.into_node());
                self.release_claim(&task);
                return Err(tree_error(kind));
            }
        ...
        unsafe {
            ...
            task.owned_state_mut().rt_sleeping = false;
            task.owned_state_mut().sleep_v = None;
            if let Some((remaining, absolute, replenish)) = renewed_deadline {
    ...
    fn put_running(
        ...
        if is_deadline_class(params.class) {
            let state = unsafe { prev.owned_state().clone() };
            let sequence = self.next_rt_sequence(false)?;
            if state.deadline_throttled {
                Self::stage_key(
                    &prev,
                    Self::throttled_deadline_key(state.deadline_replenish_at_ns, sequence),
                    u128::from(state.deadline_replenish_at_ns),
                );
                self.deadline_throttled
                    .insert(Arc::clone(&prev))
                    .map_err(|error| {
                        let kind = error.kind();
                        drop(error.into_node());
                        tree_error(kind)
                ...
                let old_eligible = unsafe { prev.eligible_at() };
                self.insert_staged(
                    &prev,
                    Self::deadline_key(state.deadline_absolute_ns, sequence),
                    0,
                    old_key,
                    old_eligible,
                )?;
    ...
    fn remove_ready(
        ...
        let removed = if unsafe { task.owned_state().deadline_throttled } {
            self.deadline_throttled.remove(task).map_err(tree_error)?
        } else {
            self.ready_tree.remove(task).map_err(tree_error)?
        };
        if is_deadline_class(task.sched_params().class) {
            self.release_deadline(task.deadline_config());
        }
    ...
    fn has_higher_rt(&self, priority: u8) -> bool {
        self.ready_tree.front().is_some_and(|task| unsafe {
            let key = task.key();
            key.class_rank() == DEADLINE_CLASS_RANK
    ...
    fn account_deadline_runtime(
        ...
        Ok(exhausted
            || self.ready_tree.front().is_some_and(|task| unsafe {
                task.key().class_rank() == DEADLINE_CLASS_RANK
                    && task.key().order() < u128::from(state.deadline_absolute_ns)
            }))
    }
    ...
    fn account_runtime_transaction(
        ...
        self.deadline_now_ns = self
            .deadline_now_ns
            .checked_add(delta.elapsed_ns())
            .ok_or(SchedulerError::ArithmeticExhausted)?;
        self.replenish_deadlines();
...
impl<T> BaseScheduler for EEVDFScheduler<T> {
    ...
    fn task_tick(&mut self, current: &Self::SchedItem) -> bool {
        ...
        if is_deadline_class(params.class) {
            self.replenish_deadlines();
            self.account_deadline_runtime(current, 1)
                .expect("deadline tick accounting failed")
        } else if is_rt_class(params.class) {
...
mod tests {
    ...
    fn runtime_remainder_class_transition_charges_old_class_first() {
        let _quantum = crate::test_utils::read_rr_timeslice();
    ...
    fn runtime_settlement_stages_whole_fraction_and_remainder_atomically() {
        ...
        let before_state = unsafe { current.owned_state().clone() };
        ...
        assert_eq!(
            format!("{:?}", unsafe { current.owned_state() }),
            format!("{before_state:?}")
        );
    ...
    fn eevdf_ready_rollback_without_intervention_restores_exact_source_state() {
        ...
        let source_eligible_at = unsafe { task.eligible_at() };
        let migration = source.begin_ready_migration(&task).unwrap();
        let metadata = unsafe { task.owned_state().migration.clone().unwrap() };
    ...
    fn eevdf_failed_ready_reconfigure_restores_node_and_clock() {
        ...
        let before_state = unsafe { second.owned_state().clone() };
        ...
        assert_eq!(
            format!("{:?}", unsafe { second.owned_state() }),
            format!("{before_state:?}")
        );
    ...
    #[test]
    fn fair_runtime_request_rebinds_the_live_eevdf_deadline() {
        let mut scheduler = EEVDFScheduler::new();
        let task = Arc::new(EEVDFTask::new(1usize));
        scheduler.add_task(Arc::clone(&task)).unwrap();
        let default_q = unsafe { task.owned_state().entity.unwrap().request.q };

        let _ = scheduler
            .set_task_fair_runtime_with_runtime(&task, 1, 10_000_000)
            .unwrap();
        let short_q = unsafe { task.owned_state().entity.unwrap().request.q };
        assert_eq!(task.fair_runtime_ns(), 1);
        assert_eq!(short_q, 1);
        assert_ne!(short_q, default_q);

        let _ = scheduler
            .set_task_fair_runtime_with_runtime(&task, 100_000_000, 10_000_000)
            .unwrap();
        let long_q = unsafe { task.owned_state().entity.unwrap().request.q };
        assert_eq!(task.fair_runtime_ns(), 100_000_000);
        assert_eq!(long_q, 10);
        assert!(long_q > short_q);
    }

    #[test]
    fn requested_slice_is_not_abi_clamped_and_accepts_batch_but_not_idle() {
        let normal = EEVDFTask::new(1usize);
        normal.configure_fair_runtime_ns(1).unwrap();
        assert_eq!(normal.fair_runtime_ns(), 1);
        normal.configure_fair_runtime_ns(u64::MAX).unwrap();
        assert_eq!(normal.fair_runtime_ns(), u64::MAX);

        let batch = EEVDFTask::new(2usize);
        batch
            .configure(EevdfTaskParams {
                class: EevdfTaskClass::Batch,
                ..EevdfTaskParams::default()
            ...
            .unwrap();
        batch.configure_fair_runtime_ns(1).unwrap();
        assert_eq!(batch.fair_runtime_ns(), 1);

        let idle = EEVDFTask::new(3usize);
        idle.configure(EevdfTaskParams {
            class: EevdfTaskClass::Idle,
            ..EevdfTaskParams::default()
        })
        .unwrap();
        assert_eq!(
            idle.configure_fair_runtime_ns(1),
            Err(SchedulerError::InvalidParameters)
        );
    }

    #[test]
    fn sleeping_deadline_reconfiguration_resets_cbs_for_next_wakeup() {
        let domain = Arc::new(DeadlineAdmissionDomain::new());
        let mut scheduler = EEVDFScheduler::new();
        scheduler
            .set_deadline_admission_domain(Arc::clone(&domain))
            .unwrap();
        let task = Arc::new(EEVDFTask::new(1));
        let old = DeadlineParameters {
            runtime_ns: 25,
            deadline_ns: 100,
            period_ns: 120,
        };
        ...
        scheduler.add_task(Arc::clone(&task)).unwrap();
        let current = scheduler.pick_next_task().unwrap();
        scheduler.advance_deadline_clock(100);
        unsafe { task.owned_state_mut().deadline_remaining_ns = 1 };
        scheduler.deactivate_task(&current, DeactivateReason::Sleep);

        assert_eq!(scheduler.deadline_utilization, 208_333_334);
        // The task layer rebases the owner to a freshly sampled monotonic
        // timestamp before changing a sleeping CBS tuple.
        scheduler.advance_deadline_clock(150);
        let _ = scheduler
            .set_task_deadline_config_with_runtime(&task, replacement)
            .unwrap();
        let state = unsafe { task.owned_state() };
        assert_eq!(task.deadline_config(), replacement);
        assert_eq!(state.deadline_remaining_ns, replacement.runtime_ns);
        assert_eq!(state.deadline_absolute_ns, 230);
        assert_eq!(state.deadline_replenish_at_ns, 240);
        assert!(!state.deadline_throttled);
        assert_eq!(scheduler.deadline_utilization, 444_444_445);
        assert_eq!(domain.utilization.load(Ordering::Acquire), 444_444_445);
    ...
    #[test]
    fn sleeping_deadline_server_keeps_admission_and_wakes_without_readmission() {
        let domain = Arc::new(DeadlineAdmissionDomain::new());
        let mut scheduler = EEVDFScheduler::new();
        scheduler
            .set_deadline_admission_domain(Arc::clone(&domain))
            .unwrap();
        let sleeper = Arc::new(EEVDFTask::new(1));
        let contender = Arc::new(EEVDFTask::new(2));
        let sleeper_config = DeadlineParameters {
            runtime_ns: 60,
            deadline_ns: 100,
            period_ns: 100,
        };
        ...
        scheduler
            ...
            .unwrap();
        let _ = scheduler
            .set_task_params(&sleeper, rt(EevdfTaskClass::Deadline, 0))
            .unwrap();
        scheduler.add_task(Arc::clone(&sleeper)).unwrap();
        let current = scheduler.pick_next_task().unwrap();
        scheduler.deactivate_task(&current, DeactivateReason::Sleep);

        assert_eq!(scheduler.deadline_utilization, 600_000_000);
        assert_eq!(domain.utilization.load(Ordering::Acquire), 600_000_000);
        scheduler
            .stage_task_deadline_config(&contender, contender_config)
            .unwrap();
        let _ = scheduler
            .set_task_params(&contender, rt(EevdfTaskClass::Deadline, 0))
            .unwrap();
        assert_eq!(scheduler.add_task(contender), Err(SchedulerError::TaskBusy));
    ...
    #[test]
    fn deadline_migration_rollback_restores_source_reservation() {
        let mut scheduler = EEVDFScheduler::new();
        let task = Arc::new(EEVDFTask::new(1));
        ...
        let _ = scheduler
            ...
            .unwrap();
        scheduler.add_task(task.clone()).unwrap();
        assert_eq!(scheduler.deadline_utilization, 250_000_000);
        let migration = scheduler.begin_ready_migration(&task).unwrap();
        assert_eq!(scheduler.deadline_utilization, 0);
        scheduler.rollback_migration(migration).unwrap();
        assert_eq!(scheduler.deadline_utilization, 250_000_000);
        assert!(task.is_linked());
    }

    #[test]
    fn task_timeslice_is_scheduler_owned_and_excludes_idle() {
        let mut scheduler = EEVDFScheduler::new();
        let normal = Arc::new(EEVDFTask::new(()));
        scheduler.add_task(normal.clone()).unwrap();
        assert!(scheduler.task_timeslice(&normal, false).unwrap().1 > 0);

        let idle = Arc::new(EEVDFTask::new(()));
        idle.configure(EevdfTaskParams {
            class: EevdfTaskClass::Idle,
            nice: 0,
            rt_priority: 0,
        ...
        .unwrap();
        scheduler.add_task(idle.clone()).unwrap();
        assert_eq!(scheduler.task_timeslice(&idle, false).unwrap().1, 0);
    }

    #[test]
    fn sleeping_fair_task_preserves_its_request_quantum() {
        let mut scheduler = EEVDFScheduler::new();
        let task = Arc::new(EEVDFTask::new(()));
        scheduler.add_task(task.clone()).unwrap();
        let running = scheduler.pick_next_task().unwrap();
        let expected = scheduler.task_timeslice(&running, false).unwrap().1;
        scheduler.deactivate_task(&running, DeactivateReason::Sleep);

        assert_eq!(scheduler.clock().total_weight, 0);
        assert!(expected > 0);
        assert_eq!(scheduler.task_timeslice(&task, true).unwrap().1, 0);
        assert_eq!(
            scheduler.task_timeslice(&task, false),
            Err(SchedulerError::TaskBusy)
        );
    }

    #[test]
    fn task_timeslice_rejects_pre_admission_and_migration_states() {
        let mut scheduler = EEVDFScheduler::new();
        let task = Arc::new(EEVDFTask::new(()));
        assert_eq!(
            scheduler.task_timeslice(&task, false),
    ...
    #[test]
    fn parked_deadline_migration_rejects_config_mutation_and_staging() {
        let mut scheduler = EEVDFScheduler::new();
        let task = Arc::new(EEVDFTask::new(1));
        let original = DeadlineParameters {
            runtime_ns: 25,
            deadline_ns: 100,
            period_ns: 100,
        };
        ...
        let parked = scheduler
            .begin_ready_migration(&task)
            .unwrap()
            .park()
            .unwrap();

        assert_eq!(
            scheduler.set_task_deadline_config_with_runtime(&parked, replacement),
            Err(SchedulerError::TaskBusy)
        );
        assert_eq!(
            scheduler.stage_task_deadline_config(&parked, replacement),
            Err(SchedulerError::TaskBusy)
        );
        assert_eq!(parked.deadline_config(), original);
    }

    #[test]
    fn deadline_wake_renews_an_expired_residual_server() {
        let mut scheduler = EEVDFScheduler::new();
        let task = Arc::new(EEVDFTask::new(1));
        let config = DeadlineParameters {
            runtime_ns: 25,
        ...
        scheduler.add_task(Arc::clone(&task)).unwrap();
        let current = scheduler.pick_next_task().unwrap();
        unsafe { task.owned_state_mut().deadline_remaining_ns = 7 };
        scheduler.deactivate_task(&current, DeactivateReason::Sleep);
        scheduler.advance_deadline_clock(101);

        scheduler
            .enqueue_task(Arc::clone(&task), EnqueueReason::Wakeup)
            .unwrap();
        let state = unsafe { task.owned_state() };
        assert_eq!(state.deadline_remaining_ns, config.runtime_ns);
        assert_eq!(state.deadline_absolute_ns, 201);
        assert_eq!(state.deadline_replenish_at_ns, 221);
    }

    #[test]
    fn exhausted_sleeping_deadline_server_wakes_throttled_or_renewed() {
        let domain = Arc::new(DeadlineAdmissionDomain::new());
        let mut scheduler = EEVDFScheduler::new();
        scheduler
            .set_deadline_admission_domain(Arc::clone(&domain))
        ...
        let _ = scheduler
            ...
            .unwrap();
        scheduler.add_task(Arc::clone(&task)).unwrap();
        let admitted = domain.utilization.load(Ordering::Acquire);

        let current = scheduler.pick_next_task().unwrap();
        assert!(scheduler.account_runtime(&current, RuntimeDelta::new(25, 1)));
        scheduler.deactivate_task(&current, DeactivateReason::Sleep);
        assert_eq!(unsafe { task.owned_state().deadline_remaining_ns }, 0);

        // Before its CBS replenishment instant, the sleeping server must be
        // structurally throttled rather than rejected as an invalid wakeup.
        scheduler
            .enqueue_task(Arc::clone(&task), EnqueueReason::Wakeup)
            .unwrap();
        ...
        let current = scheduler.pick_next_task().unwrap();
        assert_eq!(
            unsafe { current.owned_state().deadline_remaining_ns },
            config.runtime_ns
        );
        assert!(scheduler.account_runtime(&current, RuntimeDelta::new(25, 1)));
        scheduler.deactivate_task(&current, DeactivateReason::Sleep);

        // Once due, a direct wake renews the same lifetime reservation and
        // makes the server runnable immediately.
        scheduler.advance_deadline_clock(240);
        scheduler
            .enqueue_task(Arc::clone(&task), EnqueueReason::Wakeup)
            .unwrap();
        let state = unsafe { task.owned_state() };
        ...
        assert_eq!(state.deadline_remaining_ns, config.runtime_ns);
        assert_eq!(state.deadline_absolute_ns, 340);
    ...
    #[test]
    fn deadline_migration_keeps_shared_domain_reservation_continuous() {
        let domain = Arc::new(DeadlineAdmissionDomain::new());
        let config = DeadlineParameters {
            runtime_ns: 60,
            deadline_ns: 100,
        ...
        source
            ...
            .unwrap();
        destination
            .set_deadline_admission_domain(domain.clone())
            .unwrap();
        let task = Arc::new(EEVDFTask::new(1));
        source.stage_task_deadline_config(&task, config).unwrap();
        let _ = source
            .set_task_params(&task, rt(EevdfTaskClass::Deadline, 0))
            .unwrap();
        source.add_task(Arc::clone(&task)).unwrap();
        let admitted = domain.utilization.load(Ordering::Acquire);

        let migration = source.begin_ready_migration(&task).unwrap();
        assert_eq!(source.deadline_utilization, 0);
        assert_eq!(domain.utilization.load(Ordering::Acquire), admitted);
        destination.commit_migration(migration).unwrap();
        assert_eq!(destination.deadline_utilization, admitted as u128);
        assert_eq!(domain.utilization.load(Ordering::Acquire), admitted);
    }

    #[test]
    fn deadline_domain_caps_affinity_aggregate_across_runqueues() {
        let domain = Arc::new(DeadlineAdmissionDomain::new());
        let config = DeadlineParameters {
            runtime_ns: 60,
            deadline_ns: 100,
        ...
        first.set_deadline_admission_domain(domain.clone()).unwrap();
        second.set_deadline_admission_domain(domain).unwrap();
        let a = Arc::new(EEVDFTask::new(1));
        let b = Arc::new(EEVDFTask::new(2));
        for (scheduler, task) in [(&mut first, &a), (&mut second, &b)] {
            scheduler.stage_task_deadline_config(task, config).unwrap();
            let _ = scheduler
                .set_task_params(task, rt(EevdfTaskClass::Deadline, 0))
                .unwrap();
        ...
        assert_eq!(second.add_task(b.clone()), Err(SchedulerError::TaskBusy));
        assert_eq!(b.owner(), UNOWNED);
    }
}
