...
use crate::{
    ...
    future::{BlockOnError, DeadlineReservation, Elapsed, TimerRegistrationError, block_on},
};
...
macro_rules! irq_safe_listener {
    ($event:expr => $listener:ident) => {
        let mut $listener = core::pin::pin!(IrqSafeListenerSlot::new(&$event));
        let mut $listener = $listener.as_mut().listen();
    };
}
...
impl WaitQueue {
    ...
    pub fn wait(&self) -> Result<(), WaitError> {
        irq_safe_listener!(self.event => listener);
        block_on(poll_fn(|cx| poll_listener(Pin::new(&mut listener), cx)))
            .map(|_| ())
...
mod tests {
    ...
    #[test]
    fn cancelling_selected_stack_listener_forwards_notification() {
        let queue = WaitQueue::new();
        let mut next = core::pin::pin!(IrqSafeListenerSlot::new(&queue.event));
        let mut cx = core::task::Context::from_waker(core::task::Waker::noop());
        let mut next_listener;
        {
            irq_safe_listener!(queue.event => selected);
            next_listener = next.as_mut().listen();
            assert!(poll_listener(Pin::new(&mut selected), &mut cx).is_pending());
            assert!(poll_listener(Pin::new(&mut next_listener), &mut cx).is_pending());
            assert!(queue.notify_one(false));
            // Cancellation drops the owning slot, rather than polling the
            // selected listener. Its notification must pass to the next one.
        }
        assert_eq!(
            poll_listener(Pin::new(&mut next_listener), &mut cx),
            Poll::Ready(WaitWake::Notified)
        );
    }

    #[test]
    fn cancelling_pending_stack_listener_unlinks_it() {
        let queue = WaitQueue::new();
        {
            irq_safe_listener!(queue.event => listener);
            let mut cx = core::task::Context::from_waker(core::task::Waker::noop());
            assert!(poll_listener(Pin::new(&mut listener), &mut cx).is_pending());
        }
        assert_eq!(queue.notify_many(usize::MAX, false), 0);
    }
}
