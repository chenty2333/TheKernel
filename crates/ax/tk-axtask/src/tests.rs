...
#[test]
fn affinity_admission_rejects_an_empty_mask_before_mutation() {
    assert_eq!(
        axtask::validate_affinity_mask(crate::AxCpuMask::new()),
...
#[cfg(feature = "sched-eevdf")]
#[test]
fn versioned_sched_transaction_keeps_policy_rejection_and_commit_atomic() {
    let _lock = SERIAL.lock();
    init_for_test();

    let task = current().clone();
    let original = axtask::task_scheduling_snapshot(&task).unwrap();
    let rejected: Result<
        Result<crate::TaskSchedulingSnapshot, &'static str>,
        crate::TaskSchedError,
    > = axtask::update_sched_state_versioned_with_spawn_reset(&task, |old| {
        assert_eq!(old, original);
        Err("policy rejected")
    });
    assert_eq!(rejected, Ok(Err("policy rejected")));
    assert_eq!(axtask::task_scheduling_snapshot(&task).unwrap(), original);

    let committed = axtask::update_sched_state_versioned_with_spawn_reset(&task, |old| {
        assert_eq!(old, original);
        let mut state = old.state;
        state.nice = 1;
        Ok::<_, ()>((state, !old.reset_on_spawn))
    })
    .unwrap()
    .unwrap();
    assert_eq!(committed.state.nice, 1);
    assert_eq!(committed.reset_on_spawn, !original.reset_on_spawn);
    assert_eq!(committed.version, original.version.wrapping_add(1));

    let snapshot = axtask::task_scheduling_snapshot(&task).unwrap();
    assert_eq!(snapshot, committed);
    assert_eq!(snapshot.deadline, crate::DeadlineParameters::default());
    assert_eq!(snapshot.utilization_bounds, original.utilization_bounds);

    axtask::set_sched_state_versioned_with_spawn_reset(
        &task,
        original.state,
        original.reset_on_spawn,
    )
    .unwrap();
}
...
#[cfg(feature = "sched-eevdf")]
#[test]
fn scheduling_update_commits_utilization_bounds_with_scheduler_state() {
    let _lock = SERIAL.lock();
    init_for_test();

    let task = current().clone();
    let original = axtask::task_scheduling_snapshot(&task).unwrap();
    let committed = axtask::update_task_scheduling(&task, |old| {
        assert_eq!(old, original);
        let mut state = old.state;
        state.nice = 2;
        Ok::<_, ()>(crate::TaskSchedulingUpdate {
            state,
            reset_on_spawn: !old.reset_on_spawn,
            requested_slice: crate::RequestedSlice::profile_default(),
            deadline: old.deadline,
            utilization_bounds: crate::UtilizationBounds::new(256, 768).unwrap(),
            uclamp: crate::UclampRequest {
                minimum: 256,
                maximum: 768,
    ...
    .unwrap();
    assert_eq!(
        committed.utilization_bounds,
        crate::UtilizationBounds::new(256, 768).unwrap()
    );
    assert_eq!(committed.state.nice, 2);
    assert_eq!(committed.version, original.version.wrapping_add(1));

    let observed = axtask::task_scheduling_snapshot(&task).unwrap();
    assert_eq!(observed, committed);

    axtask::update_task_scheduling(&task, |_| {
        Ok::<_, ()>(crate::TaskSchedulingUpdate {
            state: original.state,
            reset_on_spawn: original.reset_on_spawn,
            requested_slice: original.requested_slice,
            deadline: original.deadline,
            utilization_bounds: original.utilization_bounds,
            uclamp: original.uclamp,
        })
    })
    .unwrap()
    .unwrap();
...
#[test]
fn task_allowed_active_cpus_snapshots_initialized_cpus_and_rejects_exit() {
    let _lock = SERIAL.lock();
    init_for_test();

    let current = current().clone();
    assert_eq!(
        axtask::task_allowed_active_cpus(&current).unwrap(),
        current.cpumask() & crate::run_queue::active_cpu_mask()
    );
    assert_eq!(
        axtask::task_affinity_nr_cpu_ids(),
        axconfig::plat::MAX_CPU_NUM
    );
    assert_eq!(
        axtask::task_affinity_mask_bytes(),
        axconfig::plat::MAX_CPU_NUM.div_ceil(usize::BITS as usize) * core::mem::size_of::<usize>()
    );

    let exited = crate::TaskInner::new_init("affinity-exited".into())
        .unwrap()
        .into_arc()
        .unwrap();
    exited.notify_exit(0);
    assert_eq!(
        axtask::task_allowed_active_cpus(&exited),
        Err(crate::TaskSchedError::TaskExited)
    );
}
