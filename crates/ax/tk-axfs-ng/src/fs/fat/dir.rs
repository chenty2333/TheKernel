...
use axfs_ng_vfs::{
    ...
    NodeUserData, ObjectKey, Reference, RenameRequest, UnlinkRequest, VfsError, VfsResult,
    WeakDirEntry,
    path::{FsName, FsNameBuf},
};
use spin::Once;
...
fn fat_name(name: &FsName) -> VfsResult<&str> {
    core::str::from_utf8(name.as_bytes()).map_err(|_| VfsError::InvalidInput)
}

fn try_ascii_lowercase(value: &str) -> VfsResult<FsNameBuf> {
...
fn try_clone_name(value: &FsName) -> VfsResult<FsNameBuf> {
    let mut result = Vec::new();
    result
...
impl NodeOps for FatDirNode {
    ...
    fn object_key(&self) -> ObjectKey {
        ...
        ObjectKey::new(
            ...
            identity.position.unwrap_or(self.inode),
            identity.generation,
        )
    }
...
impl DirNodeOps for FatDirNode {
    ...
    fn read_dir(&self, offset: u64, sink: &mut dyn DirEntrySink) -> VfsResult<usize> {
        ...
        for entry in dir.iter().skip(offset as usize) {
            ...
            let name =
                try_ascii_lowercase(&entry.try_file_name().map_err(|_| VfsError::NoMemory)?)?;
            let node_type = if entry.is_file() {
                NodeType::RegularFile
            } else {
                NodeType::Directory
            };
            if let Some(entry) = dir_node.lookup_cache(name.as_name()) {
                if !sink.accept(name.as_name(), entry.inode(), node_type, offset + count + 1) {
                    break;
                }
            } else {
                let reference_name = try_clone_name(name.as_name())?;
                let inode = self.fs.alloc_inode()?;
                let entry = self.create_entry(&fs, entry, reference_name, inode)?;
                let inode = entry.inode();
                let accepted = sink.accept(name.as_name(), inode, node_type, offset + count + 1);
    ...
    fn create_named(
        ...
        // FAT has no native project-id or POSIX ACL representation. Reject
        // prepared state before directory-entry creation; do not emulate it
        // with a post-publication side table.
    ...
    fn unlink(&self, request: UnlinkRequest<'_>) -> VfsResult<()> {
        let fs = self.fs.lock();
        let name = fat_name(request.name)?;
    ...
    fn rename(&self, request: RenameRequest<'_>) -> VfsResult<()> {
        ...
        let fs = self.fs.lock();
        let src_name = fat_name(request.src_name)?;
        let dst_name = fat_name(request.dst_name)?;
...

        {
            ...
            let result = if dst_info.is_some() {
                dir.rename_replace(src_name, dst_inner, dst_name)
            } else {
                dir.rename(src_name, dst_inner, dst_name)
            };
