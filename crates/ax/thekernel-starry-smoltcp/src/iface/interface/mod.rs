    ...
    fn socket_egress(
        ...
        for item in sockets.items_mut() {
            ...
            let socket_result = match &mut item.socket {
                ...
                Socket::Raw(socket) => socket.dispatch(
                    &mut self.inner,
                    |inner, (raw_route, ip, raw)| {
                        let mut meta = PacketMeta::default();
                        #[cfg(feature = "packetmeta-id")]
                        {
                            meta.id = raw_route;
                        }
                        respond(inner, meta, Packet::new(ip, IpPayload::Raw(raw)))
                    },
                    |_| {},
                ),
            ...
            IpRepr::Ipv4(_repr) => {
                ...
                if total_ip_len > self.caps.ip_mtu() {
                    #[cfg(feature = "proto-ipv4-fragmentation")]
                    {
                        ...
                        _repr.payload_len = first_frag_ip_len - _repr.buffer_len();
