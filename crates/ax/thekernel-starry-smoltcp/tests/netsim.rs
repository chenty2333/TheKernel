...
fn netsim() {
    ...
    let mut previous_by_loss = [0.0_f64; 8];
    for buffer in buffers {
        let mut previous_for_buffer = f64::INFINITY;
        for (loss_index, loss) in losses.iter().copied().enumerate() {
            let throughput = run_test(TestCase {
            ...
            assert!(
                ...
                "invalid throughput for buffer={buffer}, loss={loss}: {throughput}"
            );
            assert!(
                throughput <= previous_for_buffer,
                "throughput increased with loss for buffer={buffer}: \
                 {throughput} > {previous_for_buffer}"
            );
            assert!(
                throughput >= previous_by_loss[loss_index],
                "throughput decreased with buffer for loss={loss}: \
                 {throughput} < {}",
                previous_by_loss[loss_index]
            );
            previous_for_buffer = throughput;
            previous_by_loss[loss_index] = throughput;
        }
    }
}
