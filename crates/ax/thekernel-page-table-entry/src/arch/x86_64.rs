...
impl From<MappingFlags> for PTF {
    fn from(f: MappingFlags) -> Self {
        if !f.intersects(
            MappingFlags::READ | MappingFlags::WRITE | MappingFlags::EXECUTE | MappingFlags::SHADOW_STACK,
        ) {
            // Like Linux's _PAGE_PROTNONE, bit 8 distinguishes an owned,
            // inaccessible leaf from an unused entry even for physical zero.
            // GLOBAL has no hardware meaning while PRESENT is clear.
            return Self::GLOBAL;
        }
...
mod tests {
    ...
    #[test]
    fn shadow_stack_leaf_is_readonly_and_dirty() {
        let entry = X64PTE::new_page(
            PhysAddr::from(0x1234_5000usize),
            MappingFlags::READ | MappingFlags::USER | MappingFlags::SHADOW_STACK,
            false,
        );
        let raw = entry.bits() as u64;
        assert_ne!(raw & PTF::DIRTY.bits(), 0);
        assert_eq!(raw & PTF::WRITABLE.bits(), 0);
    }

    #[test]
    fn readonly_leaf_does_not_gain_shadow_stack_dirty_encoding() {
        let entry = X64PTE::new_page(
            PhysAddr::from(0x1234_5000usize),
            MappingFlags::READ | MappingFlags::USER,
            false,
        );
        assert_eq!(entry.bits() as u64 & PTF::DIRTY.bits(), 0);
    }
}
