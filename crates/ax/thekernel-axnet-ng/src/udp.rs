...
impl UdpSocketFamily {
    /// Returns whether this socket owns IPv4 or IPv6 endpoints.
    pub const fn accepts_socket_addr(self, address: core::net::SocketAddr) -> bool {
        matches!(
            (self, address),
            (Self::Ipv4, core::net::SocketAddr::V4(_)) | (Self::Ipv6, core::net::SocketAddr::V6(_))
        )
    }
...
impl UdpSocket {
    /// Returns the address family selected when this socket was created.
    pub const fn family(&self) -> UdpSocketFamily {
        self.family
    }
...
impl SocketOps for UdpSocket {
    ...
    fn connect(&self, remote_addr: SocketAddrEx) -> AxResult {
        let remote_addr = remote_addr.into_ip().map_err(|_| AxError::InvalidInput)?;
        ...
        let outbound = self
            .stack
            .get_service()
            .resolve_outbound_with_dont_route(&remote_addr.addr, bound_source, dont_route)
    ...
    fn send(&self, mut src: impl Read + IoBuf, options: SendOptions) -> AxResult<usize> {
        ...
        if status.is_continuation() {
            // One bounded pass publishes socket egress into the loopback RX
            // queue; a second bounded pass admits it to the destination
            // socket. Physical-device continuations remain owned by the RX
            // worker after this fixed local replay budget.
            self.stack.poll_interfaces();
        }
        if sent > 0 {
            ...
            .recv_poller_with_effective_nonblocking(self, effective_nonblocking, || {
                ...
                self.with_smol_socket(|socket| {
                    ...
                    match result {
                        ...
                        Err(smol::RecvError::Truncated) => Err(AxError::OutOfRange),
                    }
                })
            })
    }
...
mod tests {
    ...
    fn ipv6_family_owns_unbound_and_autobind_addresses() {
        ...
        assert_eq!(socket.bind(endpoint(0)), Err(AxError::InvalidInput));
