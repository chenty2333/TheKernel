...
pub use self::{dgram::DgramTransport, seqpacket::SeqPacketTransport, stream::StreamTransport};
use crate::{
    CMsgData, RecvOptions, SendOptions, Shutdown, Socket, SocketAddrEx, SocketFilter, SocketOps,
...
impl UnixEndpointIdentity {
    ...
    /// Kernel-side ownership registries use this opaque value only as a
    /// namespace-local key; it is never exposed to userspace.
...
pub enum UnixSocketAddr {
    ...
    Abstract(Arc<Vec<u8>>),
    /// Filesystem pathname address.  Linux pathnames are byte strings, not
    /// UTF-8 text; VFS resolution owns all pathname interpretation.
    Path(Arc<Vec<u8>>),
}

impl UnixSocketAddr {
    /// Builds a pathname address while preserving every non-NUL byte.
    pub fn try_path(bytes: &[u8]) -> AxResult<Self> {
...
pub trait TransportOps: Configurable + Pollable + Send + Sync {
    ...
    fn listen(
        ...
        _slot: &BindSlot,
        _backlog: usize,
        _credentials: SocketCredentials,
    ) -> AxResult {
...
pub enum Transport {
    ...
    /// Connection-oriented record transport.
...
impl Pollable for Transport {
    fn poll(&self) -> IoEvents {
        match self {
            ...
            Transport::SeqPacket(seqpacket) => seqpacket.poll(),
        }
    }
    ...
    fn register<'a>(
        ...
        match self {
            ...
            Transport::SeqPacket(seqpacket) => seqpacket.register(context, events),
        }
    }
}
...
impl Transport {
    fn recv_pending_len(&self) -> AxResult<usize> {
        match self {
            ...
            Self::SeqPacket(seqpacket) => seqpacket.recv_pending_len(),
        }
    }
    ...
    fn retry_transfer<T>(
        ...
        match self {
            ...
            Self::SeqPacket(seqpacket) => {
                seqpacket.retry_transfer(direction, effective_nonblocking, attempt)
            }
        }
    }
    ...
    fn rollback_bind(&self, slot: &BindSlot) {
        match self {
            ...
            Self::SeqPacket(seqpacket) => seqpacket.rollback_bind(slot),
        }
    }
}

/// Holds binding state for the distinct Unix transport kinds at one address.
#[derive(Default)]
pub struct BindSlot {
    stream: Mutex<Option<stream::Bind>>,
    dgram: Mutex<Option<dgram::Bind>>,
    seqpacket: Mutex<Option<seqpacket::Bind>>,
...
impl BindSlot {
    ...
    fn retire(&self) {
        ...
        let seqpacket = self.seqpacket.lock().take();
...
/// Prepared Unix seqpacket connection.  It has the same publication boundary
/// as stream, but commits a record-oriented transport at the accepted end.
pub struct UnixSeqPacketConnectReservation<'a> {
    socket: &'a UnixSocket,
    target: UnixSocketTarget,
    inner: Option<seqpacket::SeqPacketConnectReservation<'a>>,
    committed: bool,
}
...
impl UnixSeqPacketConnectReservation<'_> {
    pub fn listening_identity(&self) -> UnixEndpointIdentity {
        UnixEndpointIdentity::from_raw(
            self.inner
                .as_ref()
                .expect("active Unix seqpacket reservation")
                .listener_identity(),
        )
    }
    pub fn accepted_identity(&self) -> UnixEndpointIdentity {
...
impl Drop for UnixSeqPacketConnectReservation<'_> {
    fn drop(&mut self) {
        if !self.committed {
            self.socket
                .connect_state
...
pub struct UnixAcceptReservation<'a> {
    ...
    inner: Option<UnixAcceptInner>,
    local_address: UnixSocketAddr,
}

enum UnixAcceptInner {
    Stream(stream::StreamAcceptReservation),
    SeqPacket(seqpacket::SeqPacketAcceptReservation),
}
...
impl UnixMaySendReservation<'_> {
    ...
    pub fn cmsg_mut(&mut self) -> &mut [CMsgData] {
        self.inner
            .as_mut()
            .expect("active Unix datagram-send reservation")
            .cmsg_mut()
...
impl UnixAcceptReservation<'_> {
    pub fn accepted_identity(&self) -> UnixEndpointIdentity {
        UnixEndpointIdentity::from_raw(
            match self.inner.as_ref().expect("active Unix accept reservation") {
                UnixAcceptInner::Stream(inner) => inner.accepted_identity(),
                UnixAcceptInner::SeqPacket(inner) => inner.accepted_identity(),
            },
        )
    }
    ...
    pub fn commit(mut self) -> AxResult<Socket> {
        let (transport, peer_addr) =
            match self.inner.take().expect("active Unix accept reservation") {
                UnixAcceptInner::Stream(inner) => inner.commit()?,
                UnixAcceptInner::SeqPacket(inner) => inner.commit()?,
            };
        if let Transport::SeqPacket(seqpacket) = &transport {
            seqpacket.set_record_sender_address(self.local_address.clone());
        }
...
impl UnixBindReservation<'_> {
    pub fn target_endpoint_identity(&self) -> AxResult<UnixEndpointIdentity> {
        self.target.endpoint_identity()
    }
...
impl UnixSocket {
    ...
    pub fn new(transport: impl Into<Transport>, namespace: Arc<UnixNamespace>) -> Self {
        ...
        let connect_state = match &transport {
            ...
            Transport::SeqPacket(seqpacket) if seqpacket.is_connected() => ENDPOINT_BOUND,
            Transport::Stream(_) | Transport::Dgram(_) | Transport::SeqPacket(_) => {
                ENDPOINT_UNBOUND
            }
        };
        Self {
    ...
    fn prepare_seqpacket_connect_target(
        &self,
        target: UnixSocketTarget,
        credentials: SocketCredentials,
    ) -> AxResult<UnixSeqPacketConnectReservation<'_>> {
        if !matches!(&self.transport, Transport::SeqPacket(_)) {
            return Err(AxError::InvalidInput);
        }
        if self
            .connect_state
        ...
        let result = (|| {
            ...
            let local = self.local.lock().address.clone();
            let Transport::SeqPacket(seqpacket) = &self.transport else {
                unreachable!()
            };
            let inner = seqpacket.prepare_connect(target.slot(), &local, credentials)?;
            Ok(UnixSeqPacketConnectReservation {
                socket: self,
                target,
                inner: Some(inner),
                committed: false,
    ...
    fn connect_target(&self, target: UnixSocketTarget, credentials: SocketCredentials) -> AxResult {
        ...
        match &self.transport {
            Transport::Stream(_) => self
                .prepare_stream_connect_target(target, credentials)?
                .commit(),
            Transport::SeqPacket(_) => self
                .prepare_seqpacket_connect_target(target, credentials)?
                .commit(),
            Transport::Dgram(_) => unreachable!(),
        }
    }
    ...
    pub fn connect_resolved(&self, target: UnixSocketTarget) -> AxResult {
        self.connect_resolved_as(target, SocketCredentials::UNKNOWN)
    }
    ...
    /// Prepares a pathname seqpacket handshake across policy dispatch.
    pub fn prepare_seqpacket_connect_resolved_as(
        &self,
        target: UnixSocketTarget,
        credentials: SocketCredentials,
    ) -> AxResult<UnixSeqPacketConnectReservation<'_>> {
        self.prepare_seqpacket_connect_target(target, credentials)
    }
    ...
    pub fn connect_as(
        ...
        let remote_addr = remote_addr.into_unix().map_err(|_| AxError::InvalidInput)?;
        match &remote_addr {
    ...
    /// Prepares an abstract seqpacket handshake across policy dispatch.
    pub fn prepare_seqpacket_connect_as(
        &self,
        remote_addr: SocketAddrEx,
        credentials: SocketCredentials,
    ) -> AxResult<UnixSeqPacketConnectReservation<'_>> {
        ...
        match &remote_addr {
            UnixSocketAddr::Unnamed => Err(AxError::InvalidInput),
            UnixSocketAddr::Path(_) => Err(AxError::OperationNotSupported),
            UnixSocketAddr::Abstract(name) => self
                .prepare_seqpacket_connect_target(self.resolve_abstract_target(name)?, credentials),
        }
    }
    ...
    pub fn prepare_accept(&self) -> AxResult<UnixAcceptReservation<'_>> {
        ...
        let inner = match &self.transport {
            Transport::Stream(stream) => {
                ...
                UnixAcceptInner::Stream(block_on(interruptible(prepared.wait()))???)
            }
            Transport::SeqPacket(seqpacket) => {
                let mut prepared = seqpacket.prepare_accept()?;
                UnixAcceptInner::SeqPacket(block_on(interruptible(prepared.wait()))???)
            }
            Transport::Dgram(_) => return Err(AxError::InvalidInput),
        };
    ...
    pub fn listen_as(&self, backlog: usize, credentials: SocketCredentials) -> AxResult<()> {
        let slot = self
            .local
            .lock()
            .slot
    ...
    pub fn prepare_send_to_resolved(
        ...
        match &self.transport {
            ...
            Transport::Stream(_) | Transport::SeqPacket(_) => Err(AxError::InvalidInput),
        }
    }
    ...
    pub fn set_filter(&self, filter: Option<Arc<dyn SocketFilter>>) -> AxResult<()> {
        match &self.transport {
            ...
            Transport::SeqPacket(seqpacket) => seqpacket.set_filter(filter),
        }
    }
    ...
    pub fn is_connected(&self) -> bool {
        match &self.transport {
            ...
            Transport::SeqPacket(seqpacket) => seqpacket.is_connected(),
        }
    }
    ...
    /// Reports whether receives retain record boundaries and SCM records.
    pub fn is_record_oriented(&self) -> bool {
        matches!(
            &self.transport,
            Transport::Dgram(_) | Transport::SeqPacket(_)
        )
    }

    /// Reports whether this is a connection-oriented record socket.
    pub fn is_seqpacket(&self) -> bool {
        matches!(&self.transport, Transport::SeqPacket(_))
    }
...
impl SocketOps for UnixSocket {
    fn bind(&self, local_addr: SocketAddrEx) -> AxResult {
        let local_addr = local_addr.into_unix().map_err(|_| AxError::InvalidInput)?;
        match &local_addr {
            UnixSocketAddr::Unnamed => Err(AxError::InvalidInput),
            UnixSocketAddr::Path(_) => Err(AxError::OperationNotSupported),
    ...
    fn connect(&self, remote_addr: SocketAddrEx) -> AxResult {
        self.connect_as(remote_addr, SocketCredentials::UNKNOWN)
    }

    fn listen(&self, backlog: usize) -> AxResult {
        self.listen_as(backlog, SocketCredentials::UNKNOWN)
    }
...
mod tests {
    use super::*;
    use crate::{CMsgData, SendFlags, options::SocketCredentials};
    ...
    fn test_path(path: &str) -> UnixSocketAddr {
        UnixSocketAddr::try_path(path.as_bytes()).unwrap()
    }
    ...
    fn resolved_target_uses_the_identity_fixed_at_bind() {
        ...
        assert_eq!(resolved_path.as_slice(), b"/tmp/axnet-canonical-name");
        ...
        assert_eq!(peer_path.as_slice(), b"/tmp/axnet-canonical-name");
    }
    ...
    fn stream_rollback_closes_clients_reached_before_namespace_commit() {
        let credentials = SocketCredentials::new(31, 32, 33);
        let listener = test_socket(StreamTransport::new().unwrap());
        let slot = Arc::new(BindSlot::default());
        let target =
    ...
    fn datagram_reconnect_is_atomic_and_disconnect_is_idempotent() {
        ...
        assert_eq!(peer.as_slice(), b"/tmp/axnet-dgram-peer-b");
    ...
    fn connected_pairs_report_unnamed_peers() {
        let credentials = SocketCredentials::new(41, 42, 43);
        let namespace = UnixNamespace::try_new().unwrap();
