...
// SAFETY: `next` is touched only while the node is uniquely owned or while the global
...
// SAFETY: Every raw node is owned either by one DgramTransport, by this locked list,
...
impl DgramSendReservation<'_> {
    ...
    pub(super) fn commit(self, src: impl Read + IoBuf) -> AxResult<usize> {
        self.transport.send_via_channel(
            src,
...
impl Bind {
    ...
    fn connect(&self) -> AxResult<Channel> {
        ...
        Ok(Channel {
            ...
            peer_credentials: SocketCredentials::UNKNOWN,
            peer_passcred: self.passcred.clone(),
        })
    }
}
...
pub struct DgramTransport {
    ...
    sticky_peer_credentials: Option<SocketCredentials>,
    local_credentials: RwLock<SocketCredentials>,
    passcred: Arc<AtomicBool>,
...
impl DgramTransport {
    ...
    pub fn new() -> AxResult<Self> {
        ...
        Ok(DgramTransport {
            ...
            local_credentials: RwLock::new(SocketCredentials::UNKNOWN),
            passcred: Arc::try_new(AtomicBool::new(false)).map_err(|_| AxError::NoMemory)?,
    ...
    fn new_connected(
        ...
        Ok(DgramTransport {
            ...
            local_credentials: RwLock::new(local_credentials),
            passcred,
            local_addr: RwLock::new(UnixSocketAddr::Unnamed),
            poll_state: PollSet::new(),
            filter,
    ...
    pub fn new_pair(credentials: SocketCredentials) -> AxResult<(Self, Self)> {
        Self::new_pair_with_peer_credentials(credentials, credentials)
    }

    /// Creates a connected pair with the identities captured at each end of a
    /// connection-oriented Unix listener handshake.
    pub(super) fn new_pair_with_peer_credentials(
        ...
        let buffers2 = Arc::try_new(SocketBufferLimits::new(TCP_TX_BUF_LEN, TCP_RX_BUF_LEN))
            .map_err(|_| AxError::NoMemory)?;
        let passcred1 = Arc::try_new(AtomicBool::new(false)).map_err(|_| AxError::NoMemory)?;
        let passcred2 = Arc::try_new(AtomicBool::new(false)).map_err(|_| AxError::NoMemory)?;
        let transport1 = DgramTransport::new_connected(
            ...
            Channel {
                ...
                peer_passcred: passcred2.clone(),
            },
            filter1.clone(),
            general1.clone(),
            buffers1.clone(),
            Some(right_credentials),
            left_credentials,
            passcred1.clone(),
        )?;
        let transport2 = DgramTransport::new_connected(
            ...
            Channel {
                ...
                peer_passcred: passcred1,
            },
            filter2.clone(),
            general2,
            buffers2,
            Some(left_credentials),
            right_credentials,
            passcred2,
        )?;
    ...
    pub(super) fn new_record_pair(
        left: SocketCredentials,
        right: SocketCredentials,
    ) -> AxResult<(Self, Self)> {
        let (mut left, mut right) = Self::new_pair_with_peer_credentials(left, right)?;
        left.record_mode = true;
        right.record_mode = true;
        Ok((left, right))
    }

    /// Updates the address stamped onto subsequently enqueued records.  The
    /// owner holds the Unix endpoint publication state, so this is only used
    /// during connection/accept commit before the transport is exposed.
    pub(super) fn set_record_sender_address(&self, address: UnixSocketAddr) {
        let retired = core::mem::replace(&mut *self.local_addr.write(), address);
        drop(retired);
    }

    pub(super) fn passcred_enabled(&self) -> bool {
    ...
    fn channel_from_slot(slot: &BindSlot) -> AxResult<Channel> {
        ...
        if slot.stream.lock().is_some() || slot.seqpacket.lock().is_some() {
    ...
    pub(super) fn recv_pending_len(&self) -> AxResult<usize> {
        if self.rx_shutdown.load(Ordering::Acquire) && !self.record_mode {
            return Ok(0);
        }
            ...
            O::PeerCredentials(cred) => {
                **cred = self.sticky_peer_credentials.unwrap_or_else(|| {
                    self.connected
                        .read()
                        .as_ref()
                        .map_or(SocketCredentials::UNKNOWN, |channel| {
                            channel.peer_credentials
                        })
                });
            }
...
impl TransportOps for DgramTransport {
    ...
    fn connect(
        ...
        drop(retired);
        *self.local_credentials.write() = credentials;
        self.poll_state.wake();
        Ok(())
    }
    ...
    fn recv(&self, mut dst: impl Write, mut options: RecvOptions) -> AxResult<usize> {
        ...
        if options.flags.contains(RecvFlags::PEEK) {
            let effective_nonblocking = options.effective_nonblocking(self.general.nonblocking());
            return self.general.recv_poller_with_effective_nonblocking(
                self,
                effective_nonblocking,
                move || {
                    let receiver = self
                        ...
                        .cloned()
                        .ok_or(AxError::NotConnected)?;
                    let reservation = match receiver.try_reserve_inner() {
                        Ok(reservation) => reservation,
                        Err(TryRecvError::Empty) => {
                            return if self.record_mode && self.rx_shutdown.load(Ordering::Acquire) {
                                Ok(0)
                            } else {
                    ...
                    let packet = reservation.item();
                    let mut cmsg = Vec::new();
                    cmsg.try_reserve_exact(packet.cmsg.len())
                        .map_err(|_| AxError::NoMemory)?;
                    ...
                    if let Some(output) = options.cmsg.as_mut() {
                        output
                            .try_reserve(cmsg.len())
                            .map_err(|_| AxError::NoMemory)?;
                        output.extend(cmsg);
                    }
                    // Dropping the reservation restores the exact record at
                    // the head after all fallible usercopy/clone work.
    ...
    fn shutdown(&self, how: Shutdown) -> AxResult<()> {
        if how.has_read() && !self.rx_shutdown.swap(true, Ordering::AcqRel) {
            if self.record_mode {
                // Linux keeps queued records readable after SHUT_RD. Reject
                // peer sends without detaching or draining the receive queue.
                let receiver = self.data_rx.lock().queue().map(|(receiver, _, closed)| {
        ...
        if how.has_write() {
            ...
            if self.record_mode {
                let channel = self.connected.read().as_ref().cloned();
                if let Some(channel) = channel {
                    channel.data_tx.close_send();
                }
            }
        }
        self.poll_state.wake();
...
impl Pollable for DgramTransport {
    fn poll(&self) -> IoEvents {
        ...
        if self.record_mode {
            let read_closed = rx_shutdown
                || self
                    .data_rx
                    .lock()
                    .receiver()
                    .is_some_and(Receiver::sender_closed);
            let write_closed = tx_shutdown
                || connected.as_ref().is_some_and(|channel| {
                    channel.peer_closed.load(Ordering::Acquire) || channel.data_tx.is_closed()
                });
            events.set(IoEvents::READ_HANGUP, read_closed);
            events.set(IoEvents::HANGUP, read_closed && write_closed);
            if read_closed {
    ...
    fn register<'a>(
        ...
        let events = if self.record_mode {
            events | IoEvents::READABLE | IoEvents::WRITABLE
        } else {
            events
        };
...
mod tests {
    ...
    fn peer_credentials(transport: &DgramTransport) -> SocketCredentials {
    ...
    fn datagram_socketpair_snapshots_creator_credentials() {
        ...
        let pathname = UnixSocketAddr::Path(Arc::new(Vec::from(
            &b"/home/ava/.cache/thekernel-test-tmp/peercred-reconnect"[..],
        )));
        server.bind(&slot, &pathname).unwrap();
        left.connect(&slot, &UnixSocketAddr::Unnamed, SocketCredentials::UNKNOWN)
            .unwrap();
        assert_eq!(peer_credentials(&left), credentials);
    ...
    fn pathname_datagram_channel_does_not_fake_peer_credentials() {
        ...
        let bind = Bind {
            ...
            passcred: Arc::new(AtomicBool::new(false)),
        };

        assert_eq!(
            bind.connect().unwrap().peer_credentials,
            SocketCredentials::UNKNOWN
        );
    }
