...
/// Transport-neutral result selected when a caller-owned wait budget expires.
#[derive(Clone, Copy)]
enum WaitBudgetOutcome {
    Retry,
    Pending,
}

impl WaitBudgetOutcome {
    const fn error(self) -> AxError {
        match self {
            Self::Retry => AxError::WouldBlock,
            Self::Pending => AxError::InProgress,
        }
    }
}
...
#[derive(Clone, Copy)]
struct PollBehavior {
    budget_outcome: WaitBudgetOutcome,
...
impl GeneralOptions {
    ...
    pub fn set_pending_error(&self, error: SocketFault) {
        self.pending_error.store(error as u8, Ordering::Release);
    }
    ...
    pub fn take_pending_error(&self) -> Option<SocketFault> {
        let code = self.pending_error.swap(0, Ordering::AcqRel);
        SocketFault::from_raw(code)
    }
    ...
    pub fn consume_pending_error(&self) -> AxResult {
        match self.take_pending_error() {
            Some(error) => Err(error.as_ax_error()),
            None => Ok(()),
        }
    }
    ...
    pub fn connect_poller<P: Pollable, F: FnMut() -> AxResult<T>, T>(
        ...
        self.run_poller(
            ...
            self.send_timeout(),
            PollBehavior {
                budget_outcome: WaitBudgetOutcome::Pending,
    ...
    pub fn send_poller_with_effective_nonblocking<P: Pollable, F: FnMut() -> AxResult<T>, T>(
        ...
        // Data transfers consume a deferred transport error before publishing
        // new payload. Connection completion preserves it for the caller that
        // owns error observation.
        self.run_poller(
            pollable,
            IoEvents::WRITABLE,
            self.send_timeout(),
            PollBehavior {
                budget_outcome: WaitBudgetOutcome::Retry,
    ...
    /// A record receive must expose already queued data (and terminal EOF)
    /// before consuming a deferred socket error.  The operation therefore
    /// runs first on every poll iteration; only its `WouldBlock` result can
    /// be replaced by the one-shot pending error.
    pub fn recv_poller_data_first_with_effective_nonblocking<
        P: Pollable,
        F: FnMut() -> AxResult<T>,
        T,
    >(
        ...
        self.run_poller(
            pollable,
            IoEvents::READABLE | IoEvents::HANGUP | IoEvents::READ_HANGUP,
            self.recv_timeout(),
            PollBehavior {
                budget_outcome: WaitBudgetOutcome::Retry,
                effective_nonblocking,
            ...
            || match f() {
                Err(AxError::WouldBlock) => match self.consume_pending_error() {
        ...
        loop {
            ...
            if deadline.is_some_and(|end| axhal::time::wall_time() >= end) {
                return resolve_wait_failure(
                    ...
                    PollWaitFailure::Error(behavior.budget_outcome.error()),
                )
                .0;
            }
            ...
            let failure = if let Some(end) = deadline {
                ...
                match block_on(reservation.race(interruptible(wait))) {
                    ...
                    Ok(Err(_)) => PollWaitFailure::Error(behavior.budget_outcome.error()),
...
mod tests {
    ...
    fn socket_error_is_reported_once() {
        ...
        options.set_pending_error(SocketFault::ConnectionRefused);

        let mut error = None;
        options
            .get_option(GetSocketOption::Error(&mut error))
            .unwrap();
        assert_eq!(error, Some(SocketFault::ConnectionRefused));
    ...
    #[test]
    fn retry_budget_outcome_is_retryable() {
        assert_eq!(WaitBudgetOutcome::Retry.error(), AxError::WouldBlock);
    }

    #[test]
    fn pending_budget_outcome_stays_pending() {
        assert_eq!(WaitBudgetOutcome::Pending.error(), AxError::InProgress);
    }
    ...
    fn pending_error_sets_readiness_and_precedes_send_attempt() {
        ...
        let error = options
            ...
            .unwrap_err();
        assert_eq!(error, AxError::ConnectionRefused);
        assert!(!attempted);
        let events = options.add_pending_error_event(IoEvents::WRITABLE);
        assert!(events.contains(IoEvents::WRITABLE));
        assert!(!events.contains(IoEvents::ERROR));
    ...
    fn transfer_poller_consumes_pending_error_for_both_directions() {
        for direction in [
            ...
            let options = GeneralOptions::new();
            options.set_pending_error(SocketFault::ConnectionReset);
            let mut attempted = false;
            ...
            assert_eq!(error, AxError::ConnectionReset);
            assert!(!attempted);
        }
    }
}
