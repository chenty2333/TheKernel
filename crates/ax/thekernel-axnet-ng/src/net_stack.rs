...
use smoltcp::{
    iface::SocketHandle,
    socket::AnySocket,
    wire::{IpAddress, IpCidr, Ipv4Address, Ipv4Cidr, Ipv6Address, Ipv6Cidr},
};
use spin::Once;
...
pub struct NetStack {
    ...
    addresses: Mutex<Vec<AssignedAddress>>,
    pub(crate) listen_table: Arc<ListenTable>,
    pub(crate) socket_set: Arc<SocketSetWrapper<'static>>,
    pub(crate) service: Mutex<Service>,
    // Route/topology publication is serialized by `service`; retain a cheap
    // generation for users which must make an admission decision before a
    // fallible user copy and then prove that their final enqueue revalidated
    ...
    // SCTP is not a smoltcp transport.  It has one raw-IP consumer per
    // network namespace; endpoints register with this lazily-created router.
    sctp_dispatcher: Once<Arc<crate::sctp::SctpDispatcher>>,
    // DCCP, like SCTP, is carried by a single pair of namespace-owned raw
    // protocol sockets.  Individual DCCP sockets register endpoint queues
    // with this dispatcher; they never compete to consume raw ingress.
    dccp_dispatcher: Once<Arc<crate::dccp::DccpDispatcher>>,
    // TUN queues are file endpoints, not router-owned transient state.  Keep
    // the namespace-local device identity here so a later IFF_MULTI_QUEUE
    // TUNSETIFF can attach another OFD to the same live interface rather
    // than accidentally creating a same-named replacement.
    tun_devices: Mutex<Vec<TunEndpoint>>,
    tap_devices: Mutex<Vec<TapEndpoint>>,
    ipv4_conf_default_tag: AtomicI32,
    ipv4_conf_lo_tag: AtomicI32,
}

/// A retained ownership of the network service serialization domain.
///
/// Packet NOWAIT admission must be a transaction, rather than a sequence of
/// `try_lock` probes.  Retaining the guard lets a caller validate policy,
/// copy its source, and submit without a second service acquisition between
/// those steps.  It deliberately exposes only the two packet operations that
/// are safe while the service is held; publication draining remains an
/// explicit post-release action on [`NetStack`].
pub struct NetStackServicePermit<'a> {
    stack: &'a NetStack,
    service: MutexGuard<'a, Service>,
    nowait: bool,
}

impl NetStackServicePermit<'_> {
    ...
    fn publish_route_generation(&self) {
        self.stack.route_generation.fetch_add(1, Ordering::AcqRel);
    }

    pub fn route_try_add(&mut self, rule: Rule) -> AxResult {
        self.service.router.try_add_rule(rule)?;
        self.publish_route_generation();
        Ok(())
    }
    pub fn route_remove(&mut self, rule: &Rule) -> AxResult {
        self.service.router.remove_rule(rule)?;
        self.publish_route_generation();
        Ok(())
    }
    pub fn route_replace(&mut self, rule: Rule) -> AxResult {
        self.service.router.replace_rule(rule)?;
        self.publish_route_generation();
        Ok(())
    }
    pub fn route_snapshot(&self) -> Vec<RouteInfo> {
        self.service.router.routes()
    }

    pub fn route_add_interface_addr(&mut self, ifindex: u32, addr: IpCidr) -> AxResult {
        let mut registry = if self.nowait {
    ...
    pub fn route_remove_interface_addr(&mut self, ifindex: u32, addr: IpCidr) -> AxResult {
        let mut registry = if self.nowait {
            self.stack.addresses.try_lock().ok_or(AxError::WouldBlock)?
        } else {
            self.stack.addresses.lock()
    ...
    pub fn route_remove_device(&mut self, ifindex: u32) -> AxResult {
        let mut addresses = if self.nowait {
            self.stack.addresses.try_lock().ok_or(AxError::WouldBlock)?
        } else {
            self.stack.addresses.lock()
        };
        // Reserve every fallible side registry before publishing router
        // removal.  A NOWAIT contention must leave the device untouched.
        let mut tun = if self.nowait {
            self.stack
                .tun_devices
                .try_lock()
                .ok_or(AxError::WouldBlock)?
        } else {
            self.stack.tun_devices.lock()
        };
        let mut tap = if self.nowait {
            self.stack
                .tap_devices
                .try_lock()
                .ok_or(AxError::WouldBlock)?
        } else {
            self.stack.tap_devices.lock()
        };
        self.service.router.remove_device(ifindex)?;
        let interfaces = self.service.router.interfaces();
        let removed: Vec<IpCidr> = addresses
            .iter()
            .filter(|entry| {
                !interfaces
                    .iter()
                    .any(|interface| interface.index == entry.ifindex)
            })
        ...
        self.service
            .iface
            .update_ip_addrs(|active| active.retain(|candidate| !removed.contains(candidate)));
        tun.retain(|entry| entry.ifindex != ifindex);
        tap.retain(|entry| entry.ifindex != ifindex);
        self.publish_route_generation();
        Ok(())
    }

    pub fn route_create_veth_pair(
        &mut self,
        name: String,
        peer_name: String,
    ) -> AxResult<(u32, u32)> {
        ...
        if !self.service.router.has_device_capacity() {
            return Err(AxError::ResourceBusy);
        }
        let (left, right) = VethEnd::new_pair(name, peer_name);
        let left_index = self.service.router.try_add_device(Box::new(left))?;
        let right_index = match self.service.router.try_add_device(Box::new(right)) {
            Ok(index) => index,
            Err(error) => {
                let _ = self.service.router.remove_device(left_index);
                return Err(error);
            }
        };
        if let Err(error) = self
            .service
            .router
            .connect_veth_pair(left_index, right_index)
        {
            let _ = self.service.router.remove_device(left_index);
            let _ = self.service.router.remove_device(right_index);
            return Err(error);
        }
    ...
    pub fn send_packet(
        ...
        match self.service.send_packet(interface_index, origin, request) {
    ...
    /// Resolve raw-IP output while retaining the service lock through the
    /// caller's final socket-queue publication.  The returned generation is
    /// meaningful only while this permit is live.
    pub(crate) fn resolve_raw_outbound(
        &self,
        destination: core::net::IpAddr,
        bound_source: Option<core::net::IpAddr>,
        dont_route: bool,
    ) -> AxResult<(core::net::IpAddr, u64, u64)> {
        let destination = match destination {
            core::net::IpAddr::V4(address) => IpAddress::Ipv4(address.into()),
            core::net::IpAddr::V6(address) => IpAddress::Ipv6(address.into()),
        };
        let bound_source = bound_source.map(|address| match address {
            core::net::IpAddr::V4(address) => IpAddress::Ipv4(address.into()),
            core::net::IpAddr::V6(address) => IpAddress::Ipv6(address.into()),
        });
        let route = self
            .service
            .resolve_outbound_with_dont_route(&destination, bound_source, dont_route)
            .map_err(|error| error.as_ax_error())?;
        let source = match route.src_addr {
        ...
        Ok((
            source,
            route.device_mask,
            self.stack.route_generation.load(Ordering::Acquire),
        ))
    }

    /// Reserve the exact route selected for one raw socket queue entry.  The
    /// opaque handle is attached to smoltcp's per-entry metadata before this
    /// service permit is released; no wire header is used as an identity.
    pub(crate) fn reserve_raw_route(
        &mut self,
        destination: core::net::IpAddr,
        bound_source: Option<core::net::IpAddr>,
        dont_route: bool,
        header_included: bool,
        completion: Weak<crate::raw::RawRouteCompletion>,
    ) -> AxResult<(core::net::IpAddr, u64, u64, u32)> {
        ...
        let route = self
            .service
            .resolve_outbound_with_dont_route(&destination_ip, bound_source, dont_route)
            .map_err(|error| error.as_ax_error())?;
        let generation = self.stack.route_generation.load(Ordering::Acquire);
        let handle = self.service.router.reserve_raw_route(RawRoutePlan {
            handle: 0,
            destination: destination_ip,
            source: route.src_addr,
        ...
        Ok((source, route.device_mask, generation, handle))
    }

    pub(crate) fn discard_raw_route(&mut self, handle: u32) {
        self.service.router.discard_raw_route(handle);
    }

    /// Move an already accepted raw-IP socket packet through the interface
    /// egress/router seam before releasing its route admission.  This is
    /// deliberately narrower than `NetStack::poll_interfaces`: broker drains
    /// and readiness publication happen after the permit is dropped, while a
    /// concurrent route mutation cannot interpose between raw queue enqueue
    /// and the service's LOCAL_OUT/POSTROUTING traversal.
    pub(crate) fn publish_raw_egress(&mut self) -> bool {
        let mut sockets = self.stack.socket_set.inner.lock();
        let _ = self.service.poll(&mut sockets);
        self.service.router.take_quarantine_edge()
    }

    /// Claims only records which have already reached the packet broker.
    /// In particular this never calls `poll_interfaces`: an empty NOWAIT
    /// receive must report `WouldBlock` without driving a device service pass.
    pub fn receive_no_poll(
        &mut self,
...
impl NetStack {
    ...
    /// Finish a retained packet-service transaction after all permits have
    /// been dropped.  Draining and device polling must never happen while a
    /// NOWAIT caller owns the service guard.
    pub fn finish_packet_submission(&self, poll: bool) {
    ...
    /// Queue a deferred, nonblocking caller's egress for the namespace's
    /// normal task-context poll owner.  This is a scheduling edge, never a
    /// synchronous `poll_interfaces` call, so MSG_DONTWAIT cannot sleep on
    /// device/service work after it has transferred packet ownership.
    pub(crate) fn schedule_deferred_packet_poll(&self) {
        self.packet_broker.drain_staged();
    ...
    pub(crate) fn publish_quarantine_edge(&self) {
        if let Some(worker) = self.rx_worker.as_ref() {
            worker.state.publish_wake();
        }
        self.poll_source.as_ref().wake();
    ...
    /// Acquire the service domain for an ordinary packet transaction.
    pub fn acquire_packet_service(&self) -> NetStackServicePermit<'_> {
        NetStackServicePermit {
            stack: self,
            service: self.service.lock(),
            nowait: false,
        }
    }

    /// Acquire the service domain without sleeping.  The returned permit is
    /// retained through source copy and submission by callers implementing
    /// MSG_DONTWAIT/RWF_NOWAIT.
    pub fn try_acquire_packet_service(&self) -> AxResult<NetStackServicePermit<'_>> {
        self.service
            .try_lock()
            .map(|service| NetStackServicePermit {
                stack: self,
                service,
                nowait: true,
            })
    ...
    /// Resolve the actual router rule for a raw-IP protocol endpoint.  Unlike
    /// address enumeration, this preserves the rule-selected source and
    /// readiness device mask as one admission decision.
    pub(crate) fn resolve_raw_outbound(
        &self,
        destination: core::net::IpAddr,
        bound_source: Option<core::net::IpAddr>,
        dont_route: bool,
    ) -> AxResult<(core::net::IpAddr, u64)> {
        let permit = self.acquire_packet_service();
        let (source, device_mask, _) =
            permit.resolve_raw_outbound(destination, bound_source, dont_route)?;
        Ok((source, device_mask))
    }
    /// Select a concrete configured source address for a raw protocol peer.
    /// SCTP cannot use an unspecified address once it constructs IP_HDRINCL.
    pub(crate) fn route_source_address(
        &self,
        destination: core::net::IpAddr,
        ...
        self.addresses
            .lock()
            .iter()
            .find_map(|entry| match (destination, entry.cidr) {
                (core::net::IpAddr::V4(_), IpCidr::Ipv4(cidr)) => Some(core::net::IpAddr::V4(
                    core::net::Ipv4Addr::from(cidr.address().octets()),
                )),
                (core::net::IpAddr::V6(_), IpCidr::Ipv6(cidr)) => Some(core::net::IpAddr::V6(
                    core::net::Ipv6Addr::from(cidr.address().octets()),
                )),
                _ => None,
            })
    }
    /// Install or remove this stack's OS-owned packet traversal hook.  Each
    /// `NetStack` is a network namespace, so this cannot leak policy across
    /// namespaces as a process-global callback would.
    pub fn set_packet_hook(&self, hook: Option<crate::router::PacketHook>) {
        self.service.lock().router.set_packet_hook(hook);
    }
    pub fn set_pre_protocol_hook(&self, hook: Option<crate::router::PreProtocolHook>) {
        self.service.lock().router.set_pre_protocol_hook(hook);
    }

    /// Installs the namespace policy's reassembly admission query separately
    /// from the mutating hook.  This lets one netfilter link request IP
    /// defrag without silently changing packets seen by unrelated links.
    pub fn set_packet_defrag_query(&self, query: Option<crate::router::PacketDefragQuery>) {
        self.service.lock().router.set_packet_defrag_query(query);
    }
    pub(crate) fn try_new(
        ...
        for interface in service.router.interfaces() {
            addresses
                .try_reserve(interface.addresses.len())
                .map_err(|_| AxError::NoMemory)?;
            addresses.extend(interface.addresses.into_iter().map(|cidr| AssignedAddress {
        ...
        let stack = Arc::try_new(Self {
            ...
            sctp_dispatcher: Once::new(),
            dccp_dispatcher: Once::new(),
            tun_devices: Mutex::new(Vec::new()),
            tap_devices: Mutex::new(Vec::new()),
    ...
    /// Return the namespace-owned protocol-132 dispatcher.  Keeping this on
    /// `NetStack` is essential: raw sockets are queues, not multicast taps.
    pub(crate) fn sctp_dispatcher(self: &Arc<Self>) -> AxResult<Arc<crate::sctp::SctpDispatcher>> {
        self.sctp_dispatcher
            .try_call_once(|| crate::sctp::SctpDispatcher::try_new(self.clone()))
            .map(Clone::clone)
    }

    /// Return the namespace-owned protocol-33 dispatcher.  Raw IP queues are
    /// destructive consumers, so DCCP endpoint routing must be centralized
    /// here rather than installed once per socket.
    pub(crate) fn dccp_dispatcher(self: &Arc<Self>) -> AxResult<Arc<crate::dccp::DccpDispatcher>> {
        self.dccp_dispatcher
            .try_call_once(|| crate::dccp::DccpDispatcher::try_new(self.clone()))
            .map(Clone::clone)
    }
    ...
    pub fn add_device(&self, device: Box<dyn Device>) -> u32 {
    ...
    pub fn try_add_device(&self, device: Box<dyn Device>) -> AxResult<u32> {
        ...
        let ifindex = service.router.try_add_device(device)?;
        self.publish_route_generation();
        if let Some(worker) = worker {
    ...
    /// Remove a device using its stable namespace-local ifindex.  The service
    /// mutex serializes route lookup, packet dispatch, and topology changes.
    pub fn remove_device(&self, ifindex: u32) -> AxResult {
        // Keep service -> address-registry order everywhere.  Link removal
        // revokes routes in Router and address ownership in one namespace
        // transaction, so a later ifindex cannot inherit stale addresses.
        let mut service = self.service.lock();
        let mut addresses = self.addresses.lock();
        service.router.remove_device(ifindex)?;
        // `remove_device` may atomically remove a veth peer too.  Derive the
        // surviving set from the router after that transaction so neither
        // endpoint can leave an address behind for a recycled ifindex.
        let interfaces = service.router.interfaces();
        let removed: Vec<IpCidr> = addresses
            .iter()
            .filter(|entry| {
                !interfaces
        ...
        service.iface.update_ip_addrs(|active| {
            active.retain(|candidate| !removed.contains(candidate));
        });
        self.tun_devices
            .lock()
            .retain(|entry| entry.ifindex != ifindex);
        self.tap_devices
            .lock()
            .retain(|entry| entry.ifindex != ifindex);
        self.publish_route_generation();
    ...
    /// Create both ends of a namespace-local veth pair.  Both names are
    /// validated and both devices are admitted before the operation reports
    /// success; if the peer admission fails the first end is revoked again.
    pub fn create_veth_pair(&self, name: String, peer_name: String) -> AxResult<(u32, u32)> {
        if name.is_empty()
            || peer_name.is_empty()
            || name == peer_name
            || name.len() > 15
        ...
        let (left, right) = VethEnd::new_pair(name, peer_name);
        let left_index = self.try_add_device(Box::new(left))?;
        match self.try_add_device(Box::new(right)) {
            Ok(right_index) => {
                ...
                if let Err(error) = pair_result {
                    let _ = self.remove_device(left_index);
                    return Err(error);
                }
                Ok((left_index, right_index))
            }
            Err(error) => {
                // `remove_device` also cancels the bridge registration and
                // removes routes/addresses for the unpublished half.
                let _ = self.remove_device(left_index);
                Err(error)
            }
        }
    ...
    /// Create a namespace-local L3 TUN interface and return its file-facing
    /// queue endpoint.  The device is not published until all queue ownership
    /// has been allocated, so a failed `TUNSETIFF` cannot leave a half-link.
    pub fn create_tun(&self, name: String, multi_queue: bool) -> AxResult<(u32, Arc<TunHandle>)> {
        self.tun_devices
            .lock()
            .try_reserve(1)
            .map_err(|_| AxError::NoMemory)?;
        let (device, handle) = TunDevice::new(name.clone())?;
        let ifindex = self.try_add_device(Box::new(device))?;
        self.tun_devices.lock().push(TunEndpoint {
            ifindex,
            name,
            handle: handle.clone(),
            persistent: false,
    ...
    pub fn create_tap(&self, name: String, multi_queue: bool) -> AxResult<(u32, Arc<TapHandle>)> {
        self.tap_devices
            .lock()
            .try_reserve(1)
            .map_err(|_| AxError::NoMemory)?;
        let (device, handle) = TapDevice::new(name.clone())?;
        let ifindex = self.try_add_device(Box::new(device))?;
        self.tap_devices.lock().push(TapEndpoint {
            ifindex,
            name,
            handle: handle.clone(),
            persistent: false,
    ...
    /// Attaches another file queue to a live multiqueue TUN/TAP device.
    /// The handles share the device queues; router ownership and ifindex stay
    /// stable across OFD close/open cycles.
    pub fn attach_tun(&self, name: &str) -> AxResult<(u32, Arc<TunHandle>)> {
        let mut entries = self.tun_devices.lock();
        let entry = entries
            .iter_mut()
        ...
        if !entry.multi_queue {
            return Err(AxError::InvalidInput);
        }
        let handle = entry.handle.attach_queue()?;
        entry.queues = entry.queues.checked_add(1).ok_or(AxError::OutOfRange)?;
        Ok((entry.ifindex, handle))
    }
    pub fn attach_tap(&self, name: &str) -> AxResult<(u32, Arc<TapHandle>)> {
        let mut entries = self.tap_devices.lock();
        let entry = entries
            .iter_mut()
            .find(|entry| entry.name == name)
            .ok_or(AxError::NoSuchDevice)?;
        ...
        // Check the endpoint accounting before publishing the queue: there is
        // no fallible work left after `attach_queue` succeeds.
        let queues = entry.queues.checked_add(1).ok_or(AxError::OutOfRange)?;
        let handle = entry.handle.attach_queue()?;
        entry.queues = queues;
        Ok((entry.ifindex, handle))
    }
    pub fn set_tun_persist(&self, ifindex: u32, tap: bool, persistent: bool) -> AxResult {
        if tap {
            let mut entries = self.tap_devices.lock();
            let entry = entries
                .iter_mut()
                .find(|entry| entry.ifindex == ifindex)
                .ok_or(AxError::NoSuchDevice)?;
    ...
    /// Reopens the sole queue of a persistent device.  This is deliberately
    /// separate from multiqueue attach: a live non-multiqueue interface must
    /// retain Linux's busy admission rather than gain another reader.
    pub fn reopen_persistent_tun(&self, name: &str) -> AxResult<(u32, Arc<TunHandle>)> {
        let mut entries = self.tun_devices.lock();
        let entry = entries
            .iter_mut()
            .find(|entry| entry.name == name)
        ...
        if !entry.persistent || entry.multi_queue || entry.queues != 0 {
            return Err(AxError::ResourceBusy);
        }
        let handle = entry.handle.attach_queue()?;
        entry.queues = 1;
    ...
    pub fn reopen_persistent_tap(&self, name: &str) -> AxResult<(u32, Arc<TapHandle>)> {
        let mut entries = self.tap_devices.lock();
        let entry = entries
            .iter_mut()
            .find(|entry| entry.name == name)
    ...
    pub fn tun_persistent(&self, ifindex: u32, tap: bool) -> AxResult<bool> {
        if tap {
            self.tap_devices
                .lock()
                .iter()
                .find(|entry| entry.ifindex == ifindex)
                .map(|entry| entry.persistent)
                .ok_or(AxError::NoSuchDevice)
        } else {
            self.tun_devices
                .lock()
    ...
    /// Releases one queue attachment and removes a non-persistent device only
    /// after its final queue has closed.  The map lock is dropped before the
    /// router transaction, preserving the stack's router-before-endpoint
    /// ordering.
    pub fn detach_tun_queue(&self, ifindex: u32, tap: bool) -> AxResult {
        let remove = if tap {
            let mut entries = self.tap_devices.lock();
            let entry = entries
                .iter_mut()
                .find(|entry| entry.ifindex == ifindex)
                .ok_or(AxError::NoSuchDevice)?;
            entry.queues = entry.queues.checked_sub(1).ok_or(AxError::BadState)?;
            let remove = entry.queues == 0 && !entry.persistent;
            entry.retiring = remove;
            remove
        } else {
            let mut entries = self.tun_devices.lock();
            let entry = entries
        ...
        if remove {
            self.remove_device(ifindex)
        } else {
            Ok(())
        }
    }
    pub fn detach_tun_queue_handle(&self, ifindex: u32, handle: &TunHandle) -> AxResult {
        handle.detach_queue()?;
        self.detach_tun_queue(ifindex, false)
    }
    /// Releases the particular TAP queue owned by a closing file description.
    /// Unlike the legacy counter-only route this cannot detach a sibling OFD.
    pub fn detach_tap_queue_handle(&self, ifindex: u32, handle: &TapHandle) -> AxResult {
        handle.detach_queue()?;
        self.detach_tun_queue(ifindex, true)
    }

    /// Commit an already validated RTM_SETLINK proposal under the same
    /// service lock used by route lookup and packet dispatch.
    pub fn configure_link(
        &self,
        ifindex: u32,
        name: Option<String>,
        ...
        {
            let mut service = self.service.lock();
            service.router.configure_link(ifindex, name, mtu, up)?;
        }
        self.publish_route_generation();
        if up == Some(true) {
            // Re-arm after publication.  A peer may have queued ingress while
            // the link was down; replay both worker and ordinary readiness.
            if let Some(worker) = self.rx_worker.as_ref() {
                self.arm_rx_worker(&worker.irq_waker)
                    .map_err(crate::general::poll_registration_error)?;
                worker.state.publish_wake();
            }
            self.poll_source.as_ref().wake();
        }
    ...
    /// One packet send which must not wait for the network service mutex.
    /// Callers use this for operation-local MSG_DONTWAIT/RWF_NOWAIT; it does
    /// not change any socket's persistent nonblocking setting.
    pub fn send_packet_nowait(
        &self,
        interface_index: u32,
        origin: &PacketEndpoint,
        request: PacketSendRequest<'_>,
    ...
    /// Sends a frame produced by a kernel facility without an AF_PACKET
    /// endpoint identity.  This is deliberately a separate typed entrypoint:
    /// callers cannot invent an endpoint merely to reach a device queue.
    pub fn send_packet_unattributed(
        &self,
        interface_index: u32,
        request: PacketSendRequest<'_>,
    ) -> AxResult<()> {
        let result = self
            .service
            .lock()
            .send_packet_unattributed(interface_index, request);
        self.packet_broker.drain_staged();
        self.poll_interfaces();
        match result {
            Ok(PacketSendProgress::NoImmediateIngress)
    ...
    /// Add a namespace-local forwarding rule after validating its device and
    /// address-family relationship.  The service mutex is the publication
    /// boundary shared with route lookup and egress.
    pub fn try_add_route(&self, rule: Rule) -> AxResult {
        self.service.lock().router.try_add_rule(rule)?;
        self.publish_route_generation();
        Ok(())
    }

    /// Delete one exact namespace-local forwarding rule.
    pub fn remove_route(&self, rule: &Rule) -> AxResult {
        self.service.lock().router.remove_rule(rule)?;
        self.publish_route_generation();
        Ok(())
    }

    /// Replace one namespace-local forwarding rule with the same destination
    /// prefix.
    pub fn replace_route(&self, rule: Rule) -> AxResult {
        self.service.lock().router.replace_rule(rule)?;
        self.publish_route_generation();
        Ok(())
    }
...
mod tests {
    ...
    impl Device for ProbeDevice {
        ...
        fn recv(
            ...
            _buffer: &mut crate::device::IngressPacketBuffer,
            _timestamp: Instant,
        ) -> RxStep {
    ...
    fn physical_stack_with_probe(
        ...
        let mut router = Router::try_new_loopback_only(listen_table.clone()).unwrap();
        let loopback = router
    ...
    fn adding_device_rechecks_arrival_during_worker_arm() {
        ...
        // One call consumes the synthetic arrival and the next proves that
        // the source is idle before the bounded pass can report quiescence.
        assert_eq!(receives.load(Ordering::Acquire), 2);
        assert!(!backlog.load(Ordering::Acquire));
    }
