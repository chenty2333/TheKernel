...
impl<Hal: SystemHal> InodeRef<Hal> {
    ...
    pub fn lookup(mut self, name: &[u8]) -> Ext4Result<DirLookupResult<Hal>> {
        unsafe {
    ...
    pub(crate) fn add_entry(&mut self, name: &[u8], entry: &mut InodeRef<Hal>) -> Ext4Result {
    ...
    fn remove_entry_without_link_count(&mut self, name: &[u8]) -> Ext4Result {
        unsafe {
            let mut metadata_may_have_changed = false;
    ...
    pub(crate) fn remove_entry(&mut self, name: &[u8], entry: &mut InodeRef<Hal>) -> Ext4Result {
    ...
    pub(crate) fn remove_transferred_entry(&mut self, name: &[u8]) -> Ext4Result {
        self.remove_entry_without_link_count(name)
    }
}
