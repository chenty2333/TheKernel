...
pub(super) fn init() {
    IDT.call_once(|| {
        ...
        for i in 0..NUM_INT {
            ...
            if i == 2 {
                // A PMI delivered as APIC NMI must not borrow an arbitrary
                // interrupted task stack. The per-CPU GDT installs this IST
                // before the IDT is loaded on every CPU.
                // SAFETY: `NMI_IST_INDEX` is a valid TSS IST slot and
                // `gdt::init` populates it before this IDT is loaded.
                unsafe { opt.set_stack_index(super::gdt::NMI_IST_INDEX) };
            }
        }
