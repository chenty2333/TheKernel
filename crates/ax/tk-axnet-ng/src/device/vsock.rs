...
fn abort_overflowed_connection(
    ...
    if let Err(error) = dev.abort(conn_id) {
        ratelimit::warn_ratelimited!("failed to abort overflowed vsock connection: {error:?}");
    }
    match manager.on_disconnected(conn_id) {
        ...
        Err(error) => ratelimit::warn_ratelimited!("failed to publish overflow disconnect: {error:?}"),
    }
}
    ...
    match event {
        VsockDriverEvent::ConnectionRequest(conn_id) => {
            ...
            match manager.on_connection_request(conn_id) {
                ...
                Err(e) => {
                    debug!("Connection request failed: {conn_id:?}, error={e:?}");
                    ...
                    drop(manager);
                    if let Err(error) = dev.abort(conn_id) {
                        ratelimit::warn_ratelimited!("failed to abort rejected vsock connection: {error:?}");
                    }
                }
            }
            outcome
        ...
        VsockDriverEvent::Received(conn_id, len) => {
            let free_space = if let Some(conn) = manager.get_connection(conn_id) {
                ...
                debug!("Received data for unknown connection: {conn_id:?}");
            ...
            if free_space == 0 {
                if PENDING_EVENTS
                    ...
                    .is_err()
                {
                    ratelimit::warn_ratelimited!("bounded vsock deferred-event queue is full; aborting {conn_id:?}");
                    let mut outcome = EventOutcome::progress();
            ...
            match dev.recv(conn_id, &mut buf[..max_read]) {
                ...
                Err(e) => {
                    debug!("Failed to receive vsock data: conn_id={conn_id:?}, error={e:?}",);
        ...
        VsockDriverEvent::Disconnected(conn_id) => {
            ...
            match manager.on_disconnected(conn_id) {
                ...
                Err(e) => debug!("Failed to handle disconnection: {conn_id:?}, error={e:?}",),
            }
            outcome
        }
        ...
        VsockDriverEvent::Connected(conn_id) => {
            ...
            match manager.on_connected(conn_id) {
                ...
                Err(e) => {
                    debug!(
                "Failed to handle connection established: {conn_id:?}, error={e:?}",
            )
                }
            }
            outcome
        ...
        VsockDriverEvent::CreditUpdate(conn_id) => {
            ...
            match manager.on_credit_update(conn_id) {
                ...
                Err(e) => debug!("Failed to handle credit update: {conn_id:?}, error={e:?}"),
            }
            outcome
        }

        VsockDriverEvent::Unknown => {
            debug!("Received unknown vsock event");
            EventOutcome::progress()
        }
    }
}
