...
/// Checksum work that is owned by the device at a packet-policy seam.  This
/// is intentionally distinct from a packet checksum value: it tells policy
/// whether a header rewrite must be repaired in software before device send.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// Per-L3/L4 checksum ownership advertised by a concrete link device.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PacketChecksumContext {
    pub ipv4: PacketChecksum,
    pub udp: PacketChecksum,
    pub tcp: PacketChecksum,
}
impl PacketChecksumContext {
    pub const SOFTWARE: Self = Self {
        ipv4: PacketChecksum::Software,
        udp: PacketChecksum::Software,
        tcp: PacketChecksum::Software,
    };
    pub const UNKNOWN: Self = Self {
        ipv4: PacketChecksum::Unknown,
        udp: PacketChecksum::Unknown,
        tcp: PacketChecksum::Unknown,
    };
}
...
pub struct PacketDeviceCapabilities {
    ...
    pub address_len: u8,
    /// Checksum work retained or offloaded by this exact link device.
...
impl PacketDeviceCapabilities {
    ...
    pub const fn unsupported(hardware_type: LinkHardwareType) -> Self {
        Self {
            ...
            link_header_len: 0,
            address_len: 0,
            checksum: PacketChecksumContext::UNKNOWN,
...
/// One endpoint-owned packet-ring writer.
///
/// The generic broker owns registration and teardown of this callback; an ABI
/// layer owns the mapped memory behind it.  This keeps DMA/capture delivery
/// lock-free with respect to a process address space while allowing AF_PACKET
/// rings to be filled at ingress rather than lazily during `read(2)`.
pub trait PacketRingSink: Send + Sync {
    /// Publish one already-filtered visible frame. `Ok(true)` consumes it;
    /// `Ok(false)` reports a full userspace-owned ring and accounts a drop.
    fn publish(&self, metadata: PacketMetadata, bytes: &[u8]) -> AxResult<bool>;

    /// Whether at least one completed frame is currently owned by userspace.
    /// This feeds AF_PACKET poll readiness; ordinary queues stay empty while a
    /// ring sink is installed, so queue state alone is insufficient.
...
pub struct PacketEndpoint {
    ...
    readiness: PollSet,
    detached: AtomicBool,
    ring: Mutex<Option<Arc<dyn PacketRingSink>>>,
    fanout: Mutex<Option<PacketFanout>>,
}

/// AF_PACKET fanout policy.  The group is namespace-local because its broker
/// is namespace-local; selection happens after protocol/interface filtering,
/// so a member which cannot observe a frame is never selected for it.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub struct PacketFanout {
    ...
    /// Linux encodes fanout flags beside the low-byte policy. They are part
    /// of group identity: mixing them would make one group observe two
    /// delivery contracts.
    pub flags: u16,
}

impl PacketEndpoint {
    /// Joins one broker-local fanout group.  Linux requires all members to
    /// have the same capture identity; reject a mismatched group rather than
    /// silently turning it into an unrelated multicast subscription.
    pub fn set_fanout(&self, fanout: Option<PacketFanout>) -> PacketResult<()> {
        ...
        if let Some(fanout) = fanout {
            ...
            for (id, member) in &state.endpoints {
                if *id == self.id {
                    continue;
                }
                ...
                if let Some(existing) = *member.fanout.lock() {
                    if existing.group == fanout.group
                        && (member.selector() != selector
                            || existing.mode != fanout.mode
                            || existing.flags != fanout.flags)
                    {
    ...
    fn fanout(&self) -> Option<PacketFanout> {
        *self.fanout.lock()
    }
    /// Installs or removes the exclusive mapped-ring delivery target.  The
    /// broker keeps the target alive for exactly the endpoint lifetime; the
    /// caller must first drain/disable ordinary receives before publishing it.
    pub fn set_ring_sink(&self, sink: Option<Arc<dyn PacketRingSink>>) -> PacketResult<()> {
    ...
    fn enqueue(
        ...
        let ring = self.ring.lock().as_ref().cloned();
        if let Some(ring) = ring {
            ...
            match ring.publish(metadata, &visible[..captured_len]) {
                Ok(true) => {
                    drop(state);
                    self.readiness.wake();
                }
    ...
    pub(crate) fn arm_readiness<'a>(
        ...
        if self.is_detached()
            ...
            || self
                ...
                .lock()
                .as_ref()
                .is_some_and(|ring| ring.readable())
        {
            waker.wake_by_ref();
        }
        Ok(())
...
impl PacketBroker {
    ...
    fn deliver(&self, pending: PendingPacket) {
        let mut targets: [Option<(Arc<PacketEndpoint>, PacketSelector, Option<PacketFanout>)>;
            MAX_PACKET_ENDPOINTS] = core::array::from_fn(|_| None);
        let mut target_count = 0usize;
        {
            let state = self.state.lock();
            for (id, endpoint) in &state.endpoints {
                ...
                if let Some(selector) = endpoint.selector_for(pending.metadata, pending.sequence) {
                    let fanout = endpoint.fanout();
                    targets[target_count] = Some((endpoint, selector, fanout));
                    target_count += 1;
                }
            }
        }
        // Ordinary listeners receive a copy.  Each fanout group receives at
        // most one copy: choose from the already-filtered members, preserving
        // a stable packet hash and a strict round-robin fallback for policies
        // which deliberately do not expose a flow hash.
        for index in 0..target_count {
            let Some((endpoint, selector, fanout)) = targets[index].as_ref() else {
                continue;
            };
            if let Some(fanout) = fanout {
                let mut first = index;
                let mut members = 0usize;
                for candidate in 0..target_count {
                    if targets[candidate]
                        .as_ref()
                        .and_then(|entry| entry.2)
                        .is_some_and(|candidate| candidate.group == fanout.group)
                    {
                        first = first.min(candidate);
                        members += 1;
                    }
                }
                let seed = match fanout.mode {
                    PacketFanoutMode::Hash => Self::packet_fanout_hash(&pending.frame.bytes),
                    PacketFanoutMode::LoadBalance | PacketFanoutMode::Rollover => pending.sequence,
                    PacketFanoutMode::Cpu => pending.metadata.interface_index as u64,
                    PacketFanoutMode::Random => {
                        pending.sequence.rotate_left(17)
                            ^ Self::packet_fanout_hash(&pending.frame.bytes)
                    }
                };
                let mut selected = first;
                let mut rank = (seed as usize) % members;
                for candidate in 0..target_count {
                    if targets[candidate]
                        .as_ref()
                        .and_then(|entry| entry.2)
    ...
    fn packet_fanout_hash(bytes: &[u8]) -> u64 {
        // FNV-1a is deliberately deterministic across tasks and namespaces.  It
        // is not a security boundary: the packet broker never lets an untrusted
        // hash influence allocation or bounds, only the selected already-live
        // member of a fixed-size group.
