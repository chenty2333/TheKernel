...
/// One mapped virtual leaf and its physical address, permissions, and size.
pub type MappedLeaf<V> = (V, PhysAddr, MappingFlags, PageSize);
...
impl<M: PagingMetaData, PTE: GenericPTE, H: PagingHandler> PageTable64<M, PTE, H> {
    ...
    ///
    /// # Safety
    /// The root and all referenced page-table frames must remain valid and
    /// accessible through the paging handler for the lifetime of this table.
    /// The caller must synchronize access with other users of the same root.
    ...
    /// Queries an owned mapping even when PROT_NONE denies CPU access.
    /// Unlike `query`, this distinguishes a retained physical leaf from a hole.
    pub fn query_mapped(&self, vaddr: M::VirtAddr) -> PagingResult<(PhysAddr, MappingFlags, PageSize)> {
        let (entry, size) = self.get_entry(vaddr)?;
        if entry.is_unused() {
            return Err(PagingError::NotMapped);
        }
        Ok((entry.paddr().add(size.align_offset(vaddr.into())), entry.flags(), size))
    }
    ...
    /// Counts bytes backed by owned leaves, including PROT_NONE, without allocating.
    ...
    fn mapped_bytes_recursive(
        ...
        let first = (range_start - table_start) >> shift;
        let last = (range_end - 1 - table_start) >> shift;
        let mut bytes = 0;
        for (index, entry) in table.iter().enumerate().take(last + 1).skip(first) {
            if entry.is_unused() {
                continue;
            }
            let entry_start = table_start + (index << shift);
            let begin = range_start.max(entry_start);
            let end = range_end.min(entry_start + span);
            if level == M::LEVELS - 1 || entry.is_huge() {
                bytes += end - begin;
            } else {
                // A missing table pointer represents a hole, matching query_mapped.
                match self.next_table(entry) {
                    Ok(child) => bytes += self.mapped_bytes_recursive(
                        child, level + 1, entry_start, begin, end,
                    )?,
    ...
    /// Collects owned leaf mappings, including inaccessible PROT_NONE leaves.
    /// CPU accessibility remains the separate contract of [`Self::query`].
    ///
    ...
    pub fn collect_mapped_leaves(
        ...
        self.collect_mapped_leaves_recursive(
            self.table_of(self.root_paddr()),
            0,
            0,
            start_usize,
    ...
    /// Collects every owned leaf intersecting the range, retaining each leaf's
    /// actual base and size. Callers mutating a partial leaf must demote it and
    /// restrict their changes to the original range.
    pub fn collect_overlapping_mapped_leaves(
        &self,
        start: M::VirtAddr,
        size: usize,
    ) -> PagingResult<Vec<MappedLeaf<M::VirtAddr>>> {
        ...
        let expanded_begin = match self.query_mapped(start) {
            Ok((_, _, page_size)) => begin & !(page_size as usize - 1),
            Err(PagingError::NotMapped) => begin,
            Err(error) => return Err(error),
        };
        let expanded_end = match self.query_mapped((end - 1).into()) {
            Ok((_, _, page_size)) => ((end - 1) | (page_size as usize - 1))
                .checked_add(1).ok_or(PagingError::NotAligned)?,
            Err(PagingError::NotMapped) => end,
            Err(error) => return Err(error),
        };
        self.collect_mapped_leaves(expanded_begin.into(), expanded_end - expanded_begin)
    }
...
impl<M: PagingMetaData, PTE: GenericPTE, H: PagingHandler> PageTable64<M, PTE, H> {
    ...
    /// Splits only the boundary children needed to make a range leaf-aligned.
    /// All validation and frame reservation checks precede publication.
    fn demote_leaf_for_range_prepared(
        &mut self,
        start: M::VirtAddr,
        size: usize,
        prepared: &mut PreparedPageTableFrames<H>,
        ...
        let begin: usize = start.into();
        let end = begin.checked_add(size).ok_or(PagingError::NotAligned)?;
        if size == 0 || !PageSize::Size4K.is_aligned(begin)
            || !PageSize::Size4K.is_aligned(size) {
            return Err(PagingError::NotAligned);
        }
        let (entry, original) = self.get_entry_mut(start)?;
        if entry.is_unused() { return Err(PagingError::NotMapped); }
        let base = begin & !(original as usize - 1);
        if end > base + original as usize { return Err(PagingError::NotAligned); }
        if begin == base && size == original as usize { return Ok(original); }
        if original == PageSize::Size2M {
            return self.demote_leaf_to_4k_prepared(start, prepared);
        }
        if original != PageSize::Size1G { return Err(PagingError::NotAligned); }
        let paddr = entry.paddr();
        let flags = entry.flags();
        let chunk = PageSize::Size2M as usize;
        let first = (begin - base) / chunk;
        let last = (end - 1 - base) / chunk;
        let partial = |index: usize| begin > base + index * chunk
            || end < base + (index + 1) * chunk;
        let p1_count = (first..=last).filter(|&index| partial(index)).count();
        if prepared.frames.len() < 1 + p1_count { return Err(PagingError::NoMemory); }
        let p2_frame = prepared.frames.pop().expect("preflighted P2 frame");
        // Build the entire replacement subtree while the original leaf is live.
        for index in 0..512 {
            let child = if index >= first && index <= last && partial(index) {
                let p1_frame = prepared.frames.pop().expect("preflighted P1 frame");
                for (offset, child) in self.table_of_mut(p1_frame).iter_mut().enumerate() {
                    *child = PTE::new_page(paddr.add(index * chunk + offset * 4096), flags, false);
                }
                PTE::new_table(p1_frame)
            } else {
                PTE::new_page(paddr.add(index * chunk), flags, true)
            };
            self.table_of_mut(p2_frame)[index] = child;
        }
        *self.get_entry_mut(start).expect("unchanged preflighted leaf").0 = PTE::new_table(p2_frame);
        Ok(original)
    }
    ...
    fn collect_mapped_leaves_recursive(
        &self,
        table: &[PTE],
        level: usize,
        table_start: usize,
    ...
    fn validate_and_count_mapped_leaves_recursive(
        &self,
        table: &[PTE],
        level: usize,
        table_start: usize,
        ...
        for (index, entry) in table.iter().enumerate() {
            ...
            let child = self.next_table(entry)?;
            let child_count = self.validate_and_count_mapped_leaves_recursive(
                child,
                level + 1,
                entry_start,
                range_start,
...
impl<'a, M: PagingMetaData, PTE: GenericPTE, H: PagingHandler> PageTable64Cursor<'a, M, PTE, H> {
    ...
    fn drain_mapped_leaves_recursive(
        &mut self,
    ...
    ///
    /// Owned source leaves need not be physically contiguous; all mapped
    ...
    pub fn replace_2m_huge_leaf_with_pte_run(
        ...
        if original.is_unused() || !original.is_huge() || original.flags() != flags {
            return Err(PagingError::NotPromotable);
        }
    ...
    /// Returns the protection key of an owned leaf, including PROT_NONE.
    ///
    ...
    /// Changes the protection key of an owned leaf, including PROT_NONE.
    ///
    ...
    /// Demotes partial huge-page boundaries, preserving fully covered 2 MiB
    /// children and all leaves outside the requested range.
    pub fn demote_leaf_for_range_prepared(
        &mut self,
        start: M::VirtAddr,
        size: usize,
        prepared: &mut PreparedPageTableFrames<H>,
    ) -> PagingResult<PageSize> {
        self.inner.demote_leaf_for_range_prepared(start, size, prepared)
    }

    /// Changes the protection key of every owned leaf fully covered by the
    /// 4 KiB-aligned range.
    ///
    ...
    /// Removes and returns all owned leaf mappings fully contained in the
    /// given range.
    ///
    /// This is a destructive counterpart to `collect_mapped_leaves()`: it
    /// first validates that the requested range does not partially overlap any
    /// owned leaf, then clears matching leaves in place and returns them.
    pub fn drain_mapped_leaves(
        &mut self,
        start: M::VirtAddr,
        size: usize,
    ) -> PagingResult<Vec<MappedLeaf<M::VirtAddr>>> {
        ...
        let leaf_count = {
            ...
            self.inner.validate_and_count_mapped_leaves_recursive(
                root,
                0,
                0,
                start_usize,
        ...
        leaves
            .try_reserve_exact(leaf_count)
            .map_err(|_| PagingError::NoMemory)?;
        self.drain_mapped_leaves_into(start, size, &mut leaves)?;
        Ok(leaves)
    }

    /// Drains owned leaves into caller-reserved storage without allocating.
    ///
    /// This is the commit half of a prepared PTE retirement.  It validates the
    /// same complete-leaf geometry as [`Self::drain_mapped_leaves`] before
    /// touching the tree, then requires `leaves` to have enough capacity for
    /// every detached leaf.  Callers can therefore reserve their rollback
    /// journal before a MAP_FIXED-style transaction withdraws its first PTE.
    pub fn drain_mapped_leaves_into(
        &mut self,
        start: M::VirtAddr,
        size: usize,
        ...
        leaves.clear();
        if leaves.capacity() < leaf_count {
            return Err(PagingError::NoMemory);
        }
        if let Err(error) = self.drain_mapped_leaves_recursive(
    ...
    pub fn protect_region(
        ...
        while size > 0 {
            ...
            let page_size = match self.inner.get_entry_mut(vaddr) {
                Ok((entry, page_size)) => {
                    // PROT_NONE retains the physical leaf while clearing
                    // PRESENT. Restore that owned leaf as well; only a truly
                    // unused slot is a demand-mapping hole.
                    if !entry.is_unused() {
                        entry.set_flags(flags, page_size.is_huge());
                        self.push(vaddr);
                    }
                    // Leave genuinely unmapped slots untouched.
