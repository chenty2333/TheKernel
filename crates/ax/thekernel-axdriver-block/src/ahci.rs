...
impl<H: AhciHal> BlockDriverOps for AhciDriver<H> {
    ...
    fn block_capabilities(&self) -> BlockCapabilities {
        // `simple_ahci` currently exposes synchronous read/write and its
        // controller completion boundary only. Do not claim NCQ FUA, DSM
        // discard, or WRITE ZEROES until the lower implementation carries the
        // corresponding command and completion status.
        BlockCapabilities {
            flush: true,
