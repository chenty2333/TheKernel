...
use axdriver_block::{
    ...
    BlockCompletionNotifier, BlockCompletionOwner, BlockDriverOps, BlockPhysicalRequest,
...
impl<H: Hal, T: Transport + Send> BlockDriverOps for VirtIoBlkDev<H, T> {
    ...
    fn block_capabilities(&self) -> BlockCapabilities {
        BlockCapabilities {
            flush: self.inner.lock().supports_flush(),
            fua: false,
            fence: true,
            discard: false,
            write_zeroes: false,
    ...
    unsafe fn submit_physical_batch(
        ...
        let report = {
            ...
            let prepared = unsafe { inner.prepare_physical_batch_no_drain(pending.as_mut_slice()) }
