...
CASE=signal-mask-alias
WORKDIR="$REPO_ROOT/.state/ci/signal-mask-alias-host-differential"
SOURCE_REL=tests/guest/tools/signal-mask-alias.c
MANIFEST_REL=scripts/ci/differential/manifests/signal-mask-alias.markers
ALLOWLIST="$SCRIPT_DIR/differential/allowlist/signal-mask-alias.json"

usage() {
    cat <<'EOF'
Usage: scripts/ci/signal-mask-alias-host-differential.sh [OPTIONS]

Options:
  --workdir DIR                Artifact directory

Builds and runs the portable rt_sigprocmask aliasing smoke test against the
host Linux kernel and records hashes of the working-tree inputs.
EOF
}
    ...
    case "$1" in
        --workdir)
            WORKDIR=$(differential_workdir_value \
                signal-mask-alias-host-differential "$@") || exit $?
            shift 2
            ;;
        -h|--help) usage; exit 0 ;;
        *)
            printf 'signal-mask-alias-host-differential: unknown argument: %s\n' \
                "$1" >&2
            exit 2
            ;;
    esac
...
BINARY="$WORKDIR/signal-mask-alias"
LOG="$WORKDIR/signal-mask-alias.log"
...
command -v timeout >/dev/null 2>&1 || {
    printf '%s\n' \
        'signal-mask-alias-host-differential: timeout command is required' >&2
    exit 1
}
...
fail_run() {
    emit_receipt fail
    printf 'signal-mask-alias-host-differential: FAIL %s\n' "$1" \
        | tee "$RESULT" >&2
    exit 1
}
...
if grep -Fq 'CI_SIGNAL_MASK_ALIAS_FAIL' "$LOG"; then
    fail_run 'smoke_reported_failure=1'
fi
if [ "$(tail -n 1 "$LOG")" != 'CI_SIGNAL_MASK_ALIAS_PASS' ]; then
    fail_run 'final_marker_not_last=1'
fi

emit_receipt pass
printf 'signal-mask-alias-host-differential: PASS markers_expected=%s evidence=%s\n' \
