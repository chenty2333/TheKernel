...
ordering, legacy signal FP frames, rseq, vfork/pause/alarm wait semantics,
ioprio and membarrier, raw io_uring, userfaultfd, AF_PACKET, and seccomp ABI
checks. QEMU
...
for marker in \
    ...
    CI_WAIT_BOUNDARY_X86_FUTEX2_WAITV_BOUNDS_OK \
    CI_WAIT_BOUNDARY_X86_FUTEX2_SHARED_REQUEUE_OK\ same_backing=1\ different_backing=1 \
    CI_WAIT_BOUNDARY_X86_FUTEX2_MIXED_WAITV_OK\ private_index=0\ shared_index=1 \
    ...
    THEKERNEL_IO_URING_BUFFERS_ERRORS_OK \
    THEKERNEL_IO_URING_BUFFERS_REGISTRATION_OK \
    THEKERNEL_IO_URING_BUFFERS_COPY_OK \
    THEKERNEL_IO_URING_BUFFERS_BUDGET_OK \
    THEKERNEL_IO_URING_BUFFERS_GENERATION_OK \
    THEKERNEL_IO_URING_BUFFERS_INFLIGHT_OK \
    THEKERNEL_IO_URING_BUFFERS_CLOSE_OK \
    THEKERNEL_IO_URING_BUFFERS_EXEC_OK \
    THEKERNEL_IO_URING_BUFFERS_MUNMAP_OK \
    THEKERNEL_IO_URING_BUFFERS_STRESS_OK\ rounds=50 \
    THEKERNEL_IO_URING_BUFFERS_OK \
    THEKERNEL_SYSTEM_TEST_IO_URING_BUFFERS_OK \
