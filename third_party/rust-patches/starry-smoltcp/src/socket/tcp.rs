...
impl<'a> Socket<'a> {
    ...
    fn reset(&mut self) {
        ...
        #[cfg(feature = "async")]
        {
            // Our kernel-side socket wrappers do not use smoltcp's internal
            // async socket wakers. Clearing them here avoids re-entering a
            // stale wake vtable while a socket is being reset.
            self.rx_waker.clear();
            self.tx_waker.clear();
        }
    }
