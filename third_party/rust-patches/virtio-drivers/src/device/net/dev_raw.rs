...
mod tests {
    ...
    fn reset_reclaims_in_flight_receive_and_transmit_owners() {
        ...
        // buffer is intentionally dropped only after reset has cleared the
        // transport's queue pointers.
        ...
        assert_eq!(state.queues[QUEUE_RECEIVE as usize].descriptors, 0);
        assert_eq!(state.queues[QUEUE_TRANSMIT as usize].descriptors, 0);
        assert_eq!(
