...
impl<IO: Read + Write + Seek, TP, OCC> FileSystem<IO, TP, OCC> {
    ...
    /// Flushes filesystem metadata and buffered storage writes without unmounting.
    ///
    /// Unlike [`FileSystem::unmount`], this keeps the volume's dirty flag set while
    /// the filesystem remains mounted. The caller is responsible for issuing any
    /// device-level durability fence required by the backing storage.
