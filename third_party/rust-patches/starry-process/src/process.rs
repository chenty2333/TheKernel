...
/// Builder
impl Process {
    fn new(pid: Pid, parent: Option<Arc<Process>>, exit_signal: Option<u8>) -> Arc<Process> {
    ...
    pub fn new_init(pid: Pid, exit_signal: Option<u8>) -> Arc<Process> {
        Self::new(pid, None, exit_signal)
    }

    /// Creates a child [`Process`].
    pub fn fork(self: &Arc<Process>, pid: Pid, exit_signal: Option<u8>) -> Arc<Process> {
        Self::new(pid, Some(self.clone()), exit_signal)
    }
}
