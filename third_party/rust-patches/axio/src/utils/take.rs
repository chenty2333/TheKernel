...
impl<T: Seek> Seek for Take<T> {
    fn seek(&mut self, pos: SeekFrom) -> Result<u64> {
        ...
        while new_position != self.position() {
            if let Some(offset) = checked_signed_diff(new_position, self.position()) {
...
#[inline]
fn checked_signed_diff(lhs: u64, rhs: u64) -> Option<i64> {
    if lhs >= rhs {
        i64::try_from(lhs - rhs).ok()
    } else {
        i64::try_from(rhs - lhs).ok()?.checked_neg()
    }
}
