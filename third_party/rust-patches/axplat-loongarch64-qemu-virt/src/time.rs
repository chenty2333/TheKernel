...
impl TimeIf for TimeIfImpl {
    ...
    fn set_oneshot_timer(deadline_ns: u64) {
        ...
        // QEMU/LoongArch can lose an interrupt when the one-shot timer is
        // reprogrammed with an already-expired or extremely small delta. Keep
        // the deadline close, but give the virtual timer enough room to arm.
        let min_delta_ticks = Self::nanos_to_ticks(100_000).max(4);
        let init_value = ticks_deadline
            .saturating_sub(ticks_now)
            .max(min_delta_ticks);
        let init_value = init_value.saturating_add(3) & !3;

        // Reprogram the one-shot timer from a clean state. The hardware expects the
        // init value to be a multiple of 4, and very small deltas would otherwise be
        // truncated to zero by the CSR encoding.
        tcfg::set_en(false);
        tcfg::set_periodic(false);
