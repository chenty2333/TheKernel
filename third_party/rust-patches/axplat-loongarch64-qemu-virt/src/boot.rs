...
fn enable_fp_state() {
    // The kernel is built as soft-float on LoongArch, so it must not rely on
    // LSX in kernel context until that state is saved and restored explicitly.
    // Keep scalar FP enabled for userspace and for the task-switch FPU paths.
    #[cfg(feature = "fp-simd")]
    {
        axcpu::asm::enable_fp();
    }
...
unsafe extern "C" fn _start() -> ! {
    ...
    1:
        ...
        # Init MMU
        bl          {enable_fp_state}   # enable FP state
        ...
        enable_fp_state = sym enable_fp_state,
