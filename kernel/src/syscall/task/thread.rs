...
use crate::task::{current_user_cet_state, set_current_user_cet_state};
...
pub fn sys_arch_prctl(
    ...
    match code {
        ...
        ArchPrctlCode::EnableShstk => {
            ...
            let mut state = current_user_cet_state();
            if state.locked && state.u_cet & CET_SHSTK_EN == 0 {
                return Err(axerrno::LinuxError::EPERM.into());
            }
        ...
        ArchPrctlCode::LockShstk => {
            if addr & !ARCH_SHSTK_SHSTK != 0 || addr == 0 {
                return Err(AxError::InvalidInput);
            }
            let mut state = current_user_cet_state();
            state.locked = true;
        ...
        ArchPrctlCode::StatusShstk => {
            let state = current_user_cet_state();
            memory
