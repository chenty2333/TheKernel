...
//! comes from `DDI_BUF_CTL.BUF_TRANS_SELECT` and the four `PORT_TX_DW4` values
//! come from four different registers, that DDI A and DDI B read different
...
/// `PORT_TX_DW2`'s invented value: a swing select and a scalar, no meaning.
const DW2: u32 = 0x0000_0198;

/// The four invented `PORT_TX_DW4` values, one per lane and all different, so
/// that a read which collapsed them (the group register, a wrong stride) fails
/// the test instead of passing it.
const DW4: [u32; 4] = [0x8000_003F, 0x8000_0039, 0x8000_0037, 0x8000_0031];

/// `PORT_TX_DW7`'s invented value.
const DW7: u32 = 0x0000_0071;

/// The invented contents of `PORT_TX_DW5` after §8.5's step 6.
///
...
fn firmware_mock(ddi: Ddi) -> MockRegisters {
    ...
    regs.set(registers.tx_dw2, DW2);
    for (lane, register) in registers.tx_dw4.iter().enumerate() {
        regs.set(*register, DW4[lane]);
    }
    regs.set(registers.tx_dw5, dw5_training_enabled());
    regs.set(registers.tx_dw7, DW7);
    regs
}
...
///
/// A second level must move `level` and `source` and leave the four swing words
...
/// The four `PORT_TX_DW4` values come from four registers, in lane order.
#[test]
fn every_lane_is_read_from_its_own_register() {
    let regs = firmware_mock(Ddi::A);
    let swing = read_firmware_swing(&regs, Ddi::A).expect("a programmed port");
    assert_eq!(swing.dw4, DW4);
    // Four distinct values in four distinct registers: a group read would have
    // returned one value four times, and a wrong stride would have returned a
    // neighbour's word.
    for (lane, value) in DW4.iter().enumerate() {
        assert_eq!(swing.dw4[lane], *value, "lane {lane}");
    }
}
...
fn ddi_a_and_ddi_b_read_different_registers() {
    ...
    assert_eq!(a.tx_dw2.offset(), 0x16_2688, "PORT_TX_DW2_GRP(A)");
    assert_eq!(b.tx_dw2.offset(), 0x6_c688, "PORT_TX_DW2_GRP(B)");
    ...
    regs.set(b.tx_dw2, 0x0000_02BC);
    for (lane, register) in b.tx_dw4.iter().enumerate() {
        regs.set(*register, 0x4000_0000 | lane as u32);
    }
    regs.set(b.tx_dw5, dw5_training_enabled() | (1 << 30));
    regs.set(b.tx_dw7, 0x0000_0055);
    ...
    assert_eq!(swing_b.dw2, 0x0000_02BC);
    ...
    assert_eq!(swing_b.dw7, 0x0000_0055);
    assert_eq!(
        swing_b.source,
...
fn an_absent_phy_is_refused_by_name() {
    for dead in [0u32, u32::MAX] {
        ...
        regs.set(registers.tx_dw2, dead);
        regs.set(registers.tx_dw5, dead);
        regs.set(registers.tx_dw7, dead);
