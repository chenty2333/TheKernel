//! x86_64 Linux DRM/KMS userspace ABI definitions.
//!
//! Pointer-valued UAPI members are encoded as `u64`: they are userspace
//! addresses, never kernel pointers.  This module deliberately contains no
//! device policy or ioctl handling.
...
const fn ioc(direction: u64, number: u64, size: usize) -> u64 {
    number | (DRM_IOCTL_BASE << 8) | ((size as u64) << 16) | (direction << 30)
}
