...
const BIG_KEY_ABI_PAYLOAD_CHARGE: usize = 16;
const KEY_RESIDENT_NODE_OVERHEAD: usize = 64;
...
const MANAGER_MAX_LINK_BYTES: usize = 16 << 20;
...
struct AbiQuotaCharge {
    keys: usize,
    bytes: usize,
}

impl AbiQuotaCharge {
    const ZERO: Self = Self { keys: 0, bytes: 0 };
}
...
#[derive(Default)]
struct OwnerLedger {
    usage: BTreeMap<Kuid, OwnerUsage>,
}

impl OwnerLedger {
    fn usage(&self, uid: Kuid) -> OwnerUsage {
    ...
    fn apply(&mut self, update: Option<OwnerLedgerUpdate>) {
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
struct ManagerBudgetUsage {
    objects: usize,
    bytes: usize,
    link_bytes: usize,
}

struct ManagerBudget {
    limits: ManagerBudgetLimits,
    used: ManagerBudgetUsage,
}

impl ManagerBudget {
    const fn new(limits: ManagerBudgetLimits) -> Self {
        Self {
            limits,
            used: ManagerBudgetUsage {
                objects: 0,
    ...
    fn apply(&mut self, after: ManagerBudgetUsage) {
        self.used = after;
    }

    fn check_transient(&self, additional: ResidentCharge) -> AxResult<()> {
...
#[derive(Clone, Copy)]
struct AccountingPlan {
    owner: Option<OwnerLedgerUpdate>,
    budget: ManagerBudgetUsage,
}
...
impl KeyTypeKind {
    ...
    const fn abi_payload_charge(self, payload_len: usize) -> usize {
        match self {
...
fn wipe_key_bytes(bytes: &mut [u8]) {
    for byte in bytes {
...
impl Key {
    ...
    fn ongoing_quota_admission(&self) -> QuotaAdmission {
    ...
    fn payload_charges(&self, payload: &Vec<u8>) -> AxResult<(AbiQuotaCharge, ResidentCharge)> {
    ...
    fn next_link_capacity(&self) -> AxResult<Option<usize>> {
    ...
    fn stage_link_push(&self, serial: i32, new_capacity: usize) -> AxResult<Vec<i32>> {
...
impl KeyManager {
    ...
    fn key_user_records(&self) -> Vec<KeyUserRecord> {
...
fn validate_key_quota_limit(value: usize) -> AxResult<usize> {
