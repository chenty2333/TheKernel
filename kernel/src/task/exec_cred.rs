...
/// Strictly parsed `security.capability` payload.
///
/// Revisions 1 and 2 are rooted at kernel UID 0. Revision 3 carries an
/// explicit kernel-global root ID because TheKernel does not yet implement
/// idmapped mounts or per-superblock user namespaces.
...
fn read_le_u32(value: &[u8], offset: usize) -> AxResult<u32> {
    let bytes = value
        .get(offset..offset + core::mem::size_of::<u32>())
        .ok_or(AxError::InvalidInput)?;
    Ok(u32::from_le_bytes(
        bytes.try_into().map_err(|_| AxError::InvalidInput)?,
    ))
}

/// Validates the complete Linux v1/v2/v3 on-disk file-capability record.
pub(crate) fn parse_file_capabilities(value: &[u8]) -> AxResult<FileCapabilities> {
    ...
    let rootid = if revision == VFS_CAP_REVISION_3 {
        Kuid::from_raw(read_le_u32(value, 20)?).ok_or(AxError::InvalidInput)?
    } else {
        Kuid::INITIAL_ROOT
    };
...
mod tests {
    ...
    use super::*;
    use crate::task::{CredentialSlot, IdMapInputExtent, creds::GroupInfo};
    ...
    fn ambient_survives_ordinary_exec_and_clears_for_privileged_file() {
        ...
        privileged.file_capabilities = Some(FileCapabilities::new(
