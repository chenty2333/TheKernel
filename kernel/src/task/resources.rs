...
use linux_raw_sys::general::{
    RLIM_INFINITY, RLIM_NLIMITS, RLIMIT_CORE, RLIMIT_NICE, RLIMIT_NOFILE, RLIMIT_RTPRIO,
...
impl Default for Rlimits {
    fn default() -> Self {
        ...
        // Linux grants no unprivileged real-time priority increase by
        // default.  This must not inherit the generic infinity default.
        result[RLIMIT_RTPRIO] = 0.into();
...
mod tests {
    use linux_raw_sys::general::{
        RLIMIT_CORE, RLIMIT_NICE, RLIMIT_NOFILE, RLIMIT_RTPRIO, RLIMIT_SIGPENDING,
    };
    ...
    #[test]
    fn rtprio_default_grants_no_unprivileged_rt_raise() {
        let limits = Rlimits::default();
        assert_eq!(limits[RLIMIT_RTPRIO].current, 0);
        assert_eq!(limits[RLIMIT_RTPRIO].max, 0);
    }
}
