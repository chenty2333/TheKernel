...
`[MEASURED]` 273 `drm::intel` host tests pass, 36 of them this module's, none failing (`test result:
ok. 273 passed; 0 failed`; the previous revision of this document recorded 197 and 29). The revision
that added the DDI B sequence also ran `tools/thekernel.py lint` — Clippy for the product kernel
configuration, `x86_64-unknown-none`, release — which reports nothing in `output.rs`,
`output/tests.rs`, `regs/dpll.rs` or `regs/table/mod.rs`; it does report 350 pre-existing warnings
elsewhere in the kernel, none of them this module's. The host test target's own Clippy run recorded
by the previous revision was not repeated in this one.
