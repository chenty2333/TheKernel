...
`[MEASURED]` the `drm::intel` host suite passes with none failing.  This module's share of it is
counted in the integration report rather than here: three workstreams added to that count
independently -- the DDI B sequence, the corrected ADL-N PLL divider search, and the per-lane swing
batch -- and a number copied from any one of them is wrong for the merge.  (Earlier revisions of
...
`output/tests.rs`, `regs/dpll.rs` or `regs/table/mod.rs`.

The module's tests assert, among other things:
