...
enum {
    ...
    ASYNC_DIRECT_IOV_SEGMENTS = 8,
    ASYNC_DIRECT_MANY_SEGMENTS = 65,
    ASYNC_DIRECT_SIGNAL_SEGMENTS = 64,
    ASYNC_DIRECT_IOV_SIZE = TEST_PAGE_SIZE * ASYNC_DIRECT_IOV_SEGMENTS,
    ASYNC_DIRECT_MANY_SIZE = TEST_PAGE_SIZE * ASYNC_DIRECT_MANY_SEGMENTS,
    ASYNC_DIRECT_SIGNAL_SIZE = TEST_PAGE_SIZE * ASYNC_DIRECT_SIGNAL_SEGMENTS,
    SG_ATTEMPTS = 256,
    SG_FRAG_PAGES = 128,
};
...
static void *preadv_worker(void *arg)
{
    struct readv_task *task = (struct readv_task *)arg;

    errno = 0;
    task->result = preadv(task->fd, task->iov, task->iovcnt, task->offset);
    task->err = errno;
    return NULL;
}
...
static int write_io_stats_command(const char *command)
{
    size_t len = strlen(command);
    int fd = open("/proc/io_stats", O_WRONLY);
    ssize_t n;
...
static char *alloc_pages_len(size_t len)
{
...
static int read_full_at(int fd, void *buf, size_t len, off_t offset)
{
    char *cur = (char *)buf;
    size_t done = 0;

    while (done < len) {
        ssize_t n = pread(fd, cur + done, len - done, offset + (off_t)done);
        if (n < 0) {
            if (errno == EINTR) {
                continue;
            }
...
static int create_pattern_file(const char *path, size_t size,
                               unsigned char (*pattern)(size_t))
{
    ...
    while (done < size) {
        ...
        fill_pattern_at(page, chunk, done, pattern);
        if (write_full(fd, page, chunk) != 0) {
            close(fd);
            unlink(path);
            return -1;
        }
...
static int verify_file_pattern(const char *path, size_t size,
                               unsigned char (*pattern)(size_t))
{
    unsigned char *buf;
    int fd;
    ...
    buf = (unsigned char *)alloc_pages_len(size);
    if (!buf) {
        close(fd);
        return -1;
    }
    ok = read_full_at(fd, buf, size, 0) == 0
         && verify_pattern(buf, size, pattern) == 0;
    munmap(buf, size);
    close(fd);
    return ok ? 0 : -1;
}
...
static int test_async_direct_contig(void)
{
    const char *src_path = "/user_direct_async_src";
    const char *out_path = "/user_direct_async_contig_out";
    char *read_buf = NULL;
    char *write_buf = NULL;
    int fd = -1;
    int out_fd = -1;
    ...
    read_buf = alloc_pages_len(TEST_PAGE_SIZE);
    write_buf = alloc_pages_len(TEST_PAGE_SIZE);
    if (!read_buf || !write_buf) {
        printf("USER_DIRECT_ASYNC_FAIL contig mmap errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    ...
    fd = -1;
    if (n != TEST_PAGE_SIZE
        || verify_pattern((unsigned char *)read_buf, TEST_PAGE_SIZE, test_file_pattern) != 0
        || read_after <= read_base) {
        printf("USER_DIRECT_ASYNC_FAIL contig_read result=%ld errno=%d before=%lld after=%lld\n",
               (long)n, saved_errno, read_base, read_after);
        fflush(stdout);
        goto fail;
    }
    ...
    if (out_fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL contig open_write errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    ...
    if (n != TEST_PAGE_SIZE || write_after <= write_base
        || verify_file_pattern(out_path, TEST_PAGE_SIZE, sg_write_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL contig_write result=%ld errno=%d before=%lld after=%lld\n",
               (long)n, saved_errno, write_base, write_after);
        fflush(stdout);
        goto fail;
    }
    printf("USER_DIRECT_ASYNC_CONTIG_WRITE_OK\n");
    fflush(stdout);

    munmap(read_buf, TEST_PAGE_SIZE);
    munmap(write_buf, TEST_PAGE_SIZE);
    unlink(src_path);
    unlink(out_path);
    return 0;
...
static void make_page_iov(struct iovec *iov, char *buf, int count)
{
    for (int i = 0; i < count; ++i) {
        iov[i].iov_base = buf + (size_t)i * TEST_PAGE_SIZE;
        iov[i].iov_len = TEST_PAGE_SIZE;
    }
}

static int test_async_direct_iov(void)
{
    const char *src_path = "/user_direct_async_iov_src";
    const char *out_path = "/user_direct_async_iov_out";
    struct iovec iov[ASYNC_DIRECT_IOV_SEGMENTS];
    char *read_buf = NULL;
    char *write_buf = NULL;
    int fd = -1;
    int out_fd = -1;
    ...
    if (create_pattern_file(src_path, ASYNC_DIRECT_IOV_SIZE, test_file_pattern) != 0
        || create_pattern_file(out_path, ASYNC_DIRECT_IOV_SIZE, test_file_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL iov create errno=%d\n", errno);
        fflush(stdout);
        return -1;
    }

    read_buf = alloc_pages_len(ASYNC_DIRECT_IOV_SIZE);
    write_buf = alloc_pages_len(ASYNC_DIRECT_IOV_SIZE);
    if (!read_buf || !write_buf) {
        printf("USER_DIRECT_ASYNC_FAIL iov mmap errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }

    make_page_iov(iov, read_buf, ASYNC_DIRECT_IOV_SEGMENTS);
    fd = open_direct(src_path, O_RDONLY);
    if (fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL iov open_read errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    read_base = read_io_counter("user_pin.async_direct_read_hits");
    ...
    if (n != ASYNC_DIRECT_IOV_SIZE
        || verify_pattern((unsigned char *)read_buf, ASYNC_DIRECT_IOV_SIZE,
                          test_file_pattern) != 0
        || read_after <= read_base) {
        printf("USER_DIRECT_ASYNC_FAIL iov_read result=%ld errno=%d before=%lld after=%lld\n",
               (long)n, saved_errno, read_base, read_after);
        fflush(stdout);
        goto fail;
    }
    printf("USER_DIRECT_ASYNC_IOV_READ_OK\n");
    fflush(stdout);

    fill_pattern((unsigned char *)write_buf, ASYNC_DIRECT_IOV_SIZE, sg_write_pattern);
    make_page_iov(iov, write_buf, ASYNC_DIRECT_IOV_SEGMENTS);
    out_fd = open_direct(out_path, O_RDWR);
    if (out_fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL iov open_write errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    write_base = read_io_counter("user_pin.async_direct_write_hits");
    ...
    if (n != ASYNC_DIRECT_IOV_SIZE || write_after <= write_base
        || verify_file_pattern(out_path, ASYNC_DIRECT_IOV_SIZE, sg_write_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL iov_write result=%ld errno=%d before=%lld after=%lld\n",
               (long)n, saved_errno, write_base, write_after);
        fflush(stdout);
        goto fail;
    }
    printf("USER_DIRECT_ASYNC_IOV_WRITE_OK\n");
    fflush(stdout);

    munmap(read_buf, ASYNC_DIRECT_IOV_SIZE);
    munmap(write_buf, ASYNC_DIRECT_IOV_SIZE);
    unlink(src_path);
    unlink(out_path);
    return 0;
...
static int test_async_direct_file_mmap(void)
{
    const char *src_path = "/user_direct_async_mmap_src";
    const char *out_path = "/user_direct_async_mmap_out";
    const char *map_path = "/user_direct_async_mmap_buf";
    unsigned char zero[TEST_PAGE_SIZE];
    ...
    if (create_pattern_file(src_path, TEST_PAGE_SIZE, test_file_pattern) != 0
        || create_pattern_file(out_path, TEST_PAGE_SIZE, test_file_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap create_data errno=%d\n", errno);
        fflush(stdout);
        return -1;
    }

    unlink(map_path);
    map_fd = open(map_path, O_CREAT | O_TRUNC | O_RDWR, 0600);
    if (map_fd < 0 || write_full(map_fd, zero, sizeof(zero)) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap create_map errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    mapped = mmap(NULL, TEST_PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, map_fd, 0);
    if (mapped == MAP_FAILED) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap mmap errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    ...
    fd = open_direct(src_path, O_RDONLY);
    if (fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap open_read errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    read_base = read_io_counter("user_pin.async_direct_read_hits");
    ...
    fd = -1;
    if (n != TEST_PAGE_SIZE
        || verify_pattern((unsigned char *)mapped, TEST_PAGE_SIZE, test_file_pattern) != 0
        || read_after <= read_base) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap_read result=%ld errno=%d before=%lld after=%lld\n",
               (long)n, saved_errno, read_base, read_after);
        fflush(stdout);
        goto fail;
    }
    printf("USER_DIRECT_ASYNC_FILE_MMAP_READ_OK\n");
    fflush(stdout);

    fill_pattern((unsigned char *)mapped, TEST_PAGE_SIZE, sg_write_pattern);
    out_fd = open_direct(out_path, O_RDWR);
    if (out_fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap open_write errno=%d\n", errno);
        fflush(stdout);
        goto fail;
    }
    write_base = read_io_counter("user_pin.async_direct_write_hits");
    ...
    if (n != TEST_PAGE_SIZE || write_after <= write_base
        || verify_file_pattern(out_path, TEST_PAGE_SIZE, sg_write_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL file_mmap_write result=%ld errno=%d before=%lld after=%lld\n",
               (long)n, saved_errno, write_base, write_after);
        fflush(stdout);
        goto fail;
    }
    printf("USER_DIRECT_ASYNC_FILE_MMAP_WRITE_OK\n");
    fflush(stdout);

    munmap(mapped, TEST_PAGE_SIZE);
    close(map_fd);
    unlink(src_path);
    unlink(out_path);
    unlink(map_path);
...
static int test_async_direct_unaligned_fallback(void)
{
    const char *path = "/user_direct_async_unaligned";
    char *buf = NULL;
    int fd = -1;
    ssize_t n;
    int saved_errno;
    ...
    if (create_pattern_file(path, TEST_FILE_SIZE, test_file_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL unaligned create errno=%d\n", errno);
        fflush(stdout);
        return -1;
    }
    buf = alloc_pages_len(TEST_PAGE_SIZE * 2);
    if (!buf) {
        printf("USER_DIRECT_ASYNC_FAIL unaligned mmap errno=%d\n", errno);
        fflush(stdout);
        unlink(path);
        return -1;
    }
    fd = open(path, O_RDONLY);
    if (fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL unaligned open errno=%d\n", errno);
        fflush(stdout);
        munmap(buf, TEST_PAGE_SIZE * 2);
        unlink(path);
        return -1;
    }
    base = read_io_counter("user_pin.async_submit_fallbacks");
    errno = 0;
    n = pread(fd, buf + 1, TEST_PAGE_SIZE, 0);
    saved_errno = errno;
    after = read_io_counter("user_pin.async_submit_fallbacks");
    close(fd);
    munmap(buf, TEST_PAGE_SIZE * 2);
    unlink(path);
    if (n == TEST_PAGE_SIZE && after > base) {
        printf("USER_DIRECT_ASYNC_UNALIGNED_FALLBACK_OK\n");
        fflush(stdout);
        return 0;
    }
    printf("USER_DIRECT_ASYNC_FAIL unaligned result=%ld errno=%d before=%lld after=%lld\n",
           (long)n, saved_errno, base, after);
    fflush(stdout);
    return -1;
}

static int test_async_direct_many_segments_fallback(void)
{
    const char *path = "/user_direct_async_many";
    struct iovec iov[ASYNC_DIRECT_MANY_SEGMENTS];
    char *buf = NULL;
    int fd = -1;
    ssize_t n;
    int saved_errno;
    ...
    if (create_pattern_file(path, ASYNC_DIRECT_MANY_SIZE, test_file_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL many create errno=%d\n", errno);
        fflush(stdout);
        return -1;
    }
    buf = alloc_pages_len(ASYNC_DIRECT_MANY_SIZE);
    if (!buf) {
        printf("USER_DIRECT_ASYNC_FAIL many mmap errno=%d\n", errno);
        fflush(stdout);
        unlink(path);
        return -1;
    }
    make_page_iov(iov, buf, ASYNC_DIRECT_MANY_SEGMENTS);
    fd = open(path, O_RDONLY);
    if (fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL many open errno=%d\n", errno);
        fflush(stdout);
        munmap(buf, ASYNC_DIRECT_MANY_SIZE);
        unlink(path);
        return -1;
    }
    base = read_io_counter("user_pin.async_submit_fallbacks");
    errno = 0;
    n = preadv(fd, iov, ASYNC_DIRECT_MANY_SEGMENTS, 0);
    saved_errno = errno;
    after = read_io_counter("user_pin.async_submit_fallbacks");
    close(fd);
    if (n == ASYNC_DIRECT_MANY_SIZE
        && verify_pattern((unsigned char *)buf, ASYNC_DIRECT_MANY_SIZE, test_file_pattern) == 0
        && after > base) {
        printf("USER_DIRECT_ASYNC_TOO_MANY_SEGMENTS_FALLBACK_OK\n");
        fflush(stdout);
        munmap(buf, ASYNC_DIRECT_MANY_SIZE);
        unlink(path);
        return 0;
    }
    printf("USER_DIRECT_ASYNC_FAIL many result=%ld errno=%d before=%lld after=%lld\n",
           (long)n, saved_errno, base, after);
    fflush(stdout);
    munmap(buf, ASYNC_DIRECT_MANY_SIZE);
    unlink(path);
...
static int test_async_direct_signal_after_submit(void)
{
    const char *path = "/user_direct_async_signal";
    struct sigaction action;
    struct sigaction old_action;
    struct iovec iov[ASYNC_DIRECT_SIGNAL_SEGMENTS];
    char *buf = NULL;
    int fd = -1;
    int saw_signal_counter = 0;

    if (create_pattern_file(path, ASYNC_DIRECT_SIGNAL_SIZE, test_file_pattern) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL signal create errno=%d\n", errno);
        fflush(stdout);
        return -1;
    }
    ...
    sigemptyset(&action.sa_mask);
    if (sigaction(SIGUSR1, &action, &old_action) != 0) {
        printf("USER_DIRECT_ASYNC_FAIL signal sigaction errno=%d\n", errno);
        fflush(stdout);
        unlink(path);
        return -1;
    }

    fd = open_direct(path, O_RDONLY);
    if (fd < 0) {
        printf("USER_DIRECT_ASYNC_FAIL signal open errno=%d\n", errno);
        fflush(stdout);
        sigaction(SIGUSR1, &old_action, NULL);
        unlink(path);
        return -1;
    ...
    for (int attempt = 0; attempt < 16; ++attempt) {
        struct readv_task task;
        pthread_t thread;
        long long submit_base;
        long long signal_base;
        long long signal_after;
        int rc;

        buf = alloc_pages_len(ASYNC_DIRECT_SIGNAL_SIZE);
        if (!buf) {
            printf("USER_DIRECT_ASYNC_FAIL signal mmap errno=%d\n", errno);
            fflush(stdout);
            close(fd);
            sigaction(SIGUSR1, &old_action, NULL);
            unlink(path);
        ...
        make_page_iov(iov, buf, ASYNC_DIRECT_SIGNAL_SEGMENTS);
        memset(&task, 0, sizeof(task));
        task.fd = fd;
        task.iov = iov;
        task.iovcnt = ASYNC_DIRECT_SIGNAL_SEGMENTS;
        task.offset = 0;

        submit_base = read_io_counter("virtio.blk_async_submit_requests");
        signal_base = read_io_counter("user_pin.async_signal_after_submit");
        if (submit_base < 0 || signal_base < 0) {
            printf("USER_DIRECT_ASYNC_FAIL signal missing_counter\n");
            fflush(stdout);
            munmap(buf, ASYNC_DIRECT_SIGNAL_SIZE);
            close(fd);
            sigaction(SIGUSR1, &old_action, NULL);
            unlink(path);
            return -1;
        }
        if (pthread_create(&thread, NULL, preadv_worker, &task) != 0) {
            printf("USER_DIRECT_ASYNC_FAIL signal pthread_create errno=%d\n", errno);
            fflush(stdout);
            munmap(buf, ASYNC_DIRECT_SIGNAL_SIZE);
            close(fd);
            sigaction(SIGUSR1, &old_action, NULL);
        ...
        if (wait_counter_gt("virtio.blk_async_submit_requests", submit_base) == 0) {
            rc = pthread_kill(thread, SIGUSR1);
            if (rc != 0) {
                printf("USER_DIRECT_ASYNC_FAIL signal pthread_kill rc=%d\n", rc);
                fflush(stdout);
            }
        }
        if (pthread_join(thread, NULL) != 0) {
            printf("USER_DIRECT_ASYNC_FAIL signal pthread_join\n");
            fflush(stdout);
            munmap(buf, ASYNC_DIRECT_SIGNAL_SIZE);
            close(fd);
            sigaction(SIGUSR1, &old_action, NULL);
        ...
        signal_after = read_io_counter("user_pin.async_signal_after_submit");
        if (task.result == ASYNC_DIRECT_SIGNAL_SIZE
            && verify_pattern((unsigned char *)buf, ASYNC_DIRECT_SIGNAL_SIZE,
                              test_file_pattern) == 0
            && signal_after > signal_base) {
            saw_signal_counter = 1;
            munmap(buf, ASYNC_DIRECT_SIGNAL_SIZE);
            break;
        }
        munmap(buf, ASYNC_DIRECT_SIGNAL_SIZE);
    ...
    if (saw_signal_counter) {
        printf("USER_DIRECT_ASYNC_SIGNAL_AFTER_SUBMIT_OK\n");
        fflush(stdout);
        return 0;
    }

    printf("USER_DIRECT_ASYNC_SIGNAL_AFTER_SUBMIT_MISSED\n");
    fflush(stdout);
    return 0;
}

static int run_async_direct_tests(int include_signal_after_submit)
{
    int ok = 1;
    ...
    if (include_signal_after_submit) {
        (void)test_async_direct_signal_after_submit();
    } else {
        printf("USER_DIRECT_ASYNC_SIGNAL_AFTER_SUBMIT_SKIPPED\n");
        fflush(stdout);
    }

    if (ok) {
        printf("USER_DIRECT_ASYNC_OK\n");
        fflush(stdout);
        return 0;
    }

    printf("USER_DIRECT_ASYNC_FAIL\n");
    fflush(stdout);
    return 1;
}
