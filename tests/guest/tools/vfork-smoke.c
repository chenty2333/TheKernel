 ...
 * exec case likewise checks the child's raw sleep duration before exec and
 * immediately checks the child's /proc/<pid>/exe identity after vfork
 * returns.  The signal case checks a raw monotonic sleep before publishing
 ...
 * resumed during any userspace window still sees the old phase/image, so it
 * cannot pass the boundary checks.
 */
...
#define EXEC_TARGET_DEFAULT "/bin/busybox"
...
static int proc_exe_path(pid_t child, char *path, size_t path_size)
{
    char link_path[64];
...
static int test_exec_release(const char *self)
{
    ...
    /* This must be the first parent-side operation after vfork returns.  It
     * deliberately does not wait for a pipe event or for child termination. */
    if (proc_exe_path(child, child_exe, sizeof(child_exe)) != 0 ||
        stat(child_exe, &child_stat) != 0) {
