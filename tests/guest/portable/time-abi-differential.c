 ...
 *   kernel/time/timekeeping.c:1665     do_settimeofday64 bounds and floor
 ...
 *   kernel/time/posix-cpu-timers.c:159 CPU clock resolutions per CPUCLOCK_WHICH
 ...
 * RTC driver is loaded are deliberately avoided: the timex offset round trips
 * below use values that are exact for every supported HZ, the CPU clock
 * resolutions are only compared against one nanosecond, and the wake-alarm
 * clock is only asserted through its copy-out ordering.  The last case drops
 * to an unprivileged uid to observe the settimeofday(2) validation order and
 * runs after every privileged case.
 */
#define _GNU_SOURCE
...
/* Seconds between two realtime samples, as a signed count. */
...
static int case_gettimeofday(void) {
     ...
     * gettimeofday(NULL, tz) then reports, and it does not touch the clock. */
    ...
    if (raw_clock_gettime(CLOCK_REALTIME, &after) != 0 ||
        realtime_delta(&before, &after) > 2000000000L) {
        errno = EPROTO;
        return fail("timezone-store-touched-clock");
    }
