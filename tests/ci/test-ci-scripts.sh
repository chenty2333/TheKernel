...
env \
    ...
    OSCOMP_TESTSUITE_HOST_DIR="$tmp/dev-testsuites" \
...
# Hosted CI must consume exact, published sibling commits without mutating an
...
case "$*" in
    ...
    'kernel-rv-shell kernel-la-shell')
        mkdir -p .state/shell
...
[ "$(sed -n '3p' "$pr_trace")" = 'make kernel-rv-shell kernel-la-shell' ]
